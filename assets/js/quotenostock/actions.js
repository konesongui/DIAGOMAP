...
var dtID = 'quoteDatatable',
  formID = 'quoteForm',
  formEditID = 'quoteEditForm',
  dtButtons = {
      set: 'submitBtn',
      edit: 'submitEditBtn',
  },
  remoteAJAXFunctions = {
      loadData: 'admin/quotenostock/data',
      add: 'admin/quotenostock/add',
      edit: 'admin/quotenostock/update',
  };
...
$(document).ready(function() {
    ...
    $('.repeater-item').each(function() {
        ...
        var itemName = $item.find('.item-select').val();
    ...
    function populateItem(itemSelect, categoryId, selectedItemId = null) {
        if (categoryId) {
            ...
            container.find('.item-select').val('');
            
            console.log('Chargement des articles pour la catégorie:', categoryId);
            
            $.ajax({
                type: "GET",
                url: base_url + "admin/quotenostock/getItemByCategory",
                ...
                success: function (data) {
                    ...
                    var selectedItem = data.find(item => item.id == selectedItemId);
                    if (selectedItem) {
                        // Mettre à jour les champs avec les données de l'article
                        container.find('.item-select').val(selectedItem.name);
                        container.find('.unit').val(selectedItem.unit || '');
                        container.find('.price').val(selectedItem.weighted_avg_price || '');
                        container.find('.available-qty').text(selectedItem.current_quantity || 0);
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            Swal.fire('Succès', response.message, 'success').then(() => {
                                window.location.href = base_url + 'admin/quotenostock';
                            });
                        } else {
    ...
    $(document).on('click', "#"+dtButtons.edit, function(e) {
        ...
        $.ajax({
            url: baseurl + 'admin/quotenostock/update',
            type: 'POST',
            data: $form.serialize(),
            dataType: 'json',
            success: function(response) {

...
var dtID = 'deliveryDatatable',
  ...
  remoteAJAXFunctions = {
      ...
      add: 'admin/deliveryitem/add',
      edit: 'admin/deliveryitem/update',
  };
...
$(document).ready(function() {
    ...
    $(document).on('click', "#"+dtButtons.edit, function(e) {
        ...
        $.ajax({
            url: baseurl + 'admin/deliveryitem/update',
            type: 'POST',
            data: $form.serialize(),
            dataType: 'json',
            success: function(response) {
    ...
    $(document).on('click', "#"+dtButtons.validate, function(e) {
        ...
        // Envoyer la requête AJAX
        $.ajax({
            url: baseurl + 'admin/deliveryitem/validate',
            type: 'POST',
            data: $form.serialize(),
            dataType: 'json',
            success: function(response) {
                if (response.status === 'success') {
                    Swal.fire({
                        ...
                        window.location.href = base_url + 'admin/de';
                    });
                } else {
                    let errorMessage = response.message || 'Une erreur est survenue';
                    if (response.error) {

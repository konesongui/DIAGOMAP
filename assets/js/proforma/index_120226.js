...
var dtID = 'quoteDatatable',
    remoteAJAXFunctions = {
        ...
        duplicate: 'admin/quoteitem/duplicate' // NOUVELLE FONCTIONNALITÉ
    };
...
$(document).ready(function() {
            ...
            { "data": "user_name" },
            {
                ...
                render: function(data, type, row) {
                    ...
                    actions += `
                        ...
                        `;
                    // NOUVELLE OPTION : Dupliquer (disponible pour tous les statuts sauf brouillon)
                    // Dans la colonne actions, AJOUTEZ cette option :
                    if(parseInt(row.status.code) !== 0) { // 0 = brouillon, 1 = validé, etc.
                        actions += `
        <li>
    ...
    $('.' + dtID).on('click', '.edit-quote', function(e) {
        ...
        if (parseInt(statusCode) === 1) {
            if (!confirm("Ce devis est déjà validé. Voulez-vous vraiment le modifier ?")) {
                return;
            }
        }
...
// NOUVELLE FONCTION : Dupliquer un devis
    $('.' + dtID).on('click', '.duplicate-quote', function(e) {
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                return fetch(base_url + 'admin/quoteitem/duplicate/' + quoteId, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
                    body: new URLSearchParams({
                        // Si vous utilisez CSRF, ajoutez le token ici

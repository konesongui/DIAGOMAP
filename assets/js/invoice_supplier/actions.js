...
var dtID = 'invoiceDatatable',
    formID = 'invoiceForm',
    formEditID = 'invoiceEditForm',
    dtButtons = {
        set: 'submitBtn',
        edit: 'submitEditBtn',
    },
    remoteAJAXFunctions = {
        add: 'admin/invoiceitem_supplier/add',
        edit: 'admin/invoiceitem_supplier/update',
    };
...
$(document).ready(function() {
    ...
    // Dans votre $(document).ready(), remplacez la partie de chargement initial par :
    $('.repeater-item').each(function() {
        ...
        if (categoryId && itemId) {
            ...
            // Force le calcul du total si les valeurs existent
            if (quantity && price) {
        ...
        if (categoryId) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (Array.isArray(data)) {
                        $.each(data, function (i, obj) {
                            ...
                            options += `<option value='${obj.id}' 
                                          data-available='${obj.current_quantity || 0}'
                                          data-unit='${obj.unit || ''}'
                                          data-price='${obj.weighted_avg_price || 0}'
                                          ${selected}>
                                          ${obj.name}
                                      </option>`;
                        });
                    ...
                    // Si un article est sélectionné, déclencher le changement
                    if (selectedItemId) {
                        container.find('.item-select').trigger('change');
                    }
                },
                error: function(xhr) {
                    console.error('Erreur de chargement des articles:', xhr.responseText);
    ...
    function calculateGrandTotal() {
        ...
        $('#total_ht').val(grandTotal.toFixed(2));
        $('#totalHT').val(grandTotal.toFixed(2));

        // Calcul de la TVA si cochée
        if ($('#apply_tva').is(':checked')) {
            var tvaRate = parseFloat($('#tva_rate').val()) / 100 || 0;
            var tvaAmount = grandTotal * tvaRate;
            var totalTTC = grandTotal + tvaAmount;

            // Mise à jour des champs TVA et TTC
            $('#tva_amount').val(tvaAmount.toFixed(2));
            $('#tvaAmount').val(tvaAmount.toFixed(2));
            $('#total_ttc').val(totalTTC.toFixed(2));
            $('#totalTTC').val(totalTTC.toFixed(2));

            // Affichage des lignes TVA et TTC
            $('.tva-row').show();
            $('.ttc-row').show();
        } else {
            // Si TVA non cochée
            $('#tva_amount').val('0.00');
            $('#tvaAmount').val('0.00');
            $('#total_ttc').val(grandTotal.toFixed(2));
            $('#totalTTC').val(grandTotal.toFixed(2));

            // Masquage des lignes TVA et TTC
            $('.tva-row').hide();
            $('.ttc-row').hide();
        }
    }

    // Déclencheur pour le changement de taux de TVA
    $(document).on('input', '#tva_rate', function() {
        calculateGrandTotal();
    });

    // Déclencheur pour la case à cocher TVA
    $(document).on('change', '#apply_tva', function() {
        calculateGrandTotal();
    });

    // Déclencheur initial au chargement de la page
    $(document).ready(function() {
        calculateGrandTotal();
    });
    ...
    $(document).on('click', "#"+dtButtons.set, function(e) {
        ...
        Swal.fire({
            title: 'Confirmation',
            text: "Voulez-vous enregistrer cette facture ?",
            icon: 'question',
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            Swal.fire('Succès', response.message, 'success').then(() => {
                                window.location.href = base_url + 'admin/invoiceitem';
                            });
                        } else {
    ...
    $(document).on('click', "#"+dtButtons.edit, function(e) {
        ...
        $.ajax({
            url: baseurl + 'admin/invoiceitem_supplier/update',
            type: 'POST',
            data: $form.serialize(),
            dataType: 'json',
            success: function(response) {
                if (response.status === 'success') {
                    Swal.fire({
                        ...
                        window.location.href = base_url + 'admin/invoiceitem_supplier';
                    });
                } else {

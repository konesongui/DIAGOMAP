...
var dtID = 'orderDatatable',
  ...
  dtButtons = {
      set: 'submitBtn',
      edit: 'submitEditBtn',
      validate: 'submitValidateBtn',
  },
  remoteAJAXFunctions = {
      loadData: 'admin/orderformitem/data',
      add: 'admin/orderformitem/add',
      edit: 'admin/orderformitem/update',
      validate: 'admin/orders_purchases/setPartialDelivery',
  };
...
$(document).ready(function() {
    ...
    function validateForm() {
        ...
        $('.repeater-item').each(function() {
            ...
            const quantity_delivered = $item.find('.quantity_delivered').val();

            if (category || itemId || quantity || quantity_delivered) {
                hasItems = true;
                if (!category) {
                ...
                if (!quantity_delivered || quantity_delivered <= 0) {
                    errors.push('La quantité livrée doit être supérieure à 0 pour tous les articles');
                    isValid = false;
                }
            }
        });
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            Swal.fire('Succès', response.message, 'success').then(() => {
                                window.location.href = base_url + 'admin/orderformitem';
                            });
                        } else {
    ...
    $(document).on('click', "#"+dtButtons.edit, function(e) {
        ...
        $.ajax({
            url: baseurl + 'admin/orderformitem/update',
            type: 'POST',
            data: $form.serialize(),
            dataType: 'json',
            success: function(response) {
    ...
    $(document).on('click', "#"+dtButtons.validate, function(e) {
        ...
        // Envoyer la requête AJAX
        $.ajax({
            url: baseurl + '/'+ remoteAJAXFunctions.validate,
            type: 'POST',
            data: $form.serialize(),
            dataType: 'json',
            success: function(response) {
                if (response.status === 'success') {
                    Swal.fire({
                        ...
                        window.location.href = base_url + 'admin/deliveryitem';
                    });
                } else {
                    let errorMessage = response.message || 'Une erreur est survenue';
                    if (response.error) {
    ...
    $('.quantity_delivered').on('input', function() {

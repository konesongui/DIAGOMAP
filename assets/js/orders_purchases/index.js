...
var dtID = 'ordersDatatable',
    remoteAJAXFunctions = {
        loadData: 'admin/orders_purchases/data',
        reject: 'admin/orders_purchases/reject',
        validate: 'admin/orders_purchases/validate',
        view: 'admin/orders_purchases/view',
        edit: 'admin/orders_purchases/edit',
        print: 'admin/orders_purchases/print',
        sendEmail: 'admin/orders_purchases/sendEmail',
        partialDelivery: 'admin/orders_purchases/partialDelivery',
        completeDelivery: 'admin/orders_purchases/completeDelivery',
        cancelDelivery: 'admin/orders_purchases/cancelDelivery'
...
$(document).ready(function() {
    ...
    $(document).on('change', `#statusFilter`, function(e) {
        ...
        e.preventDefault();
        ordersTable.ajax.reload(null, false); // Reload data
    });
    ...
    var ordersTable = $('.'+ dtID).DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": {
        ...
        "columns": [
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    actions += `
                           <li>
                                <a class="dropdown-item edit-orders" href="${baseurl}admin/orders_purchases/edit/${row.id}">
                                    <i class="fa fa-eye me-2"></i> Modifier
                                </a>
                            </li>`;
                    ...
                    actions += `
                            <li>
                                <a class="dropdown-item view-orders" href="#" data-id="${row.id}">
                    ...
                    actions += `
                            <li>
                                <a class="dropdown-item print-orders" href="#" data-id="${row.id}">
                    ...
                    actions += `
                            <li>
                                <a class="dropdown-item send-orders" href="#" data-id="${row.id}">
                    ...
                    if(parseInt(row.status.code) === 1) {
                        actions += `
                            <li>
                                <a class="dropdown-item validate-orders" href="#" data-id="${row.id}">
                                    <i class="fa fa-check me-2"></i> Valider
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item reject-orders" href="#" data-id="${row.id}">
    ...
    // Voir un devis
    $('.' + dtID).on('click', '.view-orders', function() {
    ...
    // Modifier un devis
    $('.' + dtID).on('click', '.edit-orders', function() {
        var quoteId = $(this).data('id');
    ...
    // Rejeter un devis
    $('.' + dtID).on('click', '.validate-orders', function() {
        var quoteId = $(this).data('id');
        ...
        Swal.fire({
            ...
            icon: 'warning',
            input: 'text',
            inputPlaceholder: 'Mot de confirmation',
            inputAttributes: {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(response.status == "success") {
                            Toast.fire({
                                icon: 'success',
                                title: 'Bon de commande validé avec succès'
                            });
                            refreshTable();
                        } else {
                            Toast.fire({
    ...
    // Rejeter un devis
    $('.' + dtID).on('click', '.reject-orders', function() {
        var quoteId = $(this).data('id');
    ...
    $('.' + dtID).on('click', '.print-orders', function(e) {
        ...
        if(quoteId > 0) {
            ...
            Swal.fire({
                title: 'Attention',
                text: 'Veuillez sélectionner un bon de commande',
                icon: 'warning'
            });
        }
    });
    ...
    $('.' + dtID).on('click', '.send-orders', function(e) {
        e.preventDefault();
        var quoteId = $(this).data('id');
        
        Swal.fire({
            title: 'Envoi du bon de commande par email',
            text: "Voulez-vous envoyer ce bon de commande par email ?",
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',

            ...
            $.post(baseurl + 'admin/inventaire/get_audit_trail', { item_id: itemId, start_date: start, end_date: end }, function(res) {
                if (res.status === 'success') {
                    ...
                    if (res.data.length === 0) {
                        html = '<tr><td colspan="6" class="text-center">Aucun historique trouvé</span></td>';
                    } else {
                        $.each(res.data, function(i, log) {
                            html += `<tr>
                                <td>${log.created_at}</span></td>
                                <td>${log.action}</span></td>
                                <td>${log.field_name || '-'}</span></td>
                                <td>${log.old_value !== null ? log.old_value : '-'}</span></td>
                                <td>${log.new_value !== null ? log.new_value : '-'}</span></td>
                    ...
                    $('#auditTable tbody').html('<tr><td colspan="6" class="text-center">Erreur de chargement</span></td>');
                }
            }, 'json').fail(function() {
                $('#auditTable tbody').html('<tr><td colspan="6" class="text-center">Erreur de connexion</span></td>');
            });
        }

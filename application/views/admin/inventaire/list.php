            ...
            <div class="modal-body">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" id="auditTable">
                        <thead>
                        <tr><th>Date</th><th>Action</th><th>Champ</th><th>Ancienne valeur</th><th>Nouvelle valeur</th><th>IP</th></tr>
                        </thead>
                        <tbody>
                        <tr><td colspan="6" class="text-center">Aucune donnée</span></td>
                        </tbody>
                    </table>
                </div>
            </div>
 ...
 <div class="modal fade" id="pdfOptionsModal" tabindex="-1">
     <div class="modal-dialog">
         <div class="modal-content">
             <div class="modal-header">
                 <button type="button" class="close" data-dismiss="modal">&times;</button>
                 <h4 class="modal-title"><i class="fa fa-file-pdf-o"></i> Options d'export PDF</h4>
             </div>
             <div class="modal-body">
                 <div class="form-group">
                     <label>Titre du rapport</label>
                     <input type="text" id="report_title" class="form-control" value="État de l'inventaire">
                 </div>
                 <div class="form-group">
                 ...
                 <div class="checkbox">
                     <label>
                         <input type="checkbox" id="show_zero_stock_pdf"> Inclure les articles en rupture (stock = 0)
                     </label>
                 </div>
             </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        // Édition inline du stock réel + synchronisation stock théorique
        function attachInlineEdit() {
            $('.' + dtID + ' tbody td:nth-child(6)').off('dblclick').on('dblclick', function(e) {
                ...
                var saveEdit = function() {
                    ...
                    $.ajax({
                        url: baseurl + 'admin/inventaire/update_inventory_sync',  // méthode qui met à jour stock réel ET stock théorique
                        type: 'POST',
                        data: { item_id: itemId, new_stock_reel: newValue },
                        dataType: 'json',
            ...
            $.post(baseurl + 'admin/inventaire/get_audit_trail', { item_id: itemId, start_date: start, end_date: end }, function(res) {
                if (res.status === 'success') {
                    ...
                    if (res.data.length === 0) {
                        html = '<tr><td colspan="6" class="text-center">Aucun historique trouvé</td></tr>';
                    } else {
                        $.each(res.data, function(i, log) {
                            // Traduction
                            var actionLabel = actionMap[log.action] || log.action;
                            var fieldLabel = fieldMap[log.field_name] || log.field_name || '-';
                    ...
                    $('#auditTable tbody').html('<tr><td colspan="6" class="text-center">Erreur de chargement</td></tr>');
                }
            }, 'json').fail(function() {
                $('#auditTable tbody').html('<tr><td colspan="6" class="text-center">Erreur de connexion</td></tr>');
            });
        }
    ...
    var actionMap = {
        'stock_reel_update': 'Mise à jour du stock réel',
        'stock_theorique_sync': 'Synchronisation du stock théorique',
        'stock_reel_sync': 'Synchronisation du stock réel',
        'stock_initial_update': 'Mise à jour du stock initial',
        // Ajoutez d'autres actions si nécessaire
    };

    // Traductions pour les noms de champs
    var fieldMap = {
        'real_quantity': 'Quantité réelle',
        'current_quantity': 'Quantité théorique',
        'initial_quantity': 'Quantité initiale',
        'out_quantity': 'Quantité sortie',

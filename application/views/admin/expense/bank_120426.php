...
<script>
    ...
    function exportAllBanksPDF() {
        ...
        <body>
            ...
            minute: '2-digit'
                 ...
                 <table>
                    <thead>
                        <tr><th>Nom de la Banque</th><th>Code</th><th>Numéro de Compte</th><th>Solde</th></tr>
                    </thead>
                    <tbody>
                        ${banksData.map(bank => `
                            <tr>
                ...
                <div class="total-section">
                    ...
                    <div style="margin-top: 8px;"><strong>Solde total :</strong> <span class="${totalBalance >= 0 ? 'balance-positive' : 'balance-negative'}">${formatNumber(totalBalance)} <?php echo $currency_symbol; ?></span></div>
                </div>
                <div class="footer">
    ...
    function exportAllBanksExcel() {
        ...
        var totalBalance = 0;
        $('.bank-card').each(function() { totalBalance += parseFloat($(this).data('bank-balance')); });

        csvContent += `\r\nRésumé,,,,\r\n"Total des banques","${$('.bank-card').length}",,,\r\n"Solde total","${totalBalance.toFixed(2)} <?php echo $currency_symbol; ?>",,,\r\n"Date d'export","${new Date().toLocaleDateString('fr-FR')}",,,\r\n`;
        ...
        <body>
            <div class="pdf-container">
                ...
                <div class="summary-section">
                    ...
                    <div class="summary-item"><div class="summary-label">Solde Net</div><div class="summary-value ${totalAmount >= 0 ? 'credit' : 'debit'}">${formatNumber(totalAmount)} <?php echo $currency_symbol; ?></div></div>
                </div>
    ...
    function printBankList() {
        ...
        var printContent = `<html><head><title>Liste des Banques</title><style>
            ...
            <table><thead><tr><th>Nom</th><th>Code</th><th>Compte</th><th>Solde</th></tr></thead>
            <tbody>${banksData.map(bank => `<tr><td>${bank.name}</td><td>${bank.code}</td><td>${bank.account}</td><td>${formatNumber(bank.balance)} <?php echo $currency_symbol; ?></td></tr>`).join('')}</tbody>
            </table>
            <div class="total"><p>Total banques: ${banksData.length}</p><p>Solde total: ${formatNumber(totalBalance)} <?php echo $currency_symbol; ?></p></div>
        </body></html>`;

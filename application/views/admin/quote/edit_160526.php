                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <table class="table table-bordered">
                                    ...
                                    <tr>
                                        <td>Total après remise articles:</td>
                                        <td class="text-right"><span id="total_after_discount_items"><?php echo number_format(isset($quote['total_after_discount']) ? $quote['total_after_discount'] : $quote['total_ht'], 2); ?></span></td>
                                        <input type="hidden" name="total_after_discount_items" id="totalAfterDiscountItems" value="<?php echo isset($quote['total_after_discount']) ? $quote['total_after_discount'] : $quote['total_ht']; ?>">
                                    </tr>
                                    <!-- Ligne pour la remise globale -->
                                    <tr class="global-discount-row">
                                        <td>
                                            <div><strong>Remise globale</strong></div>
                                            <div class="global-discount-inputs">
                                                <div class="global-discount-type-group">
                                                    <button type="button" class="global-discount-btn <?php echo (isset($quote['global_discount_type']) && $quote['global_discount_type'] == 'percent') || !isset($quote['global_discount_type']) ? 'active' : ''; ?>" data-type="percent">%</button>
                                                ...
                                                <input type="number" name="global_discount_value" class="form-control form-control-sm" id="global_discount_value" min="0" step="0.01" value="<?php echo isset($quote['global_discount_value']) ? $quote['global_discount_value'] : 0; ?>" placeholder="0.00" style="width: 100px;">
                                            </div>
                                        </td>
                                        <td class="text-right">
                                            <span id="global_discount_display">0.00</span>
                                            <input type="hidden" name="global_discount_amount" id="global_discount_amount" value="<?php echo isset($quote['global_discount_amount']) ? $quote['global_discount_amount'] : 0; ?>">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Total après remise globale:</td>
                                        <td class="text-right"><span id="total_after_global_discount"><?php echo number_format($quote['total_ht'], 2); ?></span></td>
                                        <input type="hidden" name="total_after_global_discount" id="totalAfterGlobalDiscount" value="<?php echo $quote['total_ht']; ?>">
                                    </tr>
...
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Fonction pour calculer la remise globale
        function calculateGlobalDiscount(totalAfterItemDiscounts) {
            const discountType = document.getElementById('global_discount_type').value;
            const discountValue = parseFloat(document.getElementById('global_discount_value').value) || 0;

            let globalDiscountAmount = 0;

            if (discountType === 'percent') {
                globalDiscountAmount = totalAfterItemDiscounts * (discountValue / 100);
            } else {
                globalDiscountAmount = Math.min(discountValue, totalAfterItemDiscounts); // Ne pas dépasser le total
            }

            // Mise à jour de l'affichage
            document.getElementById('global_discount_display').textContent = globalDiscountAmount.toFixed(2);
            document.getElementById('global_discount_amount').value = globalDiscountAmount.toFixed(2);
        ...
        function calculateTotals() {
            ...
            const globalDiscountAmount = calculateGlobalDiscount(totalAfterDiscount);
            const totalAfterGlobalDiscount = Math.max(totalAfterDiscount - globalDiscountAmount, 0);

            // Calcul de la TVA
            const applyTVA = document.getElementById('apply_tva').checked;
            const tvaRate = <?php echo $quote['tva_rate']; ?>;
            const tvaAmount = applyTVA ? totalAfterGlobalDiscount * (tvaRate / 100) : 0;
            const totalTTC = totalAfterGlobalDiscount + tvaAmount;
            ...
            document.getElementById('total_after_discount_items').textContent = totalAfterDiscount.toFixed(2);
            document.getElementById('totalAfterDiscountItems').value = totalAfterDiscount.toFixed(2);

            document.getElementById('total_after_global_discount').textContent = totalAfterGlobalDiscount.toFixed(2);
            document.getElementById('totalAfterGlobalDiscount').value = totalAfterGlobalDiscount.toFixed(2);
        ...
        // Fonction pour changer le type de remise globale
        function setupGlobalDiscountButtons() {
            document.querySelectorAll('.global-discount-btn').forEach(button => {
                button.addEventListener('click', function() {
                    const type = this.dataset.type;
                    const discountTypeInput = document.getElementById('global_discount_type');
        ...
        // Fonction pour changer le type de remise par article
        ...
        function setupEventListeners() {
            document.addEventListener('input', function(e) {
                if (e.target.classList.contains('quantity') ||
                    ...
                    e.target.id === 'global_discount_value') {
                    calculateTotals();
                }
            });

...
// Condition pour les salaires (basée sur payment_date)
$date_condition_sal = "1=1";
if ($date_debut && $date_fin) {
    $date_condition_sal = "payment_date BETWEEN '$date_debut' AND '$date_fin'";
} elseif ($date_debut) {
    $date_condition_sal = "payment_date >= '$date_debut'";
} elseif ($date_fin) {
    $date_condition_sal = "payment_date <= '$date_fin'";
}
...
// --- AJOUT : statistiques genre ---
$sql_hommes = "SELECT COUNT(*) as nb FROM staff WHERE is_active=1 AND deleted=1 AND name != 'Super Admin' AND gender = 'Male'";
$sql_femmes = "SELECT COUNT(*) as nb FROM staff WHERE is_active=1 AND deleted=1 AND name != 'Super Admin' AND gender = 'Female'";
$nb_hommes = $conn->query($sql_hommes)->fetch_object()->nb ?? 0;
$nb_femmes = $conn->query($sql_femmes)->fetch_object()->nb ?? 0;

// --- AJOUT : effectif par département ---
$sql_dept_effectif = "SELECT d.department_name, COUNT(s.id) as nb FROM staff s LEFT JOIN department d ON s.department = d.id WHERE s.is_active=1 AND s.deleted=1 AND s.name != 'Super Admin' GROUP BY d.department_name ORDER BY nb DESC";
$res_dept_effectif = $conn->query($sql_dept_effectif);
$dept_effectif_labels = [];
$dept_effectif_values = [];
while ($row = $res_dept_effectif->fetch_assoc()) {
    $dept_effectif_labels[] = $row['department_name'] ?? 'Non défini';
    $dept_effectif_values[] = (int)$row['nb'];
}

// --- AJOUT : turnover global (nombre de départs dans l'année en cours) ---
$annee_courante = date('Y');
$sql_turnover_global = "SELECT COUNT(*) as nb FROM staff WHERE YEAR(date_of_leaving) = $annee_courante AND date_of_leaving IS NOT NULL AND date_of_leaving != '0000-00-00' AND deleted=1";
$res_turnover_global = $conn->query($sql_turnover_global);
$turnover_global = $res_turnover_global->fetch_object()->nb ?? 0;

// --- AJOUT : total des absences (congés + absences + arrêts) sur l'année ---
$sql_total_conges = "SELECT COUNT(*) as nb FROM staff_leave_request WHERE YEAR(leave_from) = $annee_courante";
$sql_total_absences = "SELECT COUNT(*) as nb FROM enquiry WHERE YEAR(date) = $annee_courante AND (reference='Permission' OR source LIKE '%maladie%' OR source LIKE '%voyage%')";
$sql_total_arrets = "SELECT COUNT(*) as nb FROM enquiry WHERE YEAR(date) = $annee_courante AND (reference='demission' OR reference LIKE '%demission%')";
$total_conges = $conn->query($sql_total_conges)->fetch_object()->nb ?? 0;
$total_absences = $conn->query($sql_total_absences)->fetch_object()->nb ?? 0;
// On ajoute aussi les absences de staff_attendance (type 4)
$sql_att_abs_total = "SELECT COUNT(*) as nb FROM staff_attendance WHERE YEAR(date) = $annee_courante AND staff_attendance_type_id = 4 AND is_active = 0";
$total_absences += $conn->query($sql_att_abs_total)->fetch_object()->nb ?? 0;
$total_arrets = $conn->query($sql_total_arrets)->fetch_object()->nb ?? 0;
$total_absences_global = $total_conges + $total_absences + $total_arrets;

// Âge médian et ancienneté
$ages = []; $anciennetes = [];
$employes->data_seek(0);
...
// Turnover : sorties par mois (inchangé)
...
// ================================================================
// MASSE SALARIALE NETTE ET SALAIRE MOYEN (selon la période filtrée)
// ================================================================
$sql_masse = "SELECT SUM(gross_salary) as total, COUNT(*) as nb FROM staff_payslip WHERE status='paid' AND $date_condition_sal";
$res_masse = $conn->query($sql_masse);
$row_masse = $res_masse->fetch_assoc();
$masse_salariale = $row_masse['total'] ?? 0;
$nb_salaires = $row_masse['nb'] ?? 0;
$salaire_net_moyen = $nb_salaires ? round($masse_salariale / $nb_salaires) : 0;

// Salaire moyen par département (inchangé, mais on pourrait aussi filtrer, on le laisse sur dernier bulletin)
...
// Évolution salaire net moyen (12 mois) inchangé
    ...
    <section class="content">
        ...
        <div class="row mt-4">
            <?php if ($this->rbac->hasPrivilege('staff_present_today_widegts', 'can_view')) : ?>
                ...
                <div class="col-lg-3"><div class="stat-card"><div class="stat-icon" style="background:#E8F5E9;"><i class="fa fa-money icon-green"></i></div><div class="stat-number"><?php echo number_format($salaire_net_moyen,0,',',' '); ?> FCFA</div><div class="stat-text">Salaire net moyen (période)</div></div></div>
            <?php endif; ?>
        </div><br>

        <!-- ============ AJOUT : NOUVELLES CARTES STATISTIQUES ============ -->
        <div class="row mt-4">
            <?php if ($this->rbac->hasPrivilege('staff_present_today_widegts', 'can_view')) : ?>
                <div class="col-lg-3">
                    <div class="stat-card">
                        <div class="stat-icon" style="background:#E3F2FD;"><i class="fa fa-mars icon-blue"></i></div>
                        <div class="stat-number"><?php echo $nb_hommes; ?></div>
                        <div class="stat-text">Hommes</div>
                    </div>
                </div>
                <div class="col-lg-3">
                    <div class="stat-card">
                        <div class="stat-icon" style="background:#FCE4EC;"><i class="fa fa-venus icon-red"></i></div>
                        <div class="stat-number"><?php echo $nb_femmes; ?></div>
                        <div class="stat-text">Femmes</div>
                    </div>
                </div>
                <div class="col-lg-3">
                    <div class="stat-card">
                        <div class="stat-icon" style="background:#FFF3E0;"><i class="fa fa-sign-out icon-orange"></i></div>
                        <div class="stat-number"><?php echo $turnover_global; ?></div>
                        <div class="stat-text">Départs (<?php echo $annee_courante; ?>)</div>
                    </div>
                </div>
                <div class="col-lg-3">
                    <div class="stat-card">
                        <div class="stat-icon" style="background:#F3E5F5;"><i class="fa fa-credit-card icon-purple"></i></div>
                        <div class="stat-number"><?php echo number_format($masse_salariale,0,',',' '); ?> FCFA</div>
                        <div class="stat-text">Masse salariale nette (période)</div>
                    </div>
                </div>
            <?php endif; ?>
        </div><br>

        <!-- ============ AJOUT : GRAPHIQUES GENRE ET DÉPARTEMENT ============ -->
        <div class="row mt-4">
            <?php if ($this->rbac->hasPrivilege('staff_present_today_widegts', 'can_view')) : ?>
                <div class="col-lg-6">
                    <div class="dashboard-card">
                        <div class="card-header"><h3><i class="fa fa-venus-mars icon-red"></i> Répartition Hommes / Femmes</h3></div>
                        <div class="chart-container"><canvas id="genderChart"></canvas></div>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="dashboard-card">
                        <div class="card-header"><h3><i class="fa fa-building icon-blue"></i> Effectif par département</h3></div>
                        <div class="chart-container"><canvas id="deptEffectifChart"></canvas></div>
                    </div>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="row" style="margin-bottom: 60px; clear: both;">
            <?php if ($this->rbac->hasPrivilege('staff_present_today_widegts', 'can_view')) : ?>
                <div class="col-md-12">
                    <div class="dashboard-card">
                        <div class="card-header"><h3><i class="fa fa-building icon-teal"></i> Salaire net moyen par département (dernier bulletin)</h3></div>
...
<script>
    ...
    var genderLabels = ['Hommes', 'Femmes'];
    var genderValues = [<?php echo $nb_hommes; ?>, <?php echo $nb_femmes; ?>];
    var deptEffectifLabels = <?php echo json_encode($dept_effectif_labels); ?>;
    var deptEffectifValues = <?php echo json_encode($dept_effectif_values); ?>;
    ...
    // ============ AJOUT : graphiques genre et département ============
    // Camembert genre
    if (genderValues.reduce((a,b)=>a+b,0) > 0) {
        new Chart(document.getElementById('genderChart'), {
            type: 'pie',
            data: {
                labels: genderLabels,
                datasets: [{ data: genderValues, backgroundColor: ['#3B82F6', '#EC4899'] }]
            },
            options: { responsive: true }
        });
    } else {
        document.getElementById('genderChart').parentNode.innerHTML = '<div class="alert alert-info">Aucune donnée de genre disponible</div>';
    }

    // Barres effectif par département
    if (deptEffectifLabels.length > 0) {
        new Chart(document.getElementById('deptEffectifChart'), {
            type: 'bar',
            data: {
                labels: deptEffectifLabels,
                datasets: [{ label: 'Effectif', data: deptEffectifValues, backgroundColor: '#0EA5E9' }]
            },
            options: { responsive: true, scales: { yAxes: [{ ticks: { beginAtZero: true, stepSize: 1 } }] } }
        });
    } else {
        document.getElementById('deptEffectifChart').parentNode.innerHTML = '<div class="alert alert-info">Aucun département renseigné</div>';
    }

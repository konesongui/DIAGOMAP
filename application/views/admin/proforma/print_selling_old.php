...
// 🔹 NOUVEAU : Récupération des informations de paiement
$amount_paid = (!empty($quote['amount_paid']) && floatval($quote['amount_paid']) > 0) ? floatval($quote['amount_paid']) : 0;
$change_amount = (!empty($quote['change_amount']) && floatval($quote['change_amount']) > 0) ? floatval($quote['change_amount']) : 0;
$remaining_amount = (!empty($quote['remaining_amount']) && floatval($quote['remaining_amount']) > 0) ? floatval($quote['remaining_amount']) : 0;
$payment_status = $quote['payment_status'] ?? 'pending';
$payment_method_type = $quote['payment_method_type'] ?? '';

// Déterminer le libellé du statut de paiement
...
$lines_first_page = 50; // Au moins 50 lignes sur la première page
$lines_other_pages = 60; // Plus de lignes pour les pages suivantes
...
$force_totals_first_page = ($total_pages == 1 || $first_page_items_count < 55);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Style spécifique pour le montant rendu */
        .payment-received {
    ...
    <div class="page-content">
        ...
        <div class="page-break tight-spacing">
            ...
            <div class="last-page-section">
                ...
                <!-- 🔹 NOUVEAU : Informations de paiement avec montant reçu et montant rendu -->
                <div class="payment-info">
                    <div class="payment-grid">
                        ...
                        <div class="payment-received">
                            <strong>💰 Montant reçu :</strong> <?= number_format($amount_paid, 2, ',', ' ') ?> FCFA
                        </div>
                        <?php if ($change_amount > 0): ?>
                            <div class="payment-change">
                                <strong>🔄 Montant rendu :</strong> <?= number_format($change_amount, 2, ',', ' ') ?> FCFA
                            </div>
                        <?php endif; ?>
                        <?php if ($remaining_amount > 0): ?>
                            <div class="payment-change">
                                <strong>⚠️ Reste à payer :</strong> <?= number_format($remaining_amount, 2, ',', ' ') ?> FCFA
                            </div>
                        <?php endif; ?>
                        <div class="payment-status">
                            <strong>📊 Statut :</strong> <?= $payment_status_label ?>
                        </div>
                        ...
                        <?php if ($tax_option === 'other' && !empty($quote['other_tax_name'])): ?>
                            <div><strong>🏷️ Taxe appliquée :</strong> <?= $quote['other_tax_name'] ?> (<?= number_format($quote['other_tax_rate'], 2, ',', ' ') ?>%)</div>
                        <?php endif; ?>
                        <?php if (!empty($payment_method_type)): ?>
                            <div><strong>💳 Type de paiement :</strong> <?= $payment_method_type == 'cash' ? 'Espèces (Caisse)' : 'Paiement bancaire' ?></div>
                        <?php endif; ?>
                    </div>
                </div>
                <!-- FIN NOUVEAU -->

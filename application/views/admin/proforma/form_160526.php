                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select class="form-control" name="customer" id="customer_select">
                                        ...
                                        <?php endforeach; ?>
                                        <option value="new">â Nouveau client</option>
                                    </select>
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <input id="quote_date" name="quote_date" type="text" class="form-control date" value="<?= set_value('quote_date', date('d/m/Y')) ?>"/>
                                </div>
                                <div class="form-group col-md-4">
                                    <label>Date limite</label>
                                    <input id="valid_until" name="valid_until" type="text" class="form-control date" value="<?= set_value('valid_until', date('d/m/Y')) ?>"/>
                                </div>
                            </div>
                            <div class="row">
                                        ...
                                        <div class="repeater-item">
                                            <div class="row">
                                                ...
                                                <div class="form-group col-md-1">
                                                    ...
                                                    <input type="number" name="price[]" class="form-control price" min="0" step="0" value="0">
                                                </div>
                                                ...
                                                <div class="form-group col-md-2">
                                                    ...
                                                    <div class="discount-input-group">
                                                        ...
                                                        <input type="number" name="discount[]" class="form-control discount discount-field" min="0" step="0" value="0" placeholder="0.00" style="border-radius: 0 4px 4px 0;">
                                                    </div>
                                                </div>
                                                <div class="form-group col-md-1">
                                                    <label>P.U NET</label>
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                    ...
                                    <div class="form-group tax-options">
                                        ...
                                        <div class="radio">
                                            <label>
                                                <input type="radio" name="tax_option" value="none" checked>
                                                Aucune taxe
                                            </label>
                                        </div>
                                        <div class="radio">
                                            <label>
                                                <input type="radio" name="tax_option" value="tva">
                                                Appliquer la TVA (18%)
                                            </label>
                                        </div>
                                        <div class="radio">
                                ...
                                <div class="col-md-4">
                                    <table class="table table-bordered">
                                        ...
                                        <tr class="other-tax-row" style="display:none;">
                                            ...
                                            <td class="text-right"><span id="other_tax_amount">0.00</span></td>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('input[name="tax_option"]').forEach(radio => {
            radio.addEventListener('change', function() {
                ...
                // Afficher/masquer le conteneur de taxe personnalisée
                ...
                // Afficher/masquer les lignes de taxe
        ...
        // ð¹ Fonction pour calculer le total d'un article
        ...
        function calculateItemTotal(itemElement) {
            ...
            const unitPriceNet = price - discountAmount;

            // Total après remise
            const totalAfterDiscount = unitPriceNet * quantity;

            // Mise à jour des affichages
            $item.find('.total-price').text(unitPriceNet.toFixed(2));
            $item.find('.total-price-after-discount').text(totalAfterDiscount.toFixed(2));
            $item.find('.line-total-after-discount').val(totalAfterDiscount.toFixed(2));
        ...
        // ð¹ Fonction principale pour calculer tous les totaux
        function calculateTotals() {
            ...
            if (taxOption === 'tva') {
                taxRate = 0.18;
                taxAmount = finalTotalAfterDiscount * taxRate;
                taxName = 'TVA (18%)';
            ...
            // ð¸ Calcul du total TTC
            const totalTTC = finalTotalAfterDiscount + taxAmount;

            // ð¸ Mise à jour affichage
        ...
        // Écouteurs pour les radios de taxe
        document.querySelectorAll('input[name="tax_option"]').forEach(radio => {
            radio.addEventListener('change', calculateTotals);
        });

        // ð¹ Ajouter/Supprimer un article
        document.getElementById('add-item').addEventListener('click', function() {
            ...
            newItem.querySelector('.quantity').value = '0';
        ...
        // ð¹ Soumission du formulaire avec confirmation
        document.getElementById('<?= $formID; ?>').addEventListener('submit', function(e) {
            e.preventDefault();

            // Validation de la taxe personnalisée
            const taxOption = document.querySelector('input[name="tax_option"]:checked').value;
            if (taxOption === 'other') {
...
<script type="text/javascript">
        ...
        if (categoryName) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (!$datalist.length) {
                        // Créer un nouveau datalist si nécessaire
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    // Remplir le prix unitaire
                    $row.find('.price').val(response.price || 0);

                    // Afficher la disponibilité du stock
                    ...
                    // Nouvel article - réinitialiser les champs
                    $row.find('.unit').val('');
                    $row.find('.price').val('');
    ...
    $(document).on('change', '.item-category', function() {
        ...
        if (categoryName) {
            // Générer un ID unique pour le datalist
    ...
    $(document).on('click', '#add-item', function() {
        // Attendre que le nouvel élément soit ajouté
        setTimeout(function() {

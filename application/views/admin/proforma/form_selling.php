...
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Fonction pour calculer le total d'un article (avec TVA incluse dans le prix)
        function calculateItemTotal(itemElement) {
            const $item = $(itemElement);
            const quantity = parseFloat($item.find('.quantity').val()) || 0;
            const priceTTC = parseFloat($item.find('.price').val()) || 0; // Prix TTC saisi
            ...
            // Calcul du prix HT à partir du prix TTC (Prix TTC / (1 + taux taxe))
        ...
        // Fonction principale pour calculer tous les totaux (CORRIGÉE)
        // Fonction principale pour calculer tous les totaux (CORRIGÉE)
        function calculateTotals() {
            ...
            // Appliquer la remise globale sur le total HT après remises ligne
            ...
            // Calcul de la TVA sur le HT final
            const taxOption = document.querySelector('input[name="tax_option"]:checked').value;
            let taxAmount = 0;
            let taxName = '';
            let taxRate = 0;
            ...
            // TTC final = HT final + taxes
            const totalTTCFinal = finalTotalHT + taxAmount;

            // ARRONDIR LE TOTAL TTC FINAL pour éviter les erreurs de précision
        ...
        // Fonction pour mettre à jour le module de paiement
        // Fonction pour mettre à jour le module de paiement (CORRIGÉE)
        function updatePaymentModule() {
            ...
            // Calculer le reste
            let remaining = totalTTC - paid;
            remaining = Math.round(remaining * 100) / 100;

            console.log('Paid:', paid, 'Total TTC:', totalTTC, 'Remaining:', remaining);

            // Si le reste est négatif ou très proche de 0 (à cause des erreurs de virgule flottante)
            if (remaining <= 0) {
                // Montant rendu
                ...
                // FORCER le champ amount_paid à la valeur exacte du total TTC
                if (paid !== totalTTC) {
...
<script type="text/javascript">
    $(document).ready(function() {
        // Gestion de l'affichage de la sélection de source selon le mode de paiement
        $('#payment_method_type').change(function() {
            var method = $(this).val();

            if (method === 'cash') {
                // Paiement en espèces -> Caisse
                $('#caisse_group').show();
                $('#banque_group').hide();
        ...
        // Mettre à jour les informations de la source sélectionnée
        ...
        // Modification de la fonction updatePaymentModule existante
        var originalUpdatePaymentModule = updatePaymentModule;
        window.updatePaymentModule = function() {
            originalUpdatePaymentModule();

            // Si le montant payé > 0, vérifier qu'une source est sélectionnée
        ...
        // Écouteur pour le montant payé (amélioré)

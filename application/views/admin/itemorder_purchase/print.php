...
$customerFullname = $order['customer_name'].' '.$order['customer_last_name']??"N/A";
$customerPhone = $order['customer_phone']??"N/A";
$customerAddress = $order['customer_address'].' / '.$order['customer_email']??"N/A";
$customerComptec = $order['comptec']??"N/A";
$quoteDate = !empty($order['quote_date'])? date('d/m/Y', strtotime($order['quote_date'])) :"N/A";
$quoteDesignation = $order['designation']??"N/A";
$quoteNumber = $order['quote_number']??"N/A";

$items = $order['items']? $order['items']:[];

$tva_amount = (!empty($order['tva_amount']) && floatval($order['tva_amount']) > 0)? floatval($order['tva_amount']) :"Non facturée";
$tva_rate = (!empty($order['tva_rate']) && floatval($order['tva_rate']) > 0)? floatval($order['tva_rate']) :0;
$total_ht = (!empty($order['total_ht']) && floatval($order['total_ht']) > 0)? floatval($order['total_ht']) :0;
$total_ttc = (!empty($order['total_ttc']) && floatval($order['total_ttc']) > 0)? floatval($order['total_ttc']) :0;
...
<div class="main-content">
    <div class="head">Devis N° <?= $orderNumber ?> du <?= $orderDate ?></div>
        ...
        <table class="info-table">
            ...
            <tr>
                <td colspan="6" style="padding: 10px;">
                    <strong>Objet :</strong> <?= $orderDesignation ;?>
                </td>
            </tr>
        </table><br>

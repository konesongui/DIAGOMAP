            ...
            <div class="col-md-4">
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        // 4. Méthode robuste pour récupérer le total des réapprovisionnements
                        $date_debut_reappro = !empty($date_debut) ? $this->db->escape($date_debut) : $this->db->escape(date('Y-m-01'));
                        $date_fin_reappro = !empty($date_fin) ? $this->db->escape($date_fin) : $this->db->escape(date('Y-m-d'));

                        $sql = "SELECT SUM(montant) as total_reappro 
                                FROM operation_caisse 
                                WHERE DATE(date) >= {$date_debut_reappro} 
                                AND DATE(date) <= {$date_fin_reappro} 
                                AND (reference LIKE 'TRF-%' OR reference LIKE 'REAPP-%' OR reference LIKE 'REAP-%' OR designation LIKE '%réappro%' OR designation LIKE '%reappro%') 
                                AND type_operation = 'ENTREE' 
                                AND deleted = 'no'";

                        $query = $this->db->query($sql);
                        $total_reappro_result = $query->row();
                        ...
                        <?php if (!empty($caisses)): ?>
                            <div class="total-centralisation">
                                            ...
                                            <form method="get" action="<?php echo base_url('admin/income') ?>" class="form-inline" id="formTotaux">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="date" name="date_totaux_debut" id="date_totaux_debut" class="form-control input-sm"
                                                           value="<?php echo $date_totaux_debut; ?>" style="margin-right: 10px;">
                                                </div>

                                                <div class="form-group">

                                                    <input type="date" name="date_totaux_fin" id="date_totaux_fin" class="form-control input-sm"
                                                           value="<?php echo $date_totaux_fin; ?>" style="margin-right: -2px;">
                                                </div>
                                ...
                                <?php if (!empty($totaux_par_mode_paiement)): ?>
                                    <div class="row" style="margin-top: 20px;">
                                        <div class="col-md-12">
                                            <div style="background: white; padding: 15px; border-radius: 5px; margin-top: 15px;">
                                                <!--<h5 style="color: #2e59d9; border-bottom: 1px solid #eee; padding-bottom: 10px; margin-bottom: 15px;">
                                                    <i class="fa fa-credit-card"></i> TOTAUX PAR MODE DE PAIEMENT
                                                </h5>-->

                                          <!--      <div class="table-responsive">
                                                    <table class="table table-bordered table-striped table-hover table-modes-paiement" style="font-size: 13px;">
                                                        <thead style="background-color: #f8f9fa;">
                                                        <tr>
                                                            <th style="width: 20%;">
                                                                <i class="fa fa-credit-card"></i> Mode de Paiement
                                                            </th>
                                                            <th style="width: 20%; text-align: right; color: #28a745;">
                                                                <i class="fa fa-sign-in"></i> Total Entrées
                                                            </th>
                                                            <th style="width: 20%; text-align: right; color: #dc3545;">
                                                                <i class="fa fa-sign-out"></i> Total Sorties
                                                            </th>
                                                            <th style="width: 20%; text-align: right;">
                                                                <i class="fa fa-calculator"></i> Solde Net
                                                            </th>
                                                            <th style="width: 10%; text-align: center;">
                                                                <i class="fa fa-list-ol"></i> Nb. Opérations
                                                            </th>
                                                            <th style="width: 10%; text-align: center;">
                                                        ...
                                                        <tbody>
                                                        <?php
                                          $total_entrees_modes = 0;
                                          $total_sorties_modes = 0;
                                          $total_operations_modes = 0;
                                          ?>

                                                        <?php foreach ($totaux_par_mode_paiement as $mode => $totaux): ?>
                                                            <?php
                                          $total_entrees_modes += $totaux['total_entrees'];
                                          $total_sorties_modes += $totaux['total_sorties'];
                                          $total_operations_modes += $totaux['nombre_operations'];
                                          ?>
                                                            <tr>
                                                                <td style="font-weight: bold;">
                                                                    <i class="fa fa-credit-card"></i> <?php echo htmlspecialchars($mode); ?>
                                                                </td>
                                                                ...
                                                                <td style="text-align: right; font-weight: bold; color: <?php echo $totaux['solde_net'] >= 0 ? '#28a745' : '#dc3545'; ?>;">
                                                                    <?php echo number_format($totaux['solde_net'], 0, ',', ' '); ?> FCFA
                                                                </td>
                                                                <td style="text-align: center;">
                                                                        <span class="badge badge-info">
                                                                            <?php echo $totaux['nombre_operations']; ?>
                                                                        </span>
                                                                </td>
                                                                <td style="text-align: center;">
                                                                    <button type="button" class="btn btn-xs btn-outline-primary"
                                                                            onclick="voirOperationsParMode('<?php echo htmlspecialchars($mode); ?>')"
                                                                            title="Voir les détails">
                                                                        <i class="fa fa-eye"></i>
                                                                    </button>
                                                                    <button type="button" class="btn btn-xs btn-outline-success"
                                                                            onclick="exporterModePaiement('<?php echo htmlspecialchars($mode); ?>')"
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="text-align: right; color: #28a745;">
                                                        <strong><?php echo number_format($total_entrees_modes, 0, ',', ' '); ?> FCFA</strong>
                                                    </td>
                                                    <td style="text-align: right; color: #dc3545;">
                                                        <strong><?php echo number_format($total_sorties_modes, 0, ',', ' '); ?> FCFA</strong>
                                                    </td>
                                                    <td style="text-align: right; color: <?php echo ($total_entrees_modes - $total_sorties_modes) >= 0 ? '#28a745' : '#dc3545'; ?>;">
                                                        <strong><?php echo number_format($total_entrees_modes - $total_sorties_modes, 0, ',', ' '); ?> FCFA</strong>
                                                    </td>
                                                    <td style="text-align: center;">
                                                                    <span class="badge badge-primary">
                                                                        <?php echo $total_operations_modes; ?>
                                                                    </span>
                                                    </td>
                                                    <td style="text-align: center;">
                                                        <button type="button" class="btn btn-xs btn-primary"
                                                                onclick="exporterTousModesPaiement()"
                                                                title="Exporter tous les modes">
                                                            <i class="fa fa-download"></i> Tout exporter
                                                        </button>
                                                    </td>
                                                </tr>
                                                </tfoot>
                                    ...
                                    <div class="row" style="margin-top: 20px;">
                                        <div class="col-md-12">
                                            <div class="alert alert-warning">
                                                <i class="fa fa-info-circle"></i> Aucune opération trouvée avec mode de paiement spécifié pour cette période.
                                            </div>
                                        </div>
                                    </div>
                                <?php endif; ?>
                                            ...
                                            <div class="btn-group btn-group-xs">
                                                ...
                                                <?php if ($this->rbac->hasPrivilege('caisse', 'can_delete')): ?>
                                                    <a href="javascript:void(0);"
                                                       onclick="confirmDeleteCaisse(<?php echo $caisse['id']; ?>, '<?php echo htmlspecialchars($caisse['name'], ENT_QUOTES); ?>')"
            ...
            <div class="col-md-8">
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <div class="filter-form">
                            <form method="get" action="<?php echo base_url('admin/income') ?>" class="form-inline">
                                <!-- Champs cachés pour conserver les dates des totaux -->
                    ...
                    <div class="box-body">
                        ...
                        <!-- NOUVEAU MESSAGE POUR LE FILTRE MODE DE PAIEMENT -->
                        ...
                        <div class="table-responsive mailbox-messages">
                            <table class="table table-striped table-bordered table-hover table-livre-caisse" id="livre-caisse-table">
                                ...
                                <tr>
                                    ...
                                    <th width="8%" class="no-print">ACTIONS
                                        <small style="font-size:9px; color:#666; display:block;">
                                            <i class="fa fa-info-circle"></i> Suppression: jour même uniquement
                                        </small>
                                    </th>
                                </tr>
                                </thead>
                                ...
                                if (!empty($operations)) {
                                    foreach ($operations as $operation) {
                                        ...
                                        // Vérifier si c'est une opération du jour pour la suppression
                                        $date_operation = !empty($operation['date']) ? date('Y-m-d', strtotime($operation['date'])) : '';
                                        $peut_supprimer = ($date_operation == $date_actuelle);
                                        ?>
                                        <tr class="<?php echo $is_reappro ? 'reappro-row' : ''; ?>"
                                            ...
                                            data-operation-id="<?php echo $operation['id']; ?>"
                                            data-date-operation="<?php echo $date_operation; ?>">
                                            <td>
                                            ...
                                            <td class="no-print text-center">
                                                <div class="btn-group btn-group-xs" role="group">
                                                    ...
                                                    <?php if ($this->rbac->hasPrivilege('caisse', 'can_edit')): ?>
                                                        <!-- Bouton Éditer (seulement pour aujourd'hui et hier) -->
                                                        <?php
                                                        $hier = date('Y-m-d', strtotime('-1 day'));
                                                        $peut_editer = ($date_operation == $date_actuelle || $date_operation == $hier);
                                                        ?>
                                                    ...
                                                    <?php if ($this->rbac->hasPrivilege('caisse', 'can_delete')): ?>
                                                        ...
                                                        // Les super admins et admins peuvent toujours supprimer, même pour les dates passées
                                                        $peut_supprimer = $is_admin_user ? true : $peut_supprimer;

                                                        // Message pour le tooltip en fonction du rôle
                                                        $title_message = '';
                                                        if (!$peut_supprimer) {
                                                            $title_message = "Opération du " . date('d/m/Y', strtotime($operation['date'])) . " - Non supprimable (date passée)";
                                                        } else if ($is_admin_user && $operation['date'] < date('Y-m-d')) {
                                                            $title_message = "Supprimer cette opération (Date passée - Autorisation Admin)";
                                                        } else {
                                                            $title_message = "Supprimer cette opération";
                                                        }
                                                        ?>

                                                        <!-- Bouton Supprimer - Les admins peuvent supprimer même pour les dates passées -->
                                                        <?php if ($peut_supprimer): ?>
                                                            <button class="btn btn-xs btn-danger delete-operation-btn"
                                                                    ...
                                                                    onclick="verifierSuppression(<?php echo $operation['id']; ?>, '<?php echo !empty($operation['date']) ? $operation['date'] : ''; ?>', <?php echo $is_admin_user ? 'true' : 'false'; ?>)">
                                                                <i class="fa fa-trash"></i>
                                                                <?php if ($is_admin_user && $operation['date'] < date('Y-m-d')): ?>
                                                                    <span class="admin-badge" style="font-size: 8px; margin-left: 2px;">⚡</span>
                                                                <?php endif; ?>
                                                            </button>
                                                        <?php else: ?>
                                                            <!-- Bouton verrouillé avec indication de date -->
                                                            <button class="btn btn-xs btn-secondary operation-locked-btn"
                                                                    title="<?php echo $title_message; ?>"
                                                                    disabled
                                                                    style="cursor: default; background-color: #6c757d; border-color: #6c757d;">
...
<script>
    ...
    function initSearchFilters() {
        ...
        // Filtrage par mode de paiement en temps réel
        $('#mode_paiement').on('change', function() {
            var selectedMode = $(this).val().toLowerCase();
            filterByPaymentMode(selectedMode);
        });
        ...
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: base_url + 'admin/income/delete_operation_ajax/' + operationId,
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            [csrfToken]: csrfHash
    ...
    // Fonction pour vérifier la suppression avec confirmation basée sur la date
    function verifierSuppression(operationId, dateOperation) {
        var dateActuelle = new Date();
        var dateOp = new Date(dateOperation);

        // Formater les dates
        var dateActuelleStr = dateActuelle.toLocaleDateString('fr-FR');
        var dateOpStr = dateOp.toLocaleDateString('fr-FR');

        // Vérifier si c'est la même date
        if (dateActuelleStr !== dateOpStr) {
            // Vérifier si SweetAlert2 est disponible
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    ...
                    html: `
                        <div style="text-align:center; padding:20px;">
                            <i class="fa fa-calendar-times" style="font-size:48px;color:#dc3545;margin-bottom:20px;"></i>
                            <p style="font-size:18px;margin-bottom:10px;">Cette opération date du <strong>${dateOpStr}</strong></p>
                            <p style="color:#666;margin-bottom:20px;">
                                La suppression n'est autorisée que pour les opérations du jour (${dateActuelleStr})
                            </p>
                        </div>
                    `,
                    icon: 'error',
                ...
                alert('Suppression non autorisée !\n\nCette opération date du ' + dateOpStr +
                    '\nLa suppression n\'est autorisée que pour les opérations du jour (' + dateActuelleStr + ')');
            }
            return false;
        }

        // Si c'est la date du jour, procéder à la suppression
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.delete-operation-btn', function(e) {
            ...
            if (operationId) {
                var dateOperation = $(this).closest('tr').data('date-operation');
                verifierSuppression(operationId, dateOperation);
            } else {
    ...
    // =============================================
    // NOUVELLES FONCTIONS POUR LES TOTAUX ET MODES DE PAIEMENT
    // =============================================
    ...
    function resetTotauxDates() {
        ...
        // Mettre aussi à jour les champs cachés dans le formulaire principal
    ...
    function confirmDeleteCaisse(id, name) {
        // Vérifier si SweetAlert2 est disponible
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                ...
                <div style="text-align: center;">
                    ...
                    <p style="font-size: 16px; margin-bottom: 10px;">
                        Êtes-vous sûr de vouloir supprimer la caisse <strong>"${name}"</strong> ?
                    </p>
            ...
            `,
                ...
                if (result.isConfirmed && result.value && result.value.success) {
                    ...
                    Swal.fire({
                        title: 'Erreur !',
                        text: result.value.message || 'Une erreur est survenue lors de la suppression.',
                        icon: 'error',
                        confirmButtonColor: '#dc3545'
                    });
                }
            ...
            if (confirm('Êtes-vous sûr de vouloir supprimer la caisse "' + name + '" ?')) {
                deleteCaisseFallback(id);
            }
        }
    }

    // Fonction AJAX pour supprimer la caisse
    function deleteCaisse(id) {
        return fetch('<?php echo base_url('admin/income/delete'); ?>/' + id, {
            method: 'POST',
            headers: {
    ...
    // Fallback sans AJAX
    function deleteCaisseFallback(id) {
        window.location.href = '<?php echo base_url('admin/income/delete/'); ?>/' + id;
    }

</script>
<script type="text/javascript">
    function verifierSuppression(operationId, dateOperation, isAdminUser = false) {
        var dateOperationObj = new Date(dateOperation);
        var aujourdHui = new Date();
        aujourdHui.setHours(0, 0, 0, 0);

        // Si l'utilisateur est admin, il peut toujours supprimer
        if (isAdminUser) {
            if (dateOperationObj < aujourdHui) {
                // Message spécial pour les admins supprimant une date passée
                var message = "⚠️ ATTENTION ADMIN ⚠️\n\n";
                message += "Vous êtes sur le point de supprimer une opération du " + formatDate(dateOperationObj) + " (date passée).\n";
                ...
                // Date normale ou future
                var message = "Voulez-vous vraiment supprimer cette opération du " + formatDate(dateOperationObj) + " ?";
                if (confirm(message)) {
                    supprimerOperation(operationId);
                }
            }
        } else {
            // Utilisateur normal - vérification standard
            if (dateOperationObj < aujourdHui) {
                alert("❌ Impossible de supprimer cette opération car elle est datée du " + formatDate(dateOperationObj) + " (date passée).\n\nCette restriction ne s'applique pas aux administrateurs.");
                return false;
            } else {
                var message = "Voulez-vous vraiment supprimer cette opération du " + formatDate(dateOperationObj) + " ?";
                if (confirm(message)) {
    ...
    function supprimerOperation(operationId) {
        // Votre logique de suppression ici
        $.ajax({
            url: '<?php echo site_url("admin/income/delete_operation"); ?>',
            type: 'POST',
            data: {
                id: operationId,
        <?php echo $this->security->get_csrf_token_name(); ?>: '<?php echo $this->security->get_csrf_hash(); ?>'

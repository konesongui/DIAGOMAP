...
// ===============================
// 9bis. DÉPENSES PAR CATÉGORIE (Transport, Taxe, Impôt) via exp_head_id
// ===============================
function getMontantParCategorie($conn, $date_debut, $date_fin, $categorie_nom) {
    $sql = "SELECT COALESCE(SUM(oc.montant), 0) AS total
            FROM operation_caisse oc
            INNER JOIN expense_head eh ON oc.exp_head_id = eh.id
            WHERE oc.type_operation = 'SORTIE'
              AND oc.date BETWEEN '$date_debut' AND '$date_fin'
              AND (oc.deleted = 0 OR oc.deleted IS NULL)
              AND (oc.reference NOT LIKE 'TRF%' OR oc.reference IS NULL)
              AND eh.exp_category = '$categorie_nom'";
    $res = $conn->query($sql);
    $montant = ($res && $row = $res->fetch_assoc()) ? (float)$row['total'] : 0;
    return $montant;
}

$total_transport = getMontantParCategorie($conn, $date_debut, $date_fin, 'Transport');
$total_taxe      = getMontantParCategorie($conn, $date_debut, $date_fin, 'Taxe');
$total_impot     = getMontantParCategorie($conn, $date_debut, $date_fin, 'Impôt');

// ===============================
// 10. TOTAUX GLOBAUX (suite)
// ===============================
...
// ===============================
// 11. MONTANT NET (CA Réalisé - Dépenses totales)
// ===============================
$montant_net = $ca_realise - $total_depenses_global;

// ===============================
// 12. RÉSULTAT NET ENCAISSÉ (CA Encaissé - Déjà payé/décaissé)
// ===============================
$resultat_net_encaisse = $ca_encaisse - $total_deja_paye;

// ===============================
// 13. FUSION DES SOURCES POUR LA COURBE MENSUELLE DES DÉPENSES
// ===============================
...
// ===============================
// 14. LISTE FOURNISSEURS POUR MODALE (top 20)
// ===============================
$stmt = $conn->prepare("
    SELECT 
...
<head>
    ...
    <style>
        ...
        .icon-transport { background: #f1c40f; }
        .icon-taxe { background: #3498db; }
        .icon-impot { background: #e67e22; }
...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <!-- KPIs DÉPENSES GLOBALES (première ligne : totaux) -->
        <div class="row">
        ...
        <!-- NOUVELLE LIGNE : Dépenses par catégorie (Transport, Taxe, Impôt) -->
        <div class="row">
            <div class="col-md-12"><h4 class="text-info"><i class="fa fa-pie-chart"></i> Ventilation par catégorie (sorties de caisse)</h4></div>
            <div class="col-md-4 col-sm-6">
                <div class="small-box" style="border-left: 5px solid #f1c40f;">
                    <div class="inner">
                        <h3><?= number_format($total_transport, 0, ",", " ") ?> FCFA</h3>
                        <p>Transport</p>
                        <small>Frais de déplacement, logistique</small>
                    </div>
                    <div class="icon icon-transport"><i class="fa fa-truck"></i></div>
                </div>
            </div>
            <div class="col-md-4 col-sm-6">
                <div class="small-box" style="border-left: 5px solid #3498db;">
                    <div class="inner">
                        <h3><?= number_format($total_taxe, 0, ",", " ") ?> FCFA</h3>
                        <p>Taxes</p>
                        <small>TVA, taxes diverses</small>
                    </div>
                    <div class="icon icon-taxe"><i class="fa fa-percent"></i></div>
                </div>
            </div>
            <div class="col-md-4 col-sm-6">
                <div class="small-box" style="border-left: 5px solid #e67e22;">
                    <div class="inner">
                        <h3><?= number_format($total_impot, 0, ",", " ") ?> FCFA</h3>
                        <p>Impôts</p>
                        <small>Impôts sur les sociétés, patente, etc.</small>
                    </div>
                    <div class="icon icon-impot"><i class="fa fa-bank"></i></div>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="alert alert-info">📊 <strong>Détail par source :</strong> Factures fournisseur : <?= formatMontant($total_achats_ttc) ?> | Sorties de caisse : <?= formatMontant($total_caisse_sorties) ?></div>
            </div>
        </div>
                ...
                <table class="table table-bordered"><thead><tr><th>Mois</th><th>Montant (FCFA)</th></tr></thead><tbody>
                    <?php if (!empty($mois_labels_evolution)): ?>
                        <?php foreach ($mois_labels_evolution as $idx => $label): ?>
                            <tr><td><?= $label ?></td><td><?= number_format($ca_realise_mensuel[$idx],0,","," ") ?> FCFA</span></td>
                        <?php endforeach; ?>
                        <tr class="active"><td><strong>TOTAL</strong></span></td><td><strong><?= number_format($ca_realise,0,","," ") ?> FCFA</strong></span></tr>
                    <?php else: ?>
                        <tr><td colspan="2">Aucune donnée</span></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
...
<div class="modal fade" id="modalCaEncaisse" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <h5>Évolution mensuelle</h5>
                <table class="table table-bordered"><thead><tr><th>Mois</th><th>Montant encaissé (FCFA)</th></td></thead><tbody>
                    <?php if (!empty($mois_labels_evolution)): ?>
                        <?php foreach ($mois_labels_evolution as $idx => $label): ?>
                            <tr><td><?= $label ?></td><td><?= number_format($ca_encaisse_mensuel[$idx],0,","," ") ?> FCFA</span></td>
                        <?php endforeach; ?>
                        <tr class="active"><td><strong>TOTAL</strong></span></td><td><strong><?= number_format($ca_encaisse,0,","," ") ?> FCFA</strong></span></tr>
                    <?php else: ?>
                        <tr><td colspan="2">Aucune donnée</span></tr>
                    <?php endif; ?>
                    </tbody>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <table class="table table-bordered"><thead><tr><th>Mois</th><th>Créance (FCFA)</th></tr></thead><tbody>
                    <?php if (!empty($mois_labels_evolution)): ?>
                        <?php foreach ($mois_labels_evolution as $idx => $label): ?>
                            <tr><td><?= $label ?></td><td><?= number_format($creance_mensuel[$idx],0,","," ") ?> FCFA</span></td>
                        <?php endforeach; ?>
                        <tr class="active"><td><strong>TOTAL</strong></td><td><strong><?= number_format($creance,0,","," ") ?> FCFA</strong></span></tr>
                    <?php else: ?>
                        <tr><td colspan="2">Aucune donnée</span></tr>
                    <?php endif; ?>
                    </tbody>
                ...
                <table class="table table-bordered table-striped"><thead><tr><th>Client</th><th>Créance (FCFA)</th></tr></thead><tbody>
                    ...
                    <tr><td><?= htmlspecialchars($client['nom']) ?></td><td><?= number_format($client['creance'],0,","," ") ?> FCFA</span></td>
                        <?php endforeach; ?>
                        <?php else: ?>
                    <tr><td colspan="2">Aucun client avec créance</span></td>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <table class="table table-bordered"><tr><th>Source</th><th>Montant (FCFA)</th></tr>
                    <tr><td>Factures fournisseur</span> </td> <?= number_format($total_achats_ttc,0,","," ") ?> </span> </tr>
                    <tr><td><span>Sorties de caisse</span> </td><?= number_format($total_caisse_sorties,0,","," ") ?> </span> </tr>
                    <tr class="active"><td><span><strong>TOTAL</strong></span> </td><strong><?= number_format($total_depenses_global,0,","," ") ?> FCFA</strong></span> </tr>
                </table>
                ...
                <div class="table-responsive"><table class="table table-bordered"><thead><tr><th>Mois</th><th>Factures</th><th>Caisse</th><th>Total</th><tr></thead><tbody>
                        <?php if (!empty($depenses_mensuel_labels)): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="4">Aucune donnée</span></tr>
                        <?php endif; ?>
                        </tbody> </table></div>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <table class="table table-bordered"><tr><th>Source</th><th>Montant décaissé (FCFA)</th></tr>
                    <tr><td>Factures fournisseur (payé)</span> </td> <?= number_format($achats_paye,0,","," ") ?> </span> </tr>
                    <tr><td>Sorties de caisse</span> </td><?= number_format($total_caisse_sorties,0,","," ") ?> </span> </tr>
                    <tr class="active"><td><strong>TOTAL DÉCAISSÉ</strong></span> </td><strong><?= number_format($total_deja_paye,0,","," ") ?> FCFA</strong></span> </tr>
                </table>
                ...
                <table class="table table-bordered"><thead><tr><th>Mois</th><th>Montant décaissé (FCFA)</th></tr></thead><tbody>
                    <?php if (!empty($depenses_mensuel_labels)): ?>
                        <?php foreach ($depenses_mensuel_labels as $idx => $label): ?>
                            <tr><td><?= $label ?></td><td><?= number_format($decaissement_mensuel_total[$idx],0,","," ") ?> FCFA</span></td>
                        <?php endforeach; ?>
                        <tr class="active"><td><strong>TOTAL</strong></span> </td><strong><?= number_format($total_deja_paye,0,","," ") ?> FCFA</strong></span> </tr>
                    <?php else: ?>
                    <tr><td colspan="2">Aucune donnée</span></td>
                        <?php endif; ?>
                    </tbody> </table>
            </div>
        </div>
                ...
                <table class="table table-bordered table-striped"><thead><tr><th>Fournisseur</th><th>Dette (FCFA)</th></tr></thead><tbody>
                    <?php if (!empty($all_fournisseurs_dette)): ?>
                        <?php foreach ($all_fournisseurs_dette as $fourn): ?>
                            <tr><td><?= htmlspecialchars($fourn['nom']) ?></td><td><?= number_format($fourn['dette'],0,","," ") ?> FCFA</span></td>
                        <?php endforeach; ?>
                        <tr class="active"><td><strong>TOTAL DETTE</strong></td><td><strong><?= number_format($reste_a_payer_global,0,","," ") ?> FCFA</strong></td></span> </tr>
                    <?php else: ?>
                    <tr><td colspan="2">Aucune dette fournisseur pour la période sélectionnée.</span></td>
                        <?php endif; ?>
                    </tbody> </table>

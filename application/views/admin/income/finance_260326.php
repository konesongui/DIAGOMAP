...
// 3. CRÉANCE (Reste à payer)
$sqlCreance = "SELECT SUM(remaining_amount) AS creance 
               FROM invoices 
               WHERE YEAR(created_at) = YEAR(CURDATE())";
$resultCreance = $conn->query($sqlCreance);
$creance = ($resultCreance && ($r = $resultCreance->fetch_assoc())) ? (float)$r['creance'] : 0;
...
// ===============================
$sqlObjectifs = "SELECT * FROM `objectifs_commercial` ORDER BY date DESC, empname ASC";
...
if ($resObjectifs && $resObjectifs->num_rows > 0) {
    $has_objectifs = true;
    while ($row = $resObjectifs->fetch_assoc()) {
        $objectifs_data[] = $row;

        // Regrouper par année
        $annee_objectif = date('Y', strtotime($row['date']));
        if (!isset($objectifs_par_annee[$annee_objectif])) {
            $objectifs_par_annee[$annee_objectif] = [];
        }
        $objectifs_par_annee[$annee_objectif][] = $row;

        // Objectif de l'année courante
        if ($annee_objectif == $annee_en_cours) {
            $objectif_annee_courante += $row['target_amount'];
        }
    }
}
...
$sqlEvolutionMensuelle = "
    SELECT 
        ...
        SUM(remaining_amount) AS creance
    FROM invoices
    WHERE YEAR(created_at) = YEAR(CURDATE())
...
if ($resEvolution && $resEvolution->num_rows > 0) {
    ...
    while ($row = $resEvolution->fetch_assoc()) {
        ...
        $creance_mensuel[$mois_index] = (float)$row['creance'];
    }
...
$sqlTopClientsCreance = "
    SELECT 
        ...
        SUM(i.remaining_amount) AS creance_totale
    FROM invoices i
...
if ($resTopClients && $resTopClients->num_rows > 0) {
    ...
    while ($row = $resTopClients->fetch_assoc()) {
        ...
        $creance_clients[] = (float)$row['creance_totale'];
    }
}
...
foreach($creance_clients as $creance) {
    $top_clients_percentages[] = $total_creance_top > 0 ? round(($creance / $total_creance_top) * 100, 1) : 0;
}
...
$sqlRepartitionStatus = "
    SELECT 
        CASE 
            WHEN remaining_amount = 0 THEN 'Payé'
...
<div class="content-wrapper">
    ...
    <section class="content">
            ...
            <div class="col-md-12">
                <div class="comparison-row">
                    <div class="row">
                        ...
                        <!--<div class="col-md-2 comparison-item">
                            <div class="comparison-label"><i class="fa fa-arrow-right"></i> ÉCART</div>
                            <div class="comparison-ecart <?= $ecart_objectif >= 0 ? 'text-success' : 'text-danger' ?>">
                                <?= ($ecart_objectif >= 0 ? '+' : '') . number_format($ecart_objectif, 0, ",", " ") ?> FCFA
                            </div>
                        </div>-->
                        <div class="col-md-2 comparison-item">
        ...
        <?php if ($has_any_data): ?>
            ...
            <!-- Tableau des objectifs par commercial -->
            ...
            <div class="row">
                ...
                <!--<div class="col-md-6">
                    <div class="box box-success">
                        <div class="box-header">
                            <h3 class="box-title"><i class="fa fa-pie-chart"></i> Répartition par statut de paiement - <?= $annee_en_cours ?></h3>
                        </div>
                        <div class="box-body">
                            <div class="chart-container">
                                <?php if ($has_status_data): ?>
                                    <canvas id="chartRepartitionStatus"></canvas>
                                <?php else: ?>
                                    <div class="chart-placeholder">
                                        <p>Aucune donnée de répartition par statut</p>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
            ...
            <!--<div class="row">
                <div class="col-md-4">
                    <div class="info-box">
                        ...
                        <div class="info-box-content">
                            <span class="info-box-text">CA Réalisé <?= $annee_en_cours ?></span>
                            <span class="info-box-number"><?= number_format($ca_realise, 0, ",", " ") ?> FCFA</span>
                            <div class="progress" style="margin-top: 10px;">
                                <div class="progress-bar bg-aqua" style="width: 100%"></div>
                            </div>
                            <small>Total des ventes facturées</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="info-box">
                        ...
                        <div class="info-box-content">
                            <span class="info-box-text">Taux d'encaissement</span>
                            <span class="info-box-number"><?= $taux_encaissement ?>%</span>
                            <div class="progress" style="margin-top: 10px;">
                                <div class="progress-bar bg-green" style="width: <?= min(100, $taux_encaissement) ?>%"></div>
                            </div>
                            <small>Pourcentage encaissé sur le CA réalisé</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="info-box">
                        ...
                        <div class="info-box-content">
                            <span class="info-box-text">Créance en attente</span>
                            <span class="info-box-number"><?= number_format($creance, 0, ",", " ") ?> FCFA</span>
                            <div class="progress" style="margin-top: 10px;">
                                <?php
            $creance_percent = ($ca_realise > 0) ? ($creance / $ca_realise * 100) : 0;
            ?>
                                <div class="progress-bar bg-red" style="width: <?= min(100, $creance_percent) ?>%"></div>
                            </div>
                            <small><?= $creance > 0 ? 'À encaisser' : 'Tout est encaissé' ?></small>
                        </div>
                    </div>
                </div>
            </div>-->
...
<script>
    ...
    if (typeof Chart !== 'undefined') {
        ...
        // 3. Répartition par statut - Version camembert avec pourcentages
        <?php if ($has_status_data): ?>
        try {
            new Chart(document.getElementById('chartRepartitionStatus').getContext('2d'), {
                type: 'pie',
                data: {
                    labels: <?= json_encode($status_labels) ?>,
                    datasets: [{
                        data: <?= json_encode($status_ca) ?>,
                        backgroundColor: [
                            'rgba(40, 167, 69, 0.8)',   // Vert pour Payé
                            'rgba(255, 193, 7, 0.8)',   // Jaune pour Partiellement payé
                            'rgba(220, 53, 69, 0.8)'    // Rouge pour Non payé
                        ],
                        borderColor: [
                            'rgb(40, 167, 69)',
                            'rgb(255, 193, 7)',
                            ...
                            labels: {
                                generateLabels: function(chart) {
                                    ...
                                    if (data.labels.length && data.datasets.length) {
                                        return data.labels.map((label, i) => {
                                            const value = data.datasets[0].data[i];
                                            const percentage = <?= json_encode($status_percentages) ?>[i];
                                            return {
                                                text: `${label}: ${percentage}% (${value.toLocaleString('fr-FR')} FCFA)`,
                                                fillStyle: data.datasets[0].backgroundColor[i],
                                                strokeStyle: data.datasets[0].borderColor[i],
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    const label = context.label || '';
                                    const value = context.raw;
                                    const percentage = <?= json_encode($status_percentages) ?>[context.dataIndex];
                                    return `${label}: ${value.toLocaleString('fr-FR')} FCFA (${percentage}%)`;
                                }
                            }
                        }
            ...
            console.log('Graphique Répartition Statut (camembert) créé');
        } catch (e) {
            console.error('Erreur création graphique Répartition Statut:', e);
        }
        <?php endif; ?>
    } else {

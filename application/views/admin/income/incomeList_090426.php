...
if (empty($date_fin)) {
    $date_fin = date('LIVRE DE CAISSEY-m-d');
}
                ...
                <div class="box box-primary">
                    ...
                    <div class="collapse" id="caissesPanel">
                        <div class="box-body">
                            ...
                            <?php
                            // 1. Récupérer les totaux de TOUTES les caisses (actives + inactives) avec jointure sur operation_caisse
                            $this->db->select('
    SUM(i.amount) as total_amount_all,
    SUM(o.entree) as total_entrees_all,
    SUM(o.sortie) as total_sorties_all,
    COUNT(DISTINCT i.id) as nb_caisses_total
');
                            $this->db->from('income as i');
                            $this->db->join('operation_caisse as o', 'o.caisse_id = i.id', 'left');
                            $this->db->where('i.is_deleted', 'no');
                            $totaux_toutes_caisses = $this->db->get()->row();

                            // 2. Récupérer les totaux des caisses ACTIVES seulement
                            $this->db->select('
    SUM(i.amount_re) as total_amount_re_actives,
    SUM(i.amount) as total_amount_actives,
    COUNT(DISTINCT i.id) as nb_caisses_actives
');
                            $this->db->from('income as i');
                            $this->db->where('i.is_deleted', 'no');
                            $this->db->where('i.est_actif', '1');
                            ...
                            $this->db->where_in('type_operation', ['ENTREE', 'SORTIE']);
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 text-center">
                                            ...
                                            <small style="font-size: 14px; color: #666;">
                                                (<?php echo date('d/m/Y', strtotime($date_totaux_debut ?? date('Y-m-01'))); ?>
                                                au <?php echo date('d/m/Y', strtotime($date_totaux_fin ?? date('Y-m-d'))); ?>)
                                            </small>
                                        </div>
                                    </div>
                                                ...
                                                <form method="get" action="<?php echo base_url('admin/income') ?>" class="form-inline" id="formTotaux">
                                                    ...
                                                    <div class="form-group">
                                                        <input type="date" name="date_totaux_debut" id="date_totaux_debut" class="form-control input-sm"
                                                               value="<?php echo $date_totaux_debut ?? date('Y-m-01'); ?>" style="margin-right: 10px;">
                                                    </div>

                                                    <div class="form-group">
                                                        <input type="date" name="date_totaux_fin" id="date_totaux_fin" class="form-control input-sm"
                                                               value="<?php echo $date_totaux_fin ?? date('Y-m-d'); ?>" style="margin-right: -2px;">
                                                    </div>
                                            ...
                                            <div class="total-stat-box box-entrees">
                                                ...
                                                <div class="total-stat-subtext">
                                                    <button type="button" class="btn btn-xs btn-info" onclick="voirOperationsParType('entree')">
                                                        <!--<i class="fa fa-eye"></i> <?php echo $nb_operations_entrees ?? 0; ?> opérations-->
                                                        <i class="fa fa-eye"></i>
                                                    </button>
                                                </div>
                                            </div>
                                            ...
                                            <div class="total-stat-box box-sorties">
                                                ...
                                                <div class="total-stat-subtext">
                                                    <button type="button" class="btn btn-xs btn-info" onclick="voirOperationsParType('sortie')">
                                                        <!-- <i class="fa fa-eye"></i> <?php echo $nb_operations_sorties ?? 0; ?> opérations-->
                                                        <i class="fa fa-eye"></i>
                                                    </button>
                                                </div>
                                            </div>
                                ...
                                <table class="table table-striped table-bordered table-hover" id="caisses-table">
                                   ...
                                   title="Supprimer la caisse">
                                    ...
                                    <tr style="background-color: #f0f0f0; font-weight: bold;">
                                        ...
                                        <td class="text-right"><?php echo number_format($total_amount, 0, ',', ' '); ?></td>
                                        ...
                                        <td class="text-right <?php echo $total_amount_re >= 0 ? 'text-success' : 'text-danger'; ?>">
                                            <?php echo number_format($total_amount_re, 0, ',', ' '); ?>
                                        </td>
                                        <td colspan="4"></td>
                                    </tr>
                                    </tfoot>
                                        ...
                                        <tr class="<?php echo $is_reappro ? 'reappro-row' : ''; ?>"
                                            ...
                                            <td class="no-print text-center">
                                                <div class="btn-group btn-group-xs" role="group">
                                                    ...
                                                    <?php
                                                    // DÉBOGAGE - À RETIRER APRÈS TEST
                                                    echo "<!-- DÉBOGAGE PERMISSIONS CAISSE -->";

                                                    // Vérifier si l'utilisateur a la permission de supprimer en général
                                                    $has_delete_permission = $this->rbac->hasPrivilege('caisse', 'can_delete');
                                                    echo "<!-- has_delete_permission: " . ($has_delete_permission ? 'OUI' : 'NON') . " -->";
                                                    ...
                                                    echo "<!-- is_superadmin: " . ($is_superadmin ? 'OUI' : 'NON') . " -->";
                                                    echo "<!-- is_admin: " . ($is_admin ? 'OUI' : 'NON') . " -->";
                                                    echo "<!-- is_admin_user: " . ($is_admin_user ? 'OUI' : 'NON') . " -->";

                                                    // Date de l'opération
                                                    $date_operation = !empty($operation['date']) ? $operation['date'] : '';
                                                    echo "<!-- date_operation brute: " . $date_operation . " -->";

                                                    // Extraire le jour de la date d'opération
                                                    $jour_operation = '';
                                                    if (!empty($date_operation)) {
                                                        if (strpos($date_operation, '-') !== false) {
                                                            if (strlen($date_operation) > 8) {
                                                                // Format Y-m-d
                                                            ...
                                                            // Autre format
                                                            $jour_operation = $date_operation;
                                                        }
                                                    }
                                                    echo "<!-- jour_operation extrait: " . $jour_operation . " -->";

                                                    // Jour actuel
                                                    $jour_actuel = date('d');
                                                    echo "<!-- jour_actuel: " . $jour_actuel . " -->";

                                                    // Déterminer si l'utilisateur peut supprimer
                                                    $peut_supprimer = false;
                                                    if ($is_admin_user) {
                                                        $peut_supprimer = true;
                                                        echo "<!-- ADMIN: peut supprimer toutes les opérations -->";
                                                    } else {
                                                        if ($has_delete_permission && $jour_operation == $jour_actuel) {
                                                            $peut_supprimer = true;
                                                            echo "<!-- USER STANDARD: peut supprimer car date du jour -->";
                                                        } else {
                                                            echo "<!-- USER STANDARD: ";
                                                            if (!$has_delete_permission) echo "pas de permission can_delete, ";
                                                            if ($jour_operation != $jour_actuel) echo "date différente du jour (" . $jour_operation . " != " . $jour_actuel . ")";
                                                            echo " -->";
                                                        }
                                                    }
                                                    echo "<!-- peut_supprimer final: " . ($peut_supprimer ? 'OUI' : 'NON') . " -->";
                                                    echo "<!-- FIN DÉBOGAGE -->";
                                                    ...
                                                    if ($has_delete_permission):
                                                        ...
                                                        $title_message = '';
                                                        if (!$peut_supprimer) {
                                                            $title_message = "Opération du " . $jour_operation . " - Vous ne pouvez supprimer que les opérations du jour (" . $jour_actuel . ")";
                                                        } else if ($is_admin_user) {
                                                            $title_message = "Supprimer cette opération (Autorisation Admin)";
                                                        } else {
                                                            $title_message = "Supprimer cette opération (Date du jour)";
                                                        }
                                                        ?>
                                                    ...
                                                    <?php endif; ?>
                                                    <?php else: ?>
                                                        <!-- L'utilisateur n'a pas la permission can_delete -->
                                                        <!-- Pas de bouton affiché -->
                                                    <?php endif; ?>
                                                </div>
                                            </td>
                                        </tr>
...
<script>
    ( function ( $ ) {
        ...
        $(document).ready(function () {
            ...
            // Initialiser DataTable pour les caisses si la bibliothèque est disponible
            if ($.fn.DataTable) {
...
<script>
    ...
    // Fonction d'impression corrigée pour le livre de caisse avec alignement des colonnes
    function imprimerLivreCaisse() {
        ...
        // Récupérer les données du tableau ligne par ligne
        var tableData = [];
        var totalEntrees = 0;
        var totalSorties = 0;
        var soldeFinal = 0;

        // Parcourir les lignes du tableau (tbody)
        var tbodyRows = document.querySelectorAll('#livre-caisse-table tbody tr');
        tbodyRows.forEach(function(row) {
            // Ignorer la ligne de solde initial si elle existe
    ...
    `;
        ...
        if (tableData.length > 0) {
            ...
            printContent += `
            <tr>
                <td colspan="9" class="text-center">Aucune opération trouvée</td>
            </tr>
        `;
        }
...
<script>
                        ...
                        if (response.success) {
                            ...
                            Swal.fire({
                                ...
                                html: '<div style="text-align:center;">' +
                                    '<i class="fa fa-check-circle" style="color:black;font-size:48px;margin-bottom:20px;"></i>' +
                    ...
                    error: function(xhr, status, error) {
                        ...
                        Swal.fire({
                            ...
                            html: '<div style="text-align:center;">' +
                                ...
                                '<p style="color:black;margin-bottom:20px;">Cliquez sur OK pour rafraîchir le tableau.</p>' +
                                '</div>',
                            icon: 'success',
                            showCancelButton: false,
                            confirmButtonColor: '#3085d6',
    ...
    $(document).ready(function() {
        ...
        // Fonction pour confirmer la suppression d'une caisse
    ...
    // Fonction pour mettre à jour la date de création
        ...
        if (confirm('Voulez-vous vraiment mettre à jour la date de création de la caisse "' + caisseName + '" avec la date du mois en cours ?')) {
            ...
            dataType: 'json',
                ...
                if (response.status === 'success') {
                    ...
                    // Recharger la page après 1 seconde pour afficher la nouvelle date
                    setTimeout(function() {
                        location.reload();
                    }, 1000);
                } else {
    ...
    // Écouteur d'événements pour les boutons (à placer dans $(document).ready)
    $(document).ready(function() {
...
</html> en arrangent les tables et données d'impression
...
if (isset($_GET['export']) && in_array($_GET['export'], ['excel', 'pdf'])) {
    ...
    // ... (le code des exports est à conserver à l'identique, non réécrit ici pour la clarté)
}
...
// ===============================
// 9. DÉPENSES : BANK (designation = 'debit' ou 'débit')
// ===============================
...
// ===============================
// 10. TOTAUX GLOBAUX DES DÉPENSES
// ===============================
...
// ===============================
// 11. FUSION DES TROIS SOURCES POUR LA COURBE MENSUELLE
// ===============================
$caisse_brut = $caisse_sorties_par_mois;
$bank_brut = $bank_debits_par_mois;
$toutes_cles = array_unique(array_merge(array_keys($achats_mensuel_data), array_keys($caisse_brut), array_keys($bank_brut)));
sort($toutes_cles);
...
foreach ($toutes_cles as $moisKey) {
    ...
    $depenses_mensuel_total[] = $facture_total + $caisse_total + $bank_total;

    $paye_facture = isset($achats_mensuel_data[$moisKey]) ? $achats_mensuel_data[$moisKey]['paye'] : 0;
    $decaissement_mensuel_total[] = $paye_facture + $caisse_total + $bank_total;
}
...
// ===============================
// EXPORTS EXCEL ET PDF (à conserver à l'identique)
// ===============================
// ... (le code d'export non réécrit ici)
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<div class="content-wrapper">
    ...
    <section class="content">
        <!-- Filtres avec bouton Actualiser réinitialisant les filtres -->
        <div class="filter-bar">
            <form method="GET" action="" class="form-inline" id="filterForm">
                ...
                <!-- Bouton Actualiser : recharge la page sans paramètres GET -->
            ...
            <div class="col-md-4 col-sm-6">
                <div class="small-box">
                    <div class="inner">
                        ...
                        <small>Factures + Caisse + Banque</small>
                        ...
                        <div class="box-body">
                           ...
                           <!-- <div class="table-responsive" style="margin-top:20px;">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                    <tr class="active"><td>CA Total</th><td><strong><?= number_format($ca_realise,0,","," ") ?> FCFA</strong></td><td>100%</td></tr>
                                    <tr class="success"><td>Déjà encaissé</th><td><?= number_format($ca_encaisse,0,","," ") ?> FCFA</td><td><?= $pourcentage_encaisse ?>%</td></tr>
                                    <tr class="danger"><td>Reste à encaisser</th><td><?= number_format($creance,0,","," ") ?> FCFA</td><td><?= $pourcentage_reste ?>%</td></tr>
                                    </tbody>
                                </table>
                            </div>-->
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <table class="table table-bordered"><tr><th>Source</th><th>Montant (FCFA)</th></tr>
                    ...
                    <tr><td>Débits bancaires</td><td><?= number_format($total_bank_debits,0,","," ") ?></td></tr>
                ...
                <div class="table-responsive"><table class="table table-bordered"><thead><tr><th>Mois</th><th>Factures</th><th>Caisse</th><th>Banque</th><th>Total</th></tr></thead><tbody>
                        <?php if (!empty($depenses_mensuel_labels)): ?>
                            <?php foreach ($depenses_mensuel_labels as $idx => $label): ?>
                                <tr><td><?= $label ?></td><td><?= number_format($depenses_mensuel_factures[$idx],0,","," ") ?></td><td><?= number_format($depenses_mensuel_caisse[$idx],0,","," ") ?></td><td><?= number_format($depenses_mensuel_bank[$idx],0,","," ") ?></td><td><strong><?= number_format($depenses_mensuel_total[$idx],0,","," ") ?></strong></td></tr>
                            <?php endforeach; ?>
                        <?php else: ?>

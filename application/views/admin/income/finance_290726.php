...
// ===============================
// Priorité : paramètre GET explicite, sinon valeur en session (entreprise courante de l'utilisateur connecté)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_observation'])) {
    // Observation stockée par entreprise pour éviter les collisions entre entreprises
...
$sqlCARealise = "SELECT COALESCE(SUM(montant), 0) AS ca_realise FROM (
                    SELECT total_ttc AS montant FROM invoices WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?
                    UNION ALL
                    SELECT total_ttc AS montant FROM quotes_selling WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?
                    UNION ALL
...
$sqlCAEncaisse = "SELECT COALESCE(SUM(montant), 0) AS ca_encaisse FROM (
                    SELECT amount_paid AS montant FROM invoices WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?
                    UNION ALL
                    SELECT amount_paid AS montant FROM quotes_selling WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?
                    UNION ALL
                    SELECT montant FROM operation_caisse 
                    WHERE type_operation = 'ENTREE' 
                      AND date BETWEEN ? AND ?
...
$sqlObjectifs = "SELECT SUM(target_amount) as total_objectif 
                 ...
                 WHERE date BETWEEN ? AND ? AND entreprise_id = ?";
$query = $db->query($sqlObjectifs, [$date_debut, $date_fin, $entreprise_id]);
$row = $query->row_array();
...
$sqlEvolutionMensuelle = "
    ...
    FROM (
        ...
        FROM invoices WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?
        UNION ALL
        SELECT DATE_FORMAT(created_at, '%Y-%m'), total_ttc, amount_paid
        FROM quotes_selling WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?
        UNION ALL
...
$query = $db->query($sqlEvolutionMensuelle, [
    $date_debut, $date_fin, $entreprise_id,
    $date_debut, $date_fin, $entreprise_id,
    $date_debut, $date_fin, $entreprise_id
]);
...
$sqlClientsCreance = "
    ...
    FROM (
        ...
        FROM invoices i JOIN clients c ON i.customer_id = c.id AND c.entreprise_id = ?
        WHERE i.created_at BETWEEN ? AND ? AND i.entreprise_id = ?
        UNION ALL
        SELECT c.item_supplier AS client_nom, q.total_ttc AS ca_realise_client, q.amount_paid AS ca_encaisse_client
        FROM quotes_selling q JOIN clients c ON q.customer_id = c.id AND c.entreprise_id = ?
        WHERE q.created_at BETWEEN ? AND ? AND q.entreprise_id = ?
    ) AS all_transactions
    GROUP BY client_nom
    HAVING (SUM(ca_realise_client) - SUM(ca_encaisse_client)) > 0
    ORDER BY (SUM(ca_realise_client) - SUM(ca_encaisse_client)) DESC
    LIMIT 20";
$query = $db->query($sqlClientsCreance, [
    $entreprise_id, $date_debut, $date_fin, $entreprise_id,
    $entreprise_id, $date_debut, $date_fin, $entreprise_id
]);
$all_clients_creance = [];
foreach ($query->result_array() as $row) {
...
$sqlRepartitionStatus = "
    ...
    FROM (
        SELECT total_ttc, amount_paid FROM invoices WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?
        UNION ALL
        SELECT total_ttc, amount_paid FROM quotes_selling WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?
    ) AS all_transactions
    GROUP BY 
        CASE 
            WHEN (total_ttc - amount_paid) = 0 THEN 'Payé'
            ...
            ELSE 'Non payé'
        END";
$query = $db->query($sqlRepartitionStatus, [
    $date_debut, $date_fin, $entreprise_id,
    $date_debut, $date_fin, $entreprise_id
]);
$status_labels = [];
...
$sqlTotalAchats = "SELECT COALESCE(SUM(total_ttc), 0) AS total_achats 
                   ...
                   WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?";
$query = $db->query($sqlTotalAchats, [$date_debut, $date_fin, $entreprise_id]);
$row = $query->row_array();
...
$sqlAchatsPaye = "SELECT COALESCE(SUM(amount_paid), 0) AS achats_paye 
                  FROM invoices_supplier 
                  WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?";
$query = $db->query($sqlAchatsPaye, [$date_debut, $date_fin, $entreprise_id]);
$row = $query->row_array();
...
$sqlEvolutionAchats = "
    ...
    GROUP BY annee_mois
    ORDER BY annee_mois ASC";
$query = $db->query($sqlEvolutionAchats, [$date_debut, $date_fin, $entreprise_id]);
...
$sqlFournisseursDette = "
    ...
    JOIN item_supplier s ON i.customer_id = s.id AND s.entreprise_id = ?
    WHERE i.created_at BETWEEN ? AND ? AND i.entreprise_id = ?
    GROUP BY s.id
    HAVING dette > 0
    ORDER BY dette DESC
    LIMIT 20";
$query = $db->query($sqlFournisseursDette, [$entreprise_id, $date_debut, $date_fin, $entreprise_id]);
...
$sqlNbTransactions = "SELECT COUNT(*) as nb FROM (
    SELECT id FROM invoices WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?
    UNION ALL
    SELECT id FROM quotes_selling WHERE created_at BETWEEN ? AND ? AND entreprise_id = ?
) AS all_transactions";
$query = $db->query($sqlNbTransactions, [
    $date_debut, $date_fin, $entreprise_id,
    $date_debut, $date_fin, $entreprise_id
]);
$row = $query->row_array();
...
$sqlCategories = "SELECT eh.id, eh.exp_category, SUM(oc.montant) as total
                  ...
                  ORDER BY total DESC";
$query = $db->query($sqlCategories, [$entreprise_id, $date_debut, $date_fin, $entreprise_id]);
$categories_depenses = [];
foreach ($query->result_array() as $row) {
    $categories_depenses[] = [
        'id' => (int)$row['id'],
...
$query = $db->query($sqlOperationsDetail, [$entreprise_id, $date_debut, $date_fin, $entreprise_id]);
$operations_detail = [];
foreach ($query->result_array() as $row) {
    $operations_detail[] = [
        'id' => $row['id'],

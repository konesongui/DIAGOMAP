<ul class="timeline timeline-inverse">
    ...
    <?php
    if (empty($candidatures_up_list)) {
        ?>
        <?php
    } else {
        foreach ($candidatures_up_list as $key => $value) {
            ?>
            <li class="time-label">
                <span class="bg-blue">
                    <?php
...
<script>
    ...
    function delete_next_call(trainingreq_up_id, trainingreq_id) {
        ...
        if (permission == false) {
            ...
            $.ajax({
                url: '<?php echo base_url(); ?>admin/training_request/follow_up_delete/' + trainingreq_up_id + '/' + trainingreq_id,

                success: function (data) {
                    follow_up(trainingreq_id);
    ...
    function follow_up(id) {
        $.ajax({
            url: '<?php echo base_url(); ?>admin/candidatures/postuler/' + id + "/" + status,
            success: function (data) {
                $('#getdetails_follow_up').html(data);
                $.ajax({

                    url: '<?php echo base_url(); ?>admin/candidatures/postuler_up_list/' + id,

<div class="row row-eq">
  ...
  <!--
        ...
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-6">
                <div class="form-group" hidden>
                    ...
                    <input type="hidden" id="trainingreq_id" name="trainingreq_id" value="<?php echo $training_data['id'] ?>">
                    <input type="hidden" id="trainingreq_status" name="trainingreq_status" value="<?php echo $training_data['status'] ?>">
                    <input type="text" id="follow_date" name="date" class="form-control date" value="<?php echo set_value('follow_up_date', date($this->customlib->getSchoolDateFormat(), $this->customlib->dateyyyymmddTodateformat($training_data['date']))); ?>" readonly="">
            ...
            <div class="col-lg-6 col-md-6 col-sm-6" hidden>
                <div class="form-group">
                    <label for="pwd">Note</label><small class="req"> *</small>
    ...
    <div class="ptbnull" hidden>
        <h4 class="box-title titlefix pb5"><?php echo $this->lang->line('follow_up'); ?> (<?php print_r($training_data['name']); ?>)</h4>
        <div class="box-tools pull-right">
        </div>
    </div>
    <div class="pt20" hidden>
        ...
        <div class="taskside">
            ...
            <h4><?php echo $this->lang->line('summary'); ?>
                <div style="font-size: 15px;" class="box-tools pull-right">
                    ...
                    <div class="form-group">
                        <select class="form-control" id="status_data" onchange="changeStatus(this.value, '<?php echo $training_data['id'] ?>')">

                            <?php
                            foreach ($enquiry_status as $enkey => $envalue) {
                                ?>
                                <option <?php
                                if ($training_data["status"] == $enkey) {
                                    echo "selected";
                                }
           ...
           <!-- <div class="task-info task-single-inline-wrap task-info-start-date">
                <h5><i class="fa task-info-icon fa-fw fa-lg fa-calendar-plus-o pull-left fa-margin"></i>
                    <?php echo $this->lang->line('enquiry'); ?> <?php echo $this->lang->line('date'); ?>: <?php print_r(date($this->customlib->getSchoolDateFormat(), $this->customlib->dateyyyymmddTodateformat($training_data['date']))); ?>
                </h5>
            </div>-->
            ...
            <!--<div class="task-info task-single-inline-wrap task-info-start-date">
                <h5><i class="fa task-info-icon fa-fw fa-lg fa-calendar-plus-o pull-left fa-margin"></i>
                    ...
                    if (!empty($next_date)) {
                        ...
                        echo date($this->customlib->getSchoolDateFormat(), $this->customlib->dateyyyymmddTodateformat($training_data['follow_up_date']));
                    }
                    ?>                                       
                </h5>
            </div>-->
            <div class="task-info task-single-inline-wrap ptt10">
                ...
                <label>Employée: <?php echo $training_data['name']; ?></label>
                <label>Intitulé de la formation: <?php echo $training_data['training_name']; ?></label>
                <label>Objectifs de la formation: <?php echo $training_data['objectifs']; ?></label>
               <!-- <label>Responsable: <?php echo $training_data['assigned']; ?></label>-->
                <label>Date: <?php echo $training_data['date']; ?></label>
                 <label><?php echo $this->lang->line('assigned'); ?>: <?php echo $training_data['assigned']; ?></label>
                <label>Statut: <?php echo $training_data['status']; ?></label>
               </div>
        </div>
    </div>  
</div>
<script>
    function follow_save() {
        ...
        var id = $('#trainingreq_id').val();
        var status = $('#enquiry_status').val();
        var responce = $('#response').val();
        var follow_date = $('#follow_date').val();       

        $.ajax({
            url: '<?php echo base_url(); ?>admin/training_request/trainingreq_up_insert',
            type: 'POST',
            dataType: 'json',
    ...
    function changeStatus(status, id) {
        ...
        $.ajax({
            url: '<?php echo base_url(); ?>admin/training_request/change_status/',
            type: 'POST',
            dataType: 'json',

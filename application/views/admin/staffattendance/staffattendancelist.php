<style type="text/css">
    ...
    .qr-result-message.success {
        color: #1d7a1d;
        background: #e9f9ec;
        border: 1px solid #b6ebc2;
        border-radius: 4px;
        padding: 8px 10px;
    }
    .qr-result-message.error {
        color: #b31b1b;
        background: #fff1f1;
                        ...
                        if (!empty($resultlist)) {
                            ...
                            <form action="<?php echo site_url('admin/staffattendance/index') ?>" id="save_attendance" method="post">
                                ...
                                <div class="qr-attendance-panel">
                                    <div class="qr-attendance-row">
                                        <div class="qr-input-group">
                                            <label for="qrManualInput"><strong>Presence par QR code</strong></label>
                                            <input type="text" id="qrManualInput" class="form-control" placeholder="Scannez ou collez le contenu QR puis validez">
                                        </div>
                                        <button type="button" id="btnSubmitQrManual" class="btn btn-success btn-sm">
                                            <i class="fa fa-check"></i> Valider le scan
                                        </button>
                                        <button type="button" id="btnOpenQrScanner" class="btn btn-info btn-sm">
                                            <i class="fa fa-camera"></i> Ouvrir la camera
                                        </button>
                                    </div>
                                    <div id="qrScanResult" class="qr-result-message"></div>
                                </div>
...
<div id="qrScannerModal" class="modal">
    <div class="modal-content" style="max-width: 560px;">
        <div class="modal-header">
            <h2><i class="fa fa-qrcode"></i> Scanner QR presence</h2>
            <span class="close-modal" id="closeQrScannerModal">&times;</span>
        </div>
        <div class="modal-body">
            <p class="text-muted">Autorisez la camera, puis presentez le badge devant l'objectif.</p>
...
<script type="text/javascript">
    ...
    function loadAbsencesData() {
        ...
        $.ajax({
            url: '<?php echo site_url("admin/staffattendance/getAbsencesList"); ?>',
            type: 'POST',
            data: {
            ...
            dataType: 'json',
            beforeSend: function() {
                $('#absencesTableBody').html('<tr><td colspan="4" class="text-center"><i class="fa fa-spinner fa-spin"></i> Chargement...</td></tr>');
            },
            success: function(response) {
                ...
                if (response.length > 0) {
                    ...
                    html = '<tr><td colspan="4" class="text-center text-muted">Aucune absence trouvée</td></tr>';
                }
                $('#absencesTableBody').html(html);
            },
            error: function() {
                $('#absencesTableBody').html('<tr><td colspan="4" class="text-center text-danger">Erreur lors du chargement des données</td></tr>');
            }
        });
    }
    ...
    function loadGlobalStats() {
        ...
        $.ajax({
            url: '<?php echo site_url("admin/staffattendance/getGlobalStats"); ?>',
            type: 'POST',
            data: {
                year: year,
                month: month
            ...
            dataType: 'json',
            beforeSend: function() {
                $('#statsSummary').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> Chargement...</div>');
                $('#employeeStatsBody').html('<tr><td colspan="4" class="text-center"><i class="fa fa-spinner fa-spin"></i> Chargement...</td></tr>');
            },
            success: function(response) {
                // Afficher le résumé
                var summaryHtml = '';
                if (response.summary) {
                    ...
                    summaryHtml += '<div class="stat-value">' + response.summary.total_absences + '</div>';
                ...
                if (response.employees && response.employees.length > 0) {
                    ...
                    employeeHtml = '<tr><td colspan="4" class="text-center text-muted">Aucun employé trouvé</td></tr>';
                }
                $('#employeeStatsBody').html(employeeHtml);
            },
            error: function() {
                $('#statsSummary').html('<div class="text-center text-danger">Erreur lors du chargement</div>');
                $('#employeeStatsBody').html('<tr><td colspan="4" class="text-center text-danger">Erreur lors du chargement</td></tr>');
            }
        });
    }
    ...
    function loadEmployeeStats(employeeId) {
        ...
        $.ajax({
            url: '<?php echo site_url("admin/staffattendance/getEmployeeStats"); ?>',
            type: 'POST',
            data: {
                employee_id: employeeId,
                year: year,
            ...
            beforeSend: function() {
                ...
                $('#monthlyAvg').text('...');
                $('#annualTotal').text('...');
                $('#employeeAbsencesList').html('<tr><td colspan="2" class="text-center"><i class="fa fa-spinner fa-spin"></i> Chargement...</td></tr>');
            },
            success: function(response) {
                $('#totalAbsences').text(response.total_absences);
                $('#monthlyAvg').text(response.monthly_avg);
                $('#annualTotal').text(response.annual_total);

                var html = '';
                if (response.absences && response.absences.length > 0) {
                    $.each(response.absences, function(index, absence) {
                        html += '<tr>';
                    ...
                    html = '<tr><td colspan="2" class="text-center text-muted">Aucune absence sur cette période</td></tr>';
                }
                $('#employeeAbsencesList').html(html);
            },
            error: function() {
                $('#employeeAbsencesList').html('<tr><td colspan="2" class="text-center text-danger">Erreur lors du chargement</td></tr>');
            }
        });
    }
    ...
    function exportStatsToPDF() {
        ...
        // Télécharger le PDF
        var date = new Date();
        var filename = 'statistiques_absences_' + date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + '.pdf';
        doc.save(filename);
    }
</script>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var wedgeBuffer = '';
        var wedgeLastKeyTs = 0;

        function setQrMessage(message, isSuccess) {
            var box = $('#qrScanResult');
            box.removeClass('success error');
            box.addClass(isSuccess ? 'success' : 'error');
            box.text(message);

            if (message) {
                box.stop(true, true).hide().fadeIn(120);
            }
        }

        function getCsrfPayload() {
            var csrfName = '<?php echo $this->security->get_csrf_token_name(); ?>';
            var csrfVal = $('input[name="' + csrfName + '"]').first().val() || '<?php echo $this->security->get_csrf_hash(); ?>';
            var payload = {};
        ...
        function getAttendanceContext() {
            var selectedRole = $('#class_id').val() || $('input[name="user_id"]').val() || 'select';
            var selectedDate = $('input[name="date"]').first().val();
        ...
        function applyScannedAttendanceToTable(staffId) {
            var absentCheckbox = $('#absent_' + staffId);
            if (!absentCheckbox.length) {
                return;
            }

            absentCheckbox.prop('checked', false).trigger('change');
            $('#hidden_attendance_' + staffId).val(presentTypeId);
        }

        function submitQrAttendance(qrData) {
            if (!qrData) {
                setQrMessage('Aucune donnee QR recue.', false);
                return;
            }

            var contextData = getAttendanceContext();
            var postData = {
                qr_data: qrData,
            ...
            $.extend(postData, getCsrfPayload());

            $.ajax({
                url: '<?php echo site_url("admin/staffattendance/scanAttendanceByQr"); ?>',
                type: 'POST',
                dataType: 'json',
                data: postData,
                success: function (response) {
                    if (!response || response.status !== 'success') {
                        var errorMsg = response && response.message ? response.message : 'Erreur lors du scan QR.';
                        setQrMessage(errorMsg, false);
                        alert(errorMsg);
                        return;
                    }

                    if (response.staff && response.staff.staff_id) {
                        applyScannedAttendanceToTable(response.staff.staff_id);
                    }

                    var successMsg = response.message;
                    if (response.staff && response.staff.name) {
                        successMsg += ' [' + response.staff.name + ' - ' + response.staff.employee_id + ']';
                    }
                    setQrMessage(successMsg, true);
                    alert(successMsg);
                    $('#qrManualInput').val('');
                },
                error: function () {
                    setQrMessage('Impossible de contacter le serveur de pointage.', false);
                    alert('Impossible de contacter le serveur de pointage.');
                }
            });
        }

        // Capture "douchette clavier": le lecteur envoie vite une chaine puis Enter.
        $(document).on('keydown', function (e) {
            var targetTag = (e.target && e.target.tagName) ? e.target.tagName.toLowerCase() : '';
            var targetType = (e.target && e.target.type) ? e.target.type.toLowerCase() : '';

            if (targetTag === 'textarea') {
                return;
            }

            if (targetTag === 'input' && (targetType === 'text' || targetType === 'search' || targetType === 'email' || targetType === 'number' || targetType === 'password')) {
                if (e.target.id !== 'qrManualInput') {
                    return;
                }
            }

            var now = Date.now();
            if (now - wedgeLastKeyTs > 250) {
                wedgeBuffer = '';
            }
            wedgeLastKeyTs = now;
            ...
            if (e.key === 'Enter') {
                if (wedgeBuffer.length >= 4) {
                    submitQrAttendance(wedgeBuffer.trim());
                    wedgeBuffer = '';
                    e.preventDefault();
                }
                return;
        ...
        $('#btnSubmitQrManual').on('click', function () {
            var qrText = $('#qrManualInput').val().trim();
        ...
        $('#qrManualInput').on('keypress', function (e) {
            if (e.which === 13) {
                e.preventDefault();
                $('#btnSubmitQrManual').trigger('click');
            }
        });

        $('#btnOpenQrScanner').on('click', function () {
            $('#qrScannerModal').show();

            if (typeof Html5Qrcode === 'undefined') {
                setQrMessage('Bibliotheque camera indisponible. Utilisez la saisie manuelle.', false);
                return;
            }
            ...
            scanner.start(
                ...
                function (decodedText) {
                    stopQrScanner();
                    $('#qrScannerModal').hide();
                    submitQrAttendance(decodedText);
                },
                function () {
                }
            ).then(function () {
                ...
                setQrMessage('Impossible d\'ouvrir la camera. Utilisez la saisie manuelle.', false);
            });
        });

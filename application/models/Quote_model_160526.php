...
class Quote_model extends CI_Model {
    ...
    private function _getListData($username = null)
    {
        ...
        // ð¹ FILTRAGE PAR UTILISATEUR SI SPÉCIFIÉ
        if ($username !== null) {
    ...
    /**
     * Récupère tous les devis d’un client avec leurs articles
    ...
    public function getQuotesByCustomer($customer_id) {
        ...
        foreach ($quotes as &$quote) {
            ...
            item.name as item_name')
                ->from('quote_items')
                ->join('item_category', 'item_category.id = quote_items.category_id')
                ->join('item', 'item.id = quote_items.item_id')
                ->where('quote_id', $quote['id'])
                ->order_by('position', 'ASC');
            $quote['items'] = $this->db->get()->result_array();
        }

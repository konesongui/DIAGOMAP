...
class Quote_purchase_model extends CI_Model {

    protected $table = 'quotes_purchase';
    protected $items_table = 'quote_purchase_items';
    ...
    public function getQuoteWithItems($quote_id)
    {
        ...
        $this->db->select('quotes_purchase.*, item_supplier.item_supplier as customer_name,
        ...
        $this->db->join('item_supplier', 'item_supplier.id = quotes_purchase.supplier_id');
        $this->db->where('quotes_purchase.id', $quote_id);
        ...
        $this->db->select('
             quote_purchase_items.*,
            item_category.item_category as category_name,
            item.name as item_name
        ');
        $this->db->from($this->items_table);
        $this->db->join('item_category', 'item_category.id =  quote_purchase_items.category_id');
        $this->db->join('item', 'item.id =  quote_purchase_items.item_id');
    ...
    public function getListData()
    {
        ...
        ');
        $this->db->from($this->table);
        $this->db->join('item_supplier', 'item_supplier.id = quotes_purchase.supplier_id', 'left');
        
        if($search) {
            $this->db->group_start();
            $this->db->like('quotes_purchase.designation', $search);
            $this->db->or_like('quotes_purchase.quote_number', $search);
        ...
        if($status !== '' &&  (int)$status > 0 ) {
            $this->db->where('quotes_purchase.status', $status);
        }
        ...
        $this->db->order_by('quotes_purchase.created_at', 'DESC');
        if($length != -1) {
    ...
    public function update($data) {
        ...
        try {
            ...
            $this->db->update('quotes_purchase', $quote_data);

...
class Orders_purchase_model extends CI_Model {

    protected $table = 'ordere_purchase';
    protected $items_table = 'ordere_purchase_items';
    ...
    public function getQuoteWithItems($quote_id)
    {
        ...
        $this->db->select('ordere_purchase.*, item_supplier.item_supplier as customer_name,
        ...
        $this->db->join('item_supplier', 'item_supplier.id = ordere_purchase.supplier_id');
        $this->db->where('ordere_purchase.id', $quote_id);
        ...
        $this->db->select('
             ordere_purchase_items.*,
            item_category.item_category as category_name,
            item.name as item_name
        ');
        $this->db->from($this->items_table);
        $this->db->join('item_category', 'item_category.id =  ordere_purchase_items.category_id');
        $this->db->join('item', 'item.id =  ordere_purchase_items.item_id');
    ...
    public function getListData()
    {
        ...
        ');
        $this->db->from($this->table);
        $this->db->join('item_supplier', 'item_supplier.id = ordere_purchase.supplier_id', 'left');
        
        if($search) {
            $this->db->group_start();
            $this->db->like('ordere_purchase.designation', $search);
            $this->db->or_like('ordere_purchase.quote_number', $search);
        ...
        if($status !== '' &&  (int)$status > 0 ) {
            $this->db->where('ordere_purchase.status', $status);
        }
        ...
        $this->db->order_by('ordere_purchase.created_at', 'DESC');
        if($length != -1) {
    ...
    public function update($data) {
        ...
        try {
            ...
            $this->db->update('ordere_purchase', $quote_data);

            // Suppression des anciens articles
            $this->db->where('quote_id', $data['id']);
            $this->db->delete('ordere_purchase_items');
            ...
            foreach ($data['items'] as $position => $item) {
                ...
                $this->db->insert('ordere_purchase_items', $item_data);
            }

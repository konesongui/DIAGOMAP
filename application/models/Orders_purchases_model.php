...
class Orders_purchases_model extends CI_Model {

    protected $table = 'orders_purchases';
    protected $items_table = 'orders_purchases_items';
     ...
     * 
     * @param array $data Les données du bon
     * @return int|bool L'ID du bon créé ou false en cas d'erreur
     */
    ...
    {   
        ...
        try {
            ...
            if (!$quote_id) {
                throw new Exception('Erreur lors de la création du bon');
            }
     ...
     * 
     * @param int $quote_id ID de le bon
     * @return array|null Les données de le bon et ses articles
     */
    public function getQuoteWithItems($quote_id)
    {
        // Récupération de le devis
        $this->db->select('orders_purchases.*, item_supplier.item_supplier as customer_name,
            item_supplier.item_supplier as customer_last_name, item_supplier.email as customer_email, item_supplier.phone as customer_phone, item_supplier.address as customer_address, item_supplier.lastname');
        $this->db->from($this->table);
        $this->db->join('item_supplier', 'item_supplier.id = orders_purchases.supplier_id');
        $this->db->where('orders_purchases.id', $quote_id);
        ...
        $this->db->select('
             orders_purchases_items.*,
            item_category.item_category as category_name,
            item.name as item_name
        ');
        $this->db->from($this->items_table);
        $this->db->join('item_category', 'item_category.id =  orders_purchases_items.category_id');
        $this->db->join('item', 'item.id =  orders_purchases_items.item_id');
    ...
    public function getListData()
    {
        ...
        ');
        $this->db->from($this->table);
        $this->db->join('item_supplier', 'item_supplier.id = orders_purchases.supplier_id', 'left');
        
        if($search) {
            $this->db->group_start();
            $this->db->like('orders_purchases.designation', $search);
            $this->db->or_like('orders_purchases.quote_number', $search);
        ...
        if($status !== '' &&  (int)$status > 0 ) {
            $this->db->where('orders_purchases.status', $status);
        }
        ...
        $this->db->order_by('orders_purchases.created_at', 'DESC');
        if($length != -1) {
     ...
     * 
     * @param array $data Les données du bon à mettre à jour
     * @return bool
     */
    public function update($data) {
        ...
        try {
            ...
            if (!$quote || $quote['status'] != self::STATUS_PENDING) {
                throw new Exception('Le bon ne peut pas être modifié dans son état actuel');
            }
            ...
            $this->db->update('orders_purchases', $quote_data);

            // Suppression des anciens articles
            $this->db->where('quote_id', $data['id']);
            $this->db->delete('orders_purchases_items');
            ...
            foreach ($data['items'] as $position => $item) {
                ...
                $this->db->insert('orders_purchases_items', $item_data);
            }
    ...
    /**
     * Récupère les données du bon formatés pour l'impression
     * 
     * @param int $quote_id ID du bon
     * @return array|null Les données formatées du bon
     */
    ...
    /**
     * Récupère les informations de validation d'un bon
     * 
     * @param int $quote_id ID du bon
     * @return array|null
     */

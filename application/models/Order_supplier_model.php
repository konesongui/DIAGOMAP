...
class Order_supplier_model extends CI_Model {

    protected $table = 'orders_supplier';
    ...
    public function getListData()
    {
        ...
        ');
        $this->db->from($this->table);
        $this->db->join('item_supplier', 'item_supplier.id = orders_supplier.customer_id', 'left');
        
        if($search) {
            $this->db->group_start();
            $this->db->like('orders_supplier.designation', $search);
            $this->db->or_like('orders_supplier.order_number', $search);
        ...
        $this->db->order_by('orders_supplier.created_at', 'DESC');
        if($length != -1) {

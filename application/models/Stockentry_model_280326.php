...
class Stockentry_model extends MY_Model
{
    protected $table = 'stock_entries';
    protected $items_table = 'stock_entry_items';
    ...
    /**
     * Crée un modèle Excel avec PhpSpreadsheet
     */
    ...
    public function add($data)
    {
        ...
        try {
            ...
            $this->db->insert($this->table, $entry_data);
            ...
            log_message('error', 'Stockentry Model Error: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public function getListData()
    {
        ...
        $this->db->select('
            stock_entries.id,
            stock_entries.reference,
            stock_entries.designation,
            stock_entries.issue_date,
            stock_entries.grand_total,
        ...
        if($search) {
            $this->db->group_start();
            $this->db->like('stock_entries.designation', $search);
            $this->db->or_like('stock_entries.reference', $search);
            $this->db->group_end();
        }
        ...
        $this->db->order_by('stock_entries.created_at', 'DESC');
        if($length != -1) {
        ...
        foreach($query->result() as $row) {
            $data[] = [
                ...
                'actions' => "
                    <div class='btn-group'>
                        <a href='".base_url("admin/stockentry/view/".$row->id)."' 
     ...
     */
    public function getEntryWithItems($entry_id)
    {
        // Récupération de l'entrée
        $this->db->select('*');
        $this->db->from($this->table);
        $this->db->where('id', $entry_id);
        $entry = $this->db->get()->row_array();
        ...
        $this->db->select('
            ...
            stock_entries.reference as entry_reference
        ');
        $this->db->from($this->items_table);
        $this->db->join('item_category', 'item_category.id = stock_entry_items.category_id');
        $this->db->join('item', 'item.id = stock_entry_items.item_id');
        $this->db->join('stock_entries', 'stock_entries.id = stock_entry_items.stock_entry_id');
        $this->db->where('stock_entry_id', $entry_id);
    ...
    /**
     * Vérifie si une référence d'entrée existe déjà
     */
    public function checkReferenceExists($reference)
    {
    ...
    /**
     * Récupère un article par code ou nom
     */
    public function getItemByCodeOrName($code = null, $name = null)
    {
        $this->db->select('*');
        $this->db->from('item');

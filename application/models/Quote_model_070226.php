...
class Quote_model extends CI_Model {
    ...
    public function getQuoteWithItems($quote_id)
    {
        ...
        $this->db->select('quotes.*, clients.item_supplier as customer_name, user_name,
    ...
    public function getListData($current_user = null)
    {
    ...
    ');
        ...
        // 🔹 AJOUTER CE FILTRE POUR L'UTILISATEUR CONNECTÉ
        if ($current_user && $this->session->userdata('admin')['role_id'] != 1) {
            // Si l'utilisateur n'est pas administrateur, filtrer par son nom
            // Vous pouvez ajuster cette condition selon vos rôles
            $this->db->where('quotes.user_name', $current_user);
        }

        if($search) {
            $this->db->group_start();
            $this->db->like('quotes.designation', $search);
            $this->db->or_like('quotes.quote_number', $search);

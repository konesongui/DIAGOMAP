...
class Quoteitem extends Admin_Controller
{
    ...
    public function data()
    {
        ...
        // Ajouter le filtre par utilisateur dans la requête du modèle
        $result = $this->quote_model->getListData($current_user);
    ...
    public function add()
    {
        ...
        try {
            ...
            $quote_data = [
                ...
                'tax_option'        => $tax_option, // Nouveau champ
                'tva_rate'          => $tva_rate,
                'tva_amount'        => $tva_amount,
                'other_tax_name'    => $other_tax_name, // Nouveau champ
                'other_tax_rate'    => $other_tax_rate, // Nouveau champ
                'other_tax_amount'  => $tax_option === 'other' ? $tax_amount : 0, // Nouveau champ
                'total_ht'          => $totals['total_ht'],
                'total_discount'    => $totals['total_discount'],
                'total_after_discount' => $totals['total_after_discount'],
                'total_tax_amount'  => $tax_amount, // Nouveau champ pour le montant total de taxe
                'total_ttc'         => $total_ttc,
    ...
    private function duplicateQuote($originalQuote, $newReference)
    {
        $quoteData = [
            ...
            'apply_tva'             => $originalQuote['apply_tva'],
...
// ðÂÂ¸ Même formule que ton JS
    private function calculateTotalsWithBossFormula($quantities, $prices, $discounts, $discount_types)
    {
        ...
        // 🔸 Gestion des taxes selon l'option sélectionnée
        $tax_option = $this->input->post('tax_option');
        $tva_amount = 0;
        $other_tax_amount = 0;
        $total_tax_amount = 0;
...
// Les autres méthodes restent identiques
    ...
    public function view($id)
    {
        ...
        $current_user = $this->session->userdata('admin')['username'];
        if ($this->session->userdata('admin')['role_id'] != 1 && $data['quote']['user_name'] != $current_user) {
    ...
    public function delete($id)
    {
        ...
        try {
            ...
            // 🔹 VÉRIFICATION DE L'UTILISATEUR
            $current_user = $this->session->userdata('admin')['username'];
            if ($this->session->userdata('admin')['role_id'] != 1 && $quote['user_name'] != $current_user) {
                if ($this->input->is_ajax_request()) {
    ...
    public function edit_171025($id)
    {
        // Vérification des permissions
        if (!$this->rbac->hasPrivilege('devis', 'can_edit')) {
            access_denied();
    ...
    public function print()
    {
        ...
        // var_dump($userdata);
        // exit;
        // âÂÂÂÂ Génération du QR Code avec numéro + client

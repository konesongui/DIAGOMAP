...
class Quoteitem_supplier extends Admin_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('quote_supplier_model');
    ...
    function index()
    {
        ...
        $this->session->set_userdata('sub_menu', 'Quoteitem_supplier/index');
        ...
        $this->load->view('admin/quote_supplier/list', $data);
    ...
    public function data()
    {
        ...
        $result = $this->quote_supplier_model->getListData();
    ...
    public function form()
    {
        ...
        $data = [
            ...
            'suppliers' => $this->itemsupplier_model->get()
        ];

        // Chargement des vues
        $this->load->view('layout/header', $data);
        $this->load->view('admin/quote_supplier/form', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add()
    {
        ...
        try {
            ...
            $this->db->insert('quotes_supplier', $quote_data);
            ...
            if ($this->db->trans_status() === false) {
                throw new Exception('Erreur lors de l\'enregistrement du devis fournisseur');
            }

            $response['status'] = 'success';
            $response['message'] = 'Devis fournisseur créé avec succès';
            $response['quote_id'] = $quote_id;
            $response['redirect_url'] = base_url('admin/quoteitem_supplier');
...
// 🔸 Même formule que ton JS
    private function calculateTotalsWithBossFormula($quantities, $prices, $discounts, $discount_types)
    {
        ...
        foreach ($quantities as $i => $qty) {
            ...
            // 🔹 Calcul du montant de remise unitaire
        ...
        $tva_amount = $apply_tva ? $final_total_after_discount * ($tva_rate / 100) : 0;
        $total_ttc = $final_total_after_discount + $tva_amount;
...
// 🔸 Détail des lignes identique à JS
...
// Les autres méthodes restent identiques
    ...
    public function add_old081025()
    {
        ...
        try {
            ...
            $this->form_validation->set_rules('quantity[]', 'Quantité', 'required|numeric|greater_than[0]');
            $this->form_validation->set_rules('price[]', 'Prix unitaire', 'required|numeric|greater_than[0]');
            $this->form_validation->set_rules('discount[]', 'Remise', 'numeric|greater_than_equal_to[0]');
            $this->form_validation->set_rules('discount_type[]', 'Type de remise', 'required|in_list[percent,amount]');
            $this->form_validation->set_rules('global_discount_amount', 'Remise globale', 'numeric|greater_than_equal_to[0]');
            $this->form_validation->set_rules('global_discount_type', 'Type de remise globale', 'required|in_list[percent,amount]');
            ...
            // Vérification client existant ou nouveau
            $customer_id = $this->input->post('customer');
            if ($customer_id === "new") {
                $new_client_name  = trim($this->input->post('new_client_name'));
                $new_client_phone = trim($this->input->post('new_client_phone'));
                $new_client_email = trim($this->input->post('new_client_email'));
            ...
            $discounts  = $this->input->post('discount');
            $discount_types = $this->input->post('discount_type');
            $line_totals_after_discount = $this->input->post('line_total_after_discount');

            if (!is_array($categories) || !is_array($items) || !is_array($quantities) || !is_array($prices)) {
                throw new Exception('Format de données invalide');
            }

            // Calcul des totaux pour vérification côté serveur
            $total_ht = 0;
            $total_discount = 0;
            $total_after_discount = 0;

            foreach ($quantities as $index => $quantity) {
                $line_total_ht = $quantity * $prices[$index];
                $total_ht += $line_total_ht;

                // Calcul de la remise selon le type
                $discount_amount = 0;
                if ($discount_types[$index] === 'percent') {
                    $discount_amount = $line_total_ht * ($discounts[$index] / 100);
                } else {
                    $discount_amount = min($discounts[$index], $line_total_ht); // Ne pas dépasser le total HT
                }

                $total_discount += $discount_amount;
                $total_after_discount += ($line_total_ht - $discount_amount);
            }
            ...
            if ($global_discount_amount > 0) {
                ...
                $total_after_discount = max($total_after_discount - $global_discount, 0);
            }

            // Calcul de la TVA
            $apply_tva = $this->input->post('apply_tva') ? 1 : 0;
            $tva_rate = $this->input->post('tva_rate') ?: 18;
            $tva_amount = $apply_tva ? $total_after_discount * ($tva_rate / 100) : 0;
            $total_ttc = $total_after_discount + $tva_amount;
            ...
            foreach ($categories as $index => $category_name) {
                ...
                // Calcul des totaux pour l'article
                $line_total_ht = $quantities[$index] * $prices[$index];
                ...
                if ($discount_types[$index] === 'percent') {
                    ...
                    $discount_amount = min($discounts[$index], $line_total_ht);
                }

                $line_total_after_discount = $line_total_ht - $discount_amount;

                // Ligne devis avec remises
                $quote_items[] = [
                    'quote_id'      => $quote_id,
                    'category_id'   => $category_id,
                    'item_id'       => $item_id,
    ...
    public function add_041025()
    {
        ...
        try {
            ...
            $quote_data = [
                ...
                'total_remise'        => $this->input->post('total_remise'),
    ...
    private function generateQuoteNumber()
    {
        ...
        $this->db->limit(1);
        $query = $this->db->get('quotes_supplier');
    ...
    public function view($id)
    {
        ...
        $data['quote'] = $this->quote_supplier_model->getQuoteWithItems($id);
        ...
        if (!$data['quote']) {
            ...
            redirect('admin/quoteitem_supplier');
        }
        ...
        // Chargement des vues
        $this->load->view('layout/header', $data);
        $this->load->view('admin/quote_supplier/view', $data);
    ...
    public function sendEmail()
    {
        ...
        try {
            // Récupération des données du devis
            $data['quote'] = $this->quote_supplier_model->getQuoteWithItems($quote_id);
            if (!$data['quote']) {
           ...
           // $data['user'] = $this->customlib->getUserData();
            $data['user'] = $this->customlib->getUserData();
            $data['user_name'] = $this->customlib->getAdminSessionUserName(); // <= ajout
    ...
    public function delete($id)
    {
        ...
        try {
            ...
            $quote = $this->quote_supplier_model->getQuoteWithItems($id);
            if (!$quote) {
                if ($this->input->is_ajax_request()) {
            ...
            // Vérifier le statut (ex: uniquement si statut = 1 → actif/brouillon)
            ...
            // Supprimer le devis
            $this->db->where('id', $id)->delete('quotes_supplier');
            ...
            // Réponse pour requête normale (redirection)
            $this->session->set_flashdata('success', 'Devis fournisseur supprimé avec succès');
            redirect('admin/quoteitem_supplier');
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            // Chargement des vues
            $this->load->view('layout/header', $data);
            $this->load->view('admin/quote_supplier/edit', $data);
            $this->load->view('layout/footer', $data);
        } catch (Exception $e) {
    ...
    public function update()
    {
        ...
        try {
            ...
            $this->db->where('id', $quote_id)->update('quotes_supplier', $quote_data);
    ...
    public function update_091025()
    {
        ...
        try {
            ...
            foreach ($categories as $index => $category_name) {
                ...
                // Calcul du total ligne et de la remise
                $line_total_ht = $quantities[$index] * $prices[$index];
    ...
    public function update_old041025()
    {
        // Initialisation de la réponse
        $response = ['status' => 'fail', 'message' => '', 'error' => []];
    ...
    private function createDeliveryFromOrder($quote_id, $order_id)
    {
        ...
        $quote = $this->quote_supplier_model->getQuoteWithItems($quote_id);
        if (!$quote) {
            return false;
        }
        ...
        try {
            ...
            $this->db->insert('deliveries_supplier', $delivery_data);
    ...
    private function generateDeliveryNumber()
    {
        ...
        $this->db->order_by('id', 'DESC');
        $this->db->limit(1);
        $query = $this->db->get('deliveries_supplier');
    ...
    private function createOrderFromQuote($quote_id, $order_number)
    {
        ...
        try {
            ...
            $this->db->insert('orders_supplier', $order_data);
    ...
    public function validate()
    {
        ...
        try {
            ...
            if ($this->quote_supplier_model->isQuoteValidated($id)) {
                throw new Exception('Ce devis est déjà validé');
            }
            if ($this->quote_supplier_model->isQuoteRejected($id)) {
            ...
            if (!$this->quote_supplier_model->validateQuote($id, $data)) {
    ...
    public function reject()
    {
        ...
        try {
            ...
            if (!$this->quote_supplier_model->rejectQuote($id, $data)) {
    ...
    public function print()
    {
        ...
        $invoice_page = $this->load->view('admin/quote_supplier/print', $data, true);

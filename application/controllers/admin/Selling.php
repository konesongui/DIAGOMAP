...
class Selling extends Admin_Controller
{
    ...
    function index()
    {
        ...
        $this->load->view('admin/quote/selling', $data);
    ...
    /**
     * Ajoute un paiement partiel à une vente
     */
    public function add_payment()
    {
        ...
        // Validation
        if (!$quote_id) {
            echo json_encode(['status' => 'error', 'message' => 'ID de vente manquant']);
            return;
        }
        ...
        if (!$payment_method_type) {
            echo json_encode(['status' => 'error', 'message' => 'Mode de paiement requis']);
            return;
        }
        ...
        try {
            ...
            $reste_a_payer = floatval($quote['remaining_amount'] ?? 0);
            if ($amount > $reste_a_payer) {
                throw new Exception('Le montant dépasse le reste à payer');
            }
            ...
            $nouveau_paye = floatval($quote['amount_paid'] ?? 0) + $amount;
            $nouveau_reste = $reste_a_payer - $amount;
            $nouveau_statut = ($nouveau_reste <= 0) ? 'paid' : 'partial';

            // Mettre à jour la vente
            $update_data = [
                'amount_paid' => $nouveau_paye,
                'remaining_amount' => max($nouveau_reste, 0),
                'payment_status' => $nouveau_statut,
                'updated_at' => date('Y-m-d H:i:s')
            ];

            if ($nouveau_reste <= 0) {
                $update_data['change_amount'] = abs($nouveau_reste);
                $update_data['amount_paid'] = floatval($quote['total_ttc']);
            }

            $this->db->where('id', $quote_id)->update('quotes_selling', $update_data);

            // Enregistrer le paiement dans la caisse ou banque
            $payment_reference = 'PAY-' . date('YmdHis') . '-' . $quote_id;

            if ($payment_method_type == 'cash') {
                // Paiement en espèces
                if (!$source_id) {
                    throw new Exception('Caisse non spécifiée');
                }
                ...
                $this->db->insert('operation_caisse', [
                    ...
                    'montant' => $amount,
                    'designation' => 'Paiement partiel - Vente #' . $quote['quote_number'],
                    'caisse_id' => $source_id,
                    'date' => date('Y-m-d H:i:s'),
                    'entree' => $amount,
                    'sortie' => 0,
                    'note' => 'Paiement partiel vente',
                    'est_actif' => 1,
                ...
                $this->db->insert('mouvements', [
                    ...
                    'montant' => $amount,
                    'description' => 'Paiement partiel vente #' . $quote['quote_number'],
                    'reference' => $payment_reference,
                ...
                // Paiement bancaire
                if (!$source_id) {
                    throw new Exception('Banque non spécifiée');
                }
                ...
                $this->db->insert('bank', [
                    ...
                    'designation' => 'Crédit',
                    'name' => 'Paiement partiel vente #' . $quote['quote_number'],
            ...
            log_message('error', 'Add Payment Error: ' . $e->getMessage());
        }
    }

    /**
     * Récupère la liste des caisses et banques pour le paiement
     */

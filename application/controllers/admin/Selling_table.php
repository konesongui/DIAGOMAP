...
class Selling_table extends Admin_Controller {
    ...
    function index() {
        ...
        if (!$this->rbac->hasPrivilege('panneau', 'can_view')) {
            access_denied();
        }

        $this->session->set_userdata('top_menu', 'Inventory');
        $this->session->set_userdata('sub_menu', 'admin/Selling_table');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('admin/selling_table/selling_tableList', $data);
            ...
            redirect('admin/selling_table/index');
        }
    }
    ...
    public function edit($id) {
        if (!$this->rbac->hasPrivilege('issue_item', 'can_edit')) {
            access_denied();
        }
    ...
    public function selling_table($id) {
        if (!$this->rbac->hasPrivilege('categorie_salaire', 'can_view')) {
            access_denied();
        }

        $results              = $this->Selling_model->getbilan();
        $data['bill'] = $results;
        $this->load->view('layout/header');
        $this->load->view('admin/selling_table/selling_tableList', $data);
        $this->load->view('layout/footer');

...
class VentesMensuelles extends Admin_Controller {
    ...
    public function index_() {
        ...
        $this->session->set_userdata('sub_menu', 'commande/index');
    ...
    public function ventesMensuelles()
    {
        ...
        SELECT 
            ...
            user_name,
            SUM(amount_paid) AS total_ventes
        FROM invoices
        GROUP BY mois, user_name
        ORDER BY mois ASC
    ");

        $result = $query->getResult();

        $objectif_mensuel = 1000000; // Objectif : 1 million
        ...
        foreach ($result as $row) {
            $mois = $row->mois;
            $nom = $row->nom_commercial;
            $montant = (float) $row->total_ventes;

            $data[$nom][$mois] = $montant;
        ...
        $datasets = [];
        foreach ($data as $commercial => $ventes) {
            $ventesParMois = [];
            foreach ($moisLabels as $mois) {
                $ventesParMois[] = $ventes[$mois] ?? 0;
            }
            ...
            $datasets[] = [
                ...
                'data' => $ventesParMois,
                'fill' => false,
                'borderColor' => '#' . substr(md5($commercial), 0, 6),
                'tension' => 0.2
            ];
        }

        // Ligne d’objectif (même valeur chaque mois)
        $objectifData = array_fill(0, count($moisLabels), $objectif_mensuel);
        $datasets[] = [
            'label' => '🎯 Objectif',
            'data' => $objectifData,
            'borderColor' => 'red',
        ...
        return view('Selling_table/selling_tableList', [
            'chartData' => [
                'labels' => $moisLabels,
                'datasets' => $datasets
            ],
    ...
    public function create() {
        $this->session->set_userdata('top_menu', 'Commerciales');
        $this->session->set_userdata('sub_menu', 'commande/index');
        $data['title'] = 'Add Issue item';
        $data['title_list'] = 'Recent Issue items';
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/commande/commandeCreate', $data);
        $this->load->view('layout/footer', $data);
    }
     ...
     public function getitemlist()
    {
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('vente', 'can_delete')) {
                    $deletebtn = '';

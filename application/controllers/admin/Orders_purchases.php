...
class Orders_purchases extends Admin_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('orders_purchases_model');
    ...
    function index()
    {
        ...
        $this->session->set_userdata('sub_menu', 'Orders_purchases/index');
        ...
        $this->load->view('admin/orders_purchases/list', $data);
    ...
    public function data()
    {
        ...
        $result = $this->orders_purchases_model->getListData();
    ...
    public function form()
    {
        ...
        $data = [
            'title' => 'Ajouter un article au bon',
            'title_list' => 'Derniers articles ajoutés au bon',
        ...
        // Chargement des vues
        $this->load->view('layout/header', $data);
        $this->load->view('admin/orders_purchases/form', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add()
    {
        ...
        try {
            ...
            // Insertion du devis
            $this->db->insert('orders_purchases', $quote_data);
            ...
            if (!empty($quote_items)) {
                $this->db->insert_batch('orders_purchases_items', $quote_items);
            }
            ...
            if ($this->db->trans_status() === false) {
                throw new Exception('Erreur lors de l\'enregistrement du bon');
            }

            $response['status'] = 'success';
            $response['message'] = 'Bon créé avec succès';
    ...
    private function generateQuoteNumber()
    {
        ...
        $this->db->limit(1);
        $query = $this->db->get('orders_purchases');
    ...
    public function view($id)
    {
        ...
        $data['quote'] = $this->orders_purchases_model->getQuoteWithItems($id);
        ...
        if (!$data['quote']) {
            $this->session->set_flashdata('error', 'Bon non trouvé');
            redirect('admin/orders_purchases');
        }

        // Préparation des données pour la vue
        $data['title'] = 'Détails du bon';
        $data['page_title'] = 'Détails de bon de commande ' . $data['quote']['quote_number'];

        // Chargement des vues
        $this->load->view('layout/header', $data);
        $this->load->view('admin/orders_purchases/view', $data);
        $this->load->view('layout/footer');
    }


    /**
     * Envoie le bon par email au client
     * 
     * @param int $quote_id ID du bon
     * @return void
     */
    public function sendEmail()
    {
        ...
        try {
            // Récupération des données du devis
            $data['quote'] = $this->orders_purchases_model->getQuoteWithItems($quote_id);
            if (!$data['quote']) {
                throw new Exception('Bon introuvable');
            }
            ...
            if ($data['quote']) {
                ...
                $this->mailsmsconf->mailsms('send_quote_no_stock', $quote_detail);
            }
            
            $response['status'] = 'success';
            $response['message'] = 'Le bon de commande fournisseur a été envoyé avec succès';
    ...
    /**
     * Affiche le formulaire d'édition d'un bon
     * 
     * @param int $id ID du bon
     * @return void
     */
    public function edit($id)
    {
        ...
        try {
            ...
            $quote = $this->orders_purchases_model->getQuoteWithItems($id);
            if (!$quote) {
                $this->session->set_flashdata('error', 'Bon introuvable');
                redirect('admin/orders_purchases');
            }

            if ((int)$quote['status'] !== 1) {
                $this->session->set_flashdata('error', 'Ce Bon ne peut plus être modifié');
                redirect('admin/orders_purchases');
            }
            ...
            // Chargement des vues
            $this->load->view('layout/header', $data);
            $this->load->view('admin/orders_purchases/edit', $data);
            $this->load->view('layout/footer', $data);
        } catch (Exception $e) {
            log_message('error', 'Quote Edit Error: ' . $e->getMessage());
            $this->session->set_flashdata('error', 'Une erreur est survenue lors de l\'édition du bon');
            redirect('admin/orders_purchases');
        }
    }
    ...
    public function update()
    {
        ...
        try {
            ...
            $this->db->where('id', $quote_id)->update('orders_purchases', $quote_data);

            // Suppression des anciens articles
            $this->db->where('quote_id', $quote_id)->delete('orders_purchases_items');
            ...
            if ($this->db->trans_status() === false) {
                throw new Exception('Erreur lors de la mise à jour du bon');
            }

            $response['status'] = 'success';
            $response['message'] = 'Bon mis à jour avec succès';
            $response['quote_id'] = $quote_id;
    ...
    /**
     * Crée un bon de commande à partir d'un bon validé
     * 
     * @param int $quote_id ID du bon
     * @return bool
     */
    private function createDeliveryFromOrder($quote_id, $order_id)
    {
        ...
        $quote = $this->orders_purchases_model->getQuoteWithItems($quote_id);
        if (!$quote) {
            return false;
        }
        ...
        try {
            ...
            $this->db->insert('deliveries_purchases', $delivery_data);
            ...
            foreach ($quote['items'] as $item) {
                ...
                if (!$this->db->insert('delivery_purchases_items', $delivery_item_data)) {
     ...
     * @return bool
     */
    private function createOrderFromQuote($quote_id, $quote_number)
    {
        // Récupération des données du devis
        $quote = $this->orders_purchases_model->getQuoteWithItems($quote_id);
        if (!$quote) {
        ...
        $quote_data = [
            ...
            'total_ttc'     => $quote['total_ttc'],
            'notes'         => $quote['notes'],
            'status'        => Orders_purchases_model::STATUS_IN_PROGRESS, // 1 = En attente
            'created_at'    => date('Y-m-d H:i:s'),
        ];
        ...
        try {
            ...
            foreach ($quote['items'] as $item) {
                ...
                if (!$this->db->insert('orders_purchases_items', $quote_item_data)) {
    ...
    public function validate()
    {
        ...
        try {
            ...
            $quote = $this->orders_purchases_model->getQuoteWithItems($id);
            if (!$quote) {
                throw new Exception('Bon non trouvé');
            }

            // Vérification que le devis n'est pas déjà validé ou rejeté
            if ($this->orders_purchases_model->isQuoteValidated($id)) {
                throw new Exception('Ce bon est déjà validé');
            }
            if ($this->orders_purchases_model->isQuoteRejected($id)) {
                throw new Exception('Ce bon a été rejeté');
            }

            // Validation du devis
            $data = [
                'status' => Orders_purchase_model::STATUS_VALIDATED,
                'validated_at' => date('Y-m-d H:i:s'),
            ];

            if (!$this->orders_purchases_model->validateQuote($id, $data)) {
                throw new Exception('Erreur lors de la validation du bon');
            }

            $response['status'] = 'success';
            $response['message'] = 'Bon validé avec succès';

            // Créer la commande à partir du devis
            $this->createOrderFromQuote($id, $quote_number);
    ...
    /**
     * Crée une sortie de stock à partir d'un bon validé
     * 
     * @param int $quote_id ID du bon
     * @return void
     */
    private function createStockRemovalFromQuote($quote_id)
    {
        ...
        // exit;
        if (!$quote) {
            throw new Exception('Bon non trouvé pour la sortie de stock');
        }
    ...
    public function completeDelivery() {
        ...
        try {
            ...
            $delivery = $this->delivery_supplier_model->getDeliveryWithItems($id);
    ...
    public function reject()
    {
        ...
        try {
            ...
            if ($this->orders_purchases_model->isQuoteRejected($id)) {
                throw new Exception('Ce bon a déjà été rejeté');
            }
            ...
            // Rejet du devis
            $data = [
                'status'        => Orders_purchases_model::STATUS_REJECTED,
            ...
            if (!$this->orders_purchases_model->rejectQuote($id, $data)) {
                throw new Exception('Erreur lors du rejet du bon');
            }

            $response['status'] = 'success';
            $response['message'] = 'Bon de commande rejeté avec succès';
        } catch (Exception $e) {
            $response['message'] = $e->getMessage();
            log_message('error', 'BON Rejection Error: ' . $e->getMessage());
        }
    ...
    public function print()
    {
        ...
        $invoice_page = $this->load->view('admin/orders_purchases/print', $data, true);
    ...
    /**
     * Génère un PDF du bon avec mPDF
     * 
     * @param int $id ID du bon
     * @return void
     */

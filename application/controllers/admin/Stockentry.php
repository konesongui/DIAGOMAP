...
class Stockentry extends Admin_Controller {
    ...
    /**
     * Récupère les articles par nom de catégorie
     */
    ...
    /**
     * Récupère les articles par catégorie (pour AJAX)
     */
    public function get_items_by_category() {
        ...
        if (!$category_id) {
            echo json_encode(['status' => 'error', 'message' => 'Catégorie non spécifiée']);
            return;
        }

        // Récupérer les articles de la catégorie
        $this->db->select('id, name, unit');
        $this->db->from('item');
    ...
    /**
     * Récupère le stock disponible d'un article (pour AJAX)
     */
    public function get_item_stock() {
        ...
        if (!$item_id) {
            echo json_encode(['status' => 'error', 'message' => 'Article non spécifié']);
            return;
        }
    ...
    public function get_items_by_category_name() {
        ...
        $this->db->select('item.id, item.name, item.unit, item.unit_price');
        $this->db->from('item');
    ...
    public function add() {
        ...
        try {
            ...
            $data = [
                ...
                'issue_date' => date('Y-m-d', strtotime(str_replace('/', '-', $this->input->post('issue_date')))),
                'grand_total' => $this->input->post('grandtotal'),
                'grand_profit' => $this->input->post('grandprofit'), // AJOUTÉ - Bénéfice total
                'items' => []
            ];

            $categories = $this->input->post('item_category_name');
            $items = $this->input->post('item_name');
            $quantities = $this->input->post('quantity');
            $prices = $this->input->post('price'); // Prix de vente (unit_price)
            $purchase_prices = $this->input->post('purchase_price'); // AJOUTÉ - Prix d'achat
            $units = $this->input->post('unit');
            ...
            foreach ($categories as $index => $category_name) {
                ...
                // Récupérer les prix
                $selling_price = isset($prices[$index]) ? floatval($prices[$index]) : 0; // Prix de vente
                $purchase_price = isset($purchase_prices[$index]) ? floatval($purchase_prices[$index]) : 0; // AJOUTÉ - Prix d'achat

                // Créer ou récupérer la catégorie
                $category_id = $this->getOrCreateCategory($category_name);

                // Créer ou récupérer l'article avec les prix
                $item_id = $this->getOrCreateItem($items[$index], $category_id, $units[$index], $purchase_price, $selling_price);

                // Calcul du bénéfice
                $profit_per_unit = $selling_price - $purchase_price;
                $total_profit = $profit_per_unit * $quantities[$index];

                $data['items'][] = [
                    'category_id' => $category_id,
                    'item_id' => $item_id,
                    'quantity' => $quantities[$index],
                    'price' => $selling_price, // price = prix de vente
                    'unit_price' => $selling_price, // unit_price = prix de vente
                    'purchase_price' => $purchase_price, // AJOUTÉ - Prix d'achat
                    'unit' => $units[$index] ?? '',
                    'line_total' => $quantities[$index] * $purchase_price, // MODIFIÉ - Total basé sur prix d'achat
    ...
    public function add8() {
        ...
        try {
            ...
            $items = $this->input->post('item_name');
            $quantities = $this->input->post('quantity');
            $prices = $this->input->post('price'); // On garde price du formulaire
            $units = $this->input->post('unit');
            ...
            foreach ($categories as $index => $category_name) {
                ...
                // Créer ou récupérer l'article avec le prix
                ...
                $data['items'][] = [
                    ...
                    'item_id' => $item_id,
                    'quantity' => $quantities[$index],
                    'price' => $price, // price devient unit_price
                    'unit_price' => $price, // price devient unit_price
                    'unit' => $units[$index] ?? '',
                    'line_total' => $quantities[$index] * $price
                ];
            }
    ...
    public function add_() {
        ...
        try {
            ...
            foreach ($categories as $index => $category_name) {
                ...
                $item_id = $this->getOrCreateItem($items[$index], $category_id, $units[$index]);
    ...
    private function getOrCreateItem($item_name, $category_id, $unit, $purchase_price = 0, $selling_price = 0) {
        $this->db->where('name', $item_name);
        $this->db->where('item_category_id', $category_id);
        $item = $this->db->get('item')->row();

        if ($item) {
            // Mettre à jour les prix si différents et si fournis
            $update_data = [];

            if ($purchase_price > 0 && $item->purchase_price != $purchase_price) {
                $update_data['purchase_price'] = $purchase_price;
            }

            if ($selling_price > 0 && $item->unit_price != $selling_price) {
                $update_data['unit_price'] = $selling_price;
            }
    ...
    private function getOrCreateItem8($item_name, $category_id, $unit, $price = 0) {
        $this->db->where('name', $item_name);
        $this->db->where('item_category_id', $category_id);
        $item = $this->db->get('item')->row();

        if ($item) {
            // Mettre à jour le prix unitaire si différent et si fourni
            if ($price > 0 && $item->unit_price != $price) {
                $this->db->where('id', $item->id);
                $this->db->update('item', ['unit_price' => $price]);
            }
            return $item->id;
        }
        ...
        $this->db->insert('item', [
            ...
            'item_category_id' => $category_id,
            'unit' => $unit ?: 'Pièce',
            'unit_price' => $price, // Le champ price du formulaire va dans unit_price de la table
            'created_at' => date('Y-m-d H:i:s')
        ]);
    ...
    private function getOrCreateItem_($item_name, $category_id, $unit, $price = 0) {
        $this->db->where('name', $item_name);
        $this->db->where('item_category_id', $category_id);
        $item = $this->db->get('item')->row();

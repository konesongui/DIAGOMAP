...
class Orders_purchase extends Admin_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('orders_purchase_model');
    ...
    function index()
    {
        ...
        $this->session->set_userdata('sub_menu', 'Orders_purchase/index');
        ...
        $this->load->view('admin/orders_purchase/list', $data);
    ...
    public function data()
    {
        ...
        $result = $this->orders_purchase_model->getListData();
    ...
    public function form()
    {
        ...
        // Chargement des vues
        $this->load->view('layout/header', $data);
        $this->load->view('admin/orders_purchase/form', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add()
    {
        ...
        try {
            ...
            // Insertion du devis
            $this->db->insert('ordere_purchase', $quote_data);
            ...
            if (!empty($quote_items)) {
                $this->db->insert_batch('ordere_purchase_items', $quote_items);
            }
    ...
    private function generateQuoteNumber()
    {
        ...
        $this->db->limit(1);
        $query = $this->db->get('ordere_purchase');
    ...
    public function view($id)
    {
        ...
        $data['quote'] = $this->orders_purchase_model->getQuoteWithItems($id);
        ...
        if (!$data['quote']) {
            ...
            redirect('admin/orders_purchase');
        }
        ...
        // Chargement des vues
        $this->load->view('layout/header', $data);
        $this->load->view('admin/orders_purchase/view', $data);
    ...
    public function sendEmail()
    {
        ...
        try {
            // Récupération des données du devis
            $data['quote'] = $this->orders_purchase_model->getQuoteWithItems($quote_id);
            if (!$data['quote']) {
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            $quote = $this->orders_purchase_model->getQuoteWithItems($id);
            if (!$quote) {
            ...
            // Chargement des vues
            $this->load->view('layout/header', $data);
            $this->load->view('admin/orders_purchase/edit', $data);
            $this->load->view('layout/footer', $data);
        } catch (Exception $e) {
    ...
    public function update()
    {
        ...
        try {
            ...
            $this->db->where('quote_id', $quote_id)->delete('ordere_purchase_items');
    ...
    private function createDeliveryFromOrder($quote_id, $order_id)
    {
        ...
        $quote = $this->orders_purchase_model->getQuoteWithItems($quote_id);
        if (!$quote) {
            return false;
        }
    ...
    private function createOrderFromQuote($quote_id, $order_number)
    {
        ...
        try {
            ...
            foreach ($quote['items'] as $item) {
                ...
                if (!$this->db->insert('orders_purchase_items', $order_item_data)) {
    ...
    public function validate()
    {
        ...
        try {
            ...
            if ($this->orders_purchase_model->isQuoteValidated($id)) {
                throw new Exception('Ce devis est déjà validé');
            }
            if ($this->orders_purchase_model->isQuoteRejected($id)) {
    ...
    public function reject()
    {
        ...
        try {
            ...
            if (!$this->orders_purchase_model->rejectQuote($id, $data)) {

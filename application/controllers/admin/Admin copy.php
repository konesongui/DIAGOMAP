...
class Admin extends Admin_Controller
{
    ...
    public function dashboard()
    {
        ...
        // Essayer de récupérer les settings de l'entreprise connectée
        if ($entreprise_id > 0) {
            $this->db->select('*');
            $this->db->from('sch_settings');
            $this->db->where('entreprise_id', $entreprise_id);
            ...
            if ($settings_query->num_rows() > 0) {
                ...
                // Fallback : récupérer le premier sch_settings
                $this->db->select('*');
                $this->db->from('sch_settings');
                $this->db->order_by('id');
                $this->db->limit(1);
                ...
                // Si toujours rien, créer un objet par défaut
            ...
            // Super admin : récupérer le premier sch_settings
            $this->db->select('*');
            $this->db->from('sch_settings');
            $this->db->order_by('id');
            $this->db->limit(1);
...
// Méthode pour configurer le cron automatique
    private function setupAutoBackupCron($day) {
        $cron_command = "0 2 * * " . $day . " curl -s " . base_url() . "admin/admin/autoBackup/" . $this->setting_model->getSetting()->cron_secret_key . " > /dev/null 2>&1";

        // Sauvegarder la commande cron dans un fichier pour référence
        $cron_info = array(
            'command' => $cron_command,
            'description' => 'Sauvegarde automatique Smart School',
            'day' => $day,
...
// Méthode pour la sauvegarde automatique via cron
    public function autoBackup($secret_key = null) {
        ...
        try {
            ...
            $db_filename = 'database_' . date('Y-m-d_H-i-s') . '.zip';
            $db_filepath = './backup/' . $db_filename;

            write_file($db_filepath, $backup);

            // Sauvegarde des fichiers importants
            $this->backupImportantFiles();

            log_message('info', 'Sauvegarde automatique effectuée avec succès: ' . $db_filename);

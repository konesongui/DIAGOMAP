...
class Income extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            // 🔹 1. Sauvegarde de l’entrée dans income
            $data = [
            ...
            // 🔹 1.bis Sauvegarde dans mouvements
            $mouvement_data = [
                'type_mouvement' => 'entree',
                'montant'        => $this->input->post('amount'),
                'description'    => $this->input->post('name'),
                'reference'      => 'RECU-' . $insert_id,
                'id_employe'     => null, // pas utilisé pour une entrée simple
                'date_mouvement' => $data['date'],
                'mode_paiement'  => $this->input->post('mode_paiement'), // à récupérer depuis le formulaire
                'created_at'     => date('Y-m-d H:i:s'),
                'updated_at'     => date('Y-m-d H:i:s'),
            ];
            $this->db->insert('mouvements', $mouvement_data);

            // 🔹 2. Sauvegarde dans journal (optionnel, comme tu avais déjà)
            $journal_data = [
                'type_mouvement' => 'entree',
                'date_operation'   => $data['date'],
                'reference_piece'  => 'RECU-' . $insert_id,
                'libelle_operation'=> 'caisse',
                'compte_id_revenu' => 571, // exemple Caisse
                'compte_id'        => 571,
            ...
            // 🔹 3. Insertion des écritures comptables dans accounting_entries
            $amount = $this->input->post('amount');
            $date   = $data['date'];
            $desc   = "Encaissement revenu : " . $this->input->post('name');

            // Débit Caisse (compte 571)
            ...
            // Crédit Revenu (compte de produit, ex: 701 Ventes ou 706 Prestations)
            $this->db->insert('accounting_entries', [
                'date'        => $date,
                'invoice_id'  => $insert_id,
                'account'     => '701 - Produits',
                'debit'       => 0,
                'credit'      => $amount,
                'description' => $desc,
                'created_at'  => date('Y-m-d H:i:s')
    ...
    public function index_100925()
    {
        ...
        $compte_comptable          = $this->comptecomptable_model->get();
        $data['compte'] = $compte_comptable;
        $this->load->view('layout/header', $data);
        $this->load->view('admin/income/incomeList', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            // ⚡ Auto ajout du montant dans amount_re
            //$amount_re = $amount_re + $amount;
            ...
            $data = array(
                ...
                'date'           => date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date'))),
                'amount'         => $amount,
                'amount_re'      => $amount_re, // ➝ déjà la bonne valeur
                'invoice_no'     => $this->input->post('invoice_no'),
                'note'           => $this->input->post('description'),
                'est_actif'      => $this->input->post('est_actif') ? 1 : 0,
    ...
    public function edit_old030925($id)
    {
        if (!$this->rbac->hasPrivilege('caisse', 'can_edit')) {
            access_denied();
        }
    ...
    public function getincomelist_old()
    {
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                $editbtn     = '';
                $deletebtn   = '';
                $toggle_status_btn = '<button type="button" class="btn btn-xs toggle-status" data-id="'.$value->id.'" data-status="'.$value->est_actif.'" style="background-color:#03A9F4;color:white;" title="Activer/Désactiver">
                    <i class="fa '.($value->est_actif == "1" ? 'fa-lock' : 'fa-unlock').'"></i>
                </button>';
               ...
               // $row[]     = $value->income_category;
                ...
                if ($value->amount_re <= 50000) {
                    $message = "<span style='color:red; font-weight:bold;'> (Solde critique)</span>";
                } else {
                    $message = "";
                }

                $row[] = number_format($value->amount_re, 0, ',', ' ') . " " . $currency_symbol . $message;
                ...
                if ($value->est_actif=="1") {
                    ...
                    $row[]     = "<h6><span class='label label-warning' style='background-color: #e91e63 !important; border-radius: 2px'>Fermée<span/></h6>";
                }
    ...
    public function setIncrease_100925()
    {
        if (!$this->rbac->hasPrivilege('caisse', 'can_add')) {
            access_denied();
        }
    ...
    public function setIncrease()
    {
        ...
        // Récupération de la ligne Income
        $oldRow = $this->db->select('*')
            ->from('income')
            ->where(['id' => $rowId])
            ->get()
        ...
        if ($rowUpdated) {
            // Enregistrement dans income_processing
            $this->Income_processing_model->createP([
                'income_id' => $rowId,
                'amount'    => $amount,
                'reason'    => $reason,
            ...
            // Crédit Banque (512) – si ton réappro vient toujours de la banque
            $this->db->insert('accounting_entries', [
                'date'        => $date_op,
                'invoice_id'  => $rowId,
                'account'     => '512 - Banque',

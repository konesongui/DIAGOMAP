...
class Payroll extends Admin_Controller
{
    ...
    public function paymentSuccess()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array('payment_mode' => $payment_mode,'date_from' => $date_from,'date_to' => $date_to, 'payment_date' => $payment_date, 'remark' => $remark, 'status' => $status);
...
// Exemple de contrôleur
    ...
    public function sendPayslipEmail()
    {
        ...
        try {
            ...
            // Récupération des informations du staff
            ...
            // Vérification de l'email du staff
            ...
            log_message('debug', '🔧 User email: ' . $data['user_email']);

            log_message('debug', '🔄 Vérification du template send_payslip...');

            // ⭐⭐ VERSION SIMPLIFIÉE : Utiliser directement send_quote ⭐⭐
            $test_detail = array(
                'id' => $payslip['id'],
                'data' => [
                    'quote' => [ // Simuler un devis
            ...
            log_message('debug', '🔄 Appel de send_payslip...');

            // Appeler send_quote qui fonctionne
            $this->mailsmsconf->mailsms('send_quote_no_stock', $test_detail);

            $response['status'] = 'success';
            $response['message'] = 'Bulletin de paie envoyé avec succès';

...
class Invoiceitem_supplier extends Admin_Controller
{
    ...
    public function form()
    {
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/invoice_supplier/form', $data);
    ...
    public function add()
    {
        ...
        try {
            ...
            $data = [
                ...
                'invoice_number' => $this->invoice_supplier_model->generateInvoiceNumber(),
            ...
            $insert_id = $this->invoice_supplier_model->add($data);
    ...
    public function view($id)
    {
        ...
        $data['invoice'] = $this->invoice_supplier_model->getInvoiceWithItems($id);
        if (!$data['invoice']) {
            $this->session->set_flashdata('error', 'Facture non trouvée');
            redirect('admin/invoiceitem_supplier');
        }

        $data['payments']  = $this->invoice_supplier_model->getPayments($id);
        $data['title']     = 'Détails de la facture';
        $data['page_title'] = 'Facture '.$data['invoice']['invoice_number'];

        $this->load->view('layout/header', $data);
        $this->load->view('admin/invoice_supplier/view', $data);
        $this->load->view('layout/footer');
    }
    ...
    public function edit($id)
    {
        ...
        try {
            $invoice = $this->invoice_supplier_model->getInvoiceWithItems($id);
            if (!$invoice) {
                $this->session->set_flashdata('error', 'Facture introuvable');
                redirect('admin/invoiceitem');
            }
            if ($this->invoice_supplier_model->isPaid($id)) {
            ...
            $this->load->view('layout/header', $data);
            $this->load->view('admin/invoice_supplier/edit', $data);
            $this->load->view('layout/footer');
        } catch (Exception $e) {
    ...
    public function cancel()
    {
        ...
        try {
            ...
            if (!$this->invoice_supplier_model->cancel($id, $data)) {
    ...
    public function setPayment()
    {
        ...
        try {
            ...
            if (empty($reference)) {
                $reference = 'PAY-SUP-' . date('YmdHis') . '-' . $invoice_id;
            }
            ...
            if ($source_type == 'caisse') {
                ...
                if ($new_balance < 0) {
                    throw new Exception('Solde insuffisant en caisse');
                }
                ...
                $operation_caisse_data = [
                    ...
                    'montant'             => $amount,
                    'designation'         => 'Paiement fournisseur #' . $invoice['invoice_number'],
                    'caisse_id'           => $source_id,
                ...
                $this->db->insert('mouvements', [
                    ...
                    'montant' => $amount,
                    'description' => 'Paiement fournisseur #' . $invoice['invoice_number'],
                    'reference' => $reference,
            ...
            // --- 3. BANQUE ---
                ...
                if ($new_balance < 0) {
                    throw new Exception('Solde insuffisant en banque');
                }
                ...
                $this->db->insert('bank', [
                    ...
                    'designation' => 'Débit',
                    'name' => 'Paiement fournisseur #' . $invoice['invoice_number'],
                    'nom' => $invoice['supplier_name'] ?? 'Fournisseur',
                    'amount' => $amount,
                    'reference' => $reference,
                    'payment_mode' => $method,
                    'note' => $notes,
            ...
            $this->db->trans_rollback();
            $response['message'] = '❌ Erreur: ' . $e->getMessage();
            log_message('error', 'Payment Supplier Error: ' . $e->getMessage());
        }
    ...
    public function setPayment_16()
    {
        ...
        try {
            ...
            // ðÂÂ¹ Enregistrement dans mouvements
            $mouvement_data = [
                'type_mouvement' => 'entree',
                'montant'        => $amount,
                // 'description'    => 'Paiement facture #' . $invoice['invoice_number'] . ' - ' . $notes,
    ...
    public function setPayment090925()
    {
        if (!$this->rbac->hasPrivilege('facture', 'can_edit')) {
            access_denied();
        }
        ...
        try {
            ...
            // --- Écriture comptable dans le journal ---
            $journal_entry = [
                'date_operation'   => $payment_date,
                'reference_piece'  => 'RECU-'.$invoice_id,
                'libelle_operation'=> 'Paiement facture #'.$invoice['invoice_number'],
                'compte_id_revenu' => 701, // compte de vente
                'compte_id'        => 511, // compte bancaire ou caisse
                'montant_revenu'   => $amount,
                'montant'          => $amount,
            ];
            $this->journal_model->save($journal_entry);

            // --- Écriture comptable dans accounting_entries ---
            $entries = [
                [
                    'date'        => $payment_date,
                    'invoice_id'  => $invoice_id,
            ...
            $this->db->insert_batch('accounting_entries', $entries);

            $response['status']  = 'success';
            $response['message'] = 'Le paiement a été enregistré avec succès (journal + comptabilité).';
        } catch (Exception $e) {
            $response['message'] = 'Erreur: '.$e->getMessage();
            log_message('error', 'Payment Add Error: '.$e->getMessage());
        }
    ...
    public function setPayment_old080925()
    {
        if (!$this->rbac->hasPrivilege('facture', 'can_edit')) {
            access_denied();
        }
    ...
    public function print()
    {
        ...
        $page = $this->load->view('admin/invoice_supplier/print', $data, true);
    ...
    public function addPaymentForm()
    {
        ...
        $this->load->view('admin/invoice_supplier/paymentForm', $data);
    }

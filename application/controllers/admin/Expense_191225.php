...
// =======================
                $amount = $this->input->post('amount');
                $date_op = date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date')));
                $desc = $this->input->post('description') ?? "Dépense caisse";
                $invoice_no = $this->input->post('invoice_no') ?? null;

// Débit : Compte de charge (606 Achats par défaut, ou selon exp_head_id)
...
// Crédit : Compte de caisse (571)
                ...
                $data['expenselist'] = $this->expense_model->getexpenselist();
                ...
                // 🔹 4. Enregistrement dans mouvements
                $mouvement_data = [
        ...
        $data = [
            ...
            'libelle_operation'  => 'Achats',
            'category' => $this->input->post('exp_category_name'),
            'compte_id_depense'    => 606,
            'compte_id'    => 606,
            'montant_depense'      => $this->input->post('amount'),
    ...
    public function bank()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            // Prepare expense data (exp_head_id can be empty or null)
            $data = [
                'exp_head_id' => $this->input->post('exp_head_id') ?: null,
                'category' => $this->input->post('exp_category_name') ?: null,
                'name'        => $this->input->post('name'),
                'date'        => date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date'))),
                'amount'      => $this->input->post('amount'),
            ...
            // Crédit : Compte de caisse (571) - only if caisse was selected
            if ($incomeId > 0) {
                $this->db->insert('accounting_entries', [
                    'date'        => $date_op,
                    'invoice_id'  => $invoice_no,
            ...
            // Journal entry (optional, as in your original code)
            $journal_data = [
                'type_mouvement' => 'depense',
                'date_operation' => date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date'))),
                'reference_piece' => 'RECU-' . $insert_id,
            ...
            $this->journal_model->save($journal_data);
    ...
    public function delete_bank_transaction($id)
    {
        ...
        $transaction = $this->expense_model->get_bank_transaction($id);

        if ($transaction) {
            // Si besoin, ajuster le solde de la banque
            if ($transaction->bank_id) {
                // Logique d'ajustement du solde selon le type de transaction
                $this->adjust_bank_balance($transaction->bank_id, $transaction->amount, $transaction->transaction_type, 'reverse');
            }

            // Supprimer la transaction
            $this->expense_model->delete_bank_transaction($id);

            $this->session->set_flashdata('msg', '<div class="alert alert-success">Transaction supprimée avec succès</div>');
        } else {
            $this->session->set_flashdata('msg', '<div class="alert alert-danger">Transaction non trouvée</div>');
        }

        redirect('admin/expense/bank_management');
    }

    // Ajoutez cette méthode pour récupérer une banque par son ID
...
// Modifiez la méthode getbanklist() pour inclure les informations de banque

// Méthode pour ajuster le solde d'une banque
    private function adjust_bank_balance($bank_id, $amount, $transaction_type, $operation = 'add')
    {
        $bank = $this->expense_model->get_bank_by_id($bank_id);
        if (!$bank) return;
        ...
        $is_credit = in_array($transaction_type, ['Dépôt', 'Virement entrant']);
        ...
        $this->expense_model->update_bank_balance($bank_id, $new_balance);
    }
    ...
    public function delete_bank($id)
    {
        ...
        $data['title'] = 'Fees Master List';
        $this->expense_model->remove_bank($id);
        redirect('admin/expense/bank');
    }
    ...
    public function edit_bank($id)
    {
        ...
        $expnseHead          = $this->expensehead_model->get();
        $data['expheadlist'] = $expnseHead;
        $data['expenselist'] = $this->expense_model->getbank();
        $expense            = $this->expense_model->getbank($id);
        $data['expense']    = $expense;
        $data['title_list'] = 'Fees Master List';

        $expense_result         = $this->expense_model->getbank();
        $data['expenselist']    = $expense_result;

        // ❌ Supprimé : récupération des têtes de dépenses
        // $expnseHead          = $this->expensehead_model->get();
        // $data['expheadlist'] = $expnseHead;

        // ❌ Supprimé : validation du champ exp_head_id
        // $this->form_validation->set_rules('exp_head_id', $this->lang->line('expense_head'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/expense/bankEdit', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $data = array(
                'id'         => $id,
    ...
    public function bank_management()
    {
        ...
        $this->session->set_userdata('top_menu', 'Caisse');
        $this->session->set_userdata('sub_menu', 'expense/bank_management');

        $data['title'] = 'Gestion des Banques';

        // Récupérer la liste des banques
        $data['banks'] = $this->expense_model->get_banks();

        $this->load->view('layout/header', $data);
        $this->load->view('admin/expense/bank_management', $data); // Créez cette vue
        $this->load->view('layout/footer', $data);
    }
    ...
    public function save_bank()
    {
        ...
        $this->form_validation->set_rules('bank_name', 'Nom de la banque', 'trim|required|min_length[2]');
        $this->form_validation->set_rules('bank_code', 'Code banque', 'trim|required|min_length[2]');
        ...
        $data = [
            ...
            'balance' => $this->input->post('initial_balance') ? floatval($this->input->post('initial_balance')) : 0.00,
            'currency' => 'XOF',
            'status' => 1,
            'created_at' => date('Y-m-d H:i:s'),
            'updated_at' => date('Y-m-d H:i:s')
        ...
        if ($bank_id) {
            // Mise à jour - ne pas modifier le solde existant
            unset($data['balance']); // Garder le solde actuel
            unset($data['created_at']); // Garder la date de création originale
            $this->expense_model->update_bank($bank_id, $data);
            $message = 'Banque mise à jour avec succès';
        } else {
            // Création
            $this->expense_model->add_bank($data);
    ...
    public function get_bank_transactions()
    {
        ...
        $bank = $this->expense_model->get_bank_by_id($bank_id);
        if (!$bank) {
            echo '<div class="alert alert-danger">Banque non trouvée</div>';
            return;
        }

        // Récupérer les transactions de la banque
        ...
        foreach ($transactions as $transaction) {
            if (in_array($transaction->designation, ['Débit', 'Retrait', 'Virement sortant', 'Chèque', 'Prélèvement', 'Frais bancaires'])) {
        ...
        $this->load->view('admin/expense/bank_transactions_view', $data);
    }
    ...
    public function getexpenselist()
    {
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('depense', 'can_delete')) {
                    $deletebtn = "<a onclick=\"return confirm('".$this->lang->line('delete_confirm')."')\" 
                    href='".base_url()."admin/expense/delete/".$value->id."' 
                    class='btn btn-default btn-xs' 
                    data-placement='left' 
                    title='".$this->lang->line('delete')."' 
                    data-toggle='tooltip'>

...
class Expense extends Admin_Controller
{
    ...
    public function save_bank_()
    {
        ...
        if ($bank_id) {
            ...
            // Création avec solde initial
            $data['balance'] = $initial_balance;

            // Si solde initial > 0, créer une transaction d'ouverture
            if ($initial_balance > 0) {
                $this->db->trans_start();

                // Créer la banque
                $this->db->insert('banks', $data);
                $new_bank_id = $this->db->insert_id();
                ...
                $transaction_data = [
                    ...
                    'designation' => 'Crédit',
                    'reference' => 'OUVERTURE',
                    'note' => 'Solde initial de la banque',
                    'created_at' => date('Y-m-d H:i:s')
                ];
                $this->db->insert('bank', $transaction_data);

                $this->db->trans_complete();
            } else {
                // Créer la banque sans transaction
    ...
    public function get_bank_transactions_12()
    {
        $bank_id = $this->input->post('bank_id');
        ...
        echo '<h4><i class="fa fa-university"></i> Transactions de la banque: ' . htmlspecialchars($bank_name) . '</h4>';
        echo '<div class="transaction-stats d-flex gap-3 mt-2">';
        echo '<span class="badge badge-primary">Total: ' . count($transactions) . ' transaction(s)</span>';
        echo '<span class="badge ' . ($total_balance >= 0 ? 'badge-success' : 'badge-danger') . '">';
        echo 'Solde: ' . number_format($total_balance, 2, ',', ' ') . ' ' . $currency_symbol;
        echo '</span>';
        echo '</div>';
        echo '</div>';
        ...
        if (empty($transactions)) {
            ...
            foreach ($transactions as $transaction) {
                ...
                // Montant
                echo '<td class="transaction-amount ' . ($is_credit ? 'text-success' : 'text-danger') . '">';
                echo ($is_credit ? '+' : '-') . number_format($transaction->amount, 2, ',', ' ') . ' ' . $currency_symbol;
                echo '</td>';
    ...
    public function get_bank_transactions_()
    {
        $bank_id = $this->input->post('bank_id');

        echo "<h3>Transactions de la banque (ID: $bank_id)</h3>";

        // Simple requête directe
        $sql = "SELECT 
                b.id,
                b.nom,
                b.name,
        ...
        $transactions = $this->db->query($sql, [$bank_id])->result();

        if (empty($transactions)) {
            echo "<p class='alert alert-warning'>Aucune transaction trouvée</p>";
            return;
        }
        ...
        echo "<p>Total: " . count($transactions) . " transaction(s)</p>";
    }

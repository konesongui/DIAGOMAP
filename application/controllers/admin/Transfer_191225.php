...
class Transfer extends Admin_Controller
{
    ...
    public function index_old()
    {
        ...
        $this->session->set_userdata('sub_menu', 'Transfer/index');
    ...
    public function index()
    {
        ...
        $data['caisses'] = $this->db->get('income')->result(); // Pour alimenter le menu déroulant
        $income_result       = $this->income_model->get();
        $data['incomelist']  = $income_result;
        // Tu peux aussi charger les noms des caisses si besoin
    ...
    public function transfer_amount()
    {
        ...
        $from = $this->income_model->getcaisse($from_id);
        $to   = $this->income_model->getcaisse($to_id);

        if ($from && $to) {
            if ($from->amount_re >= $amount && $amount > 0) {

                $this->income_model->Update_Caisses($from_id, [
                    'amount_re' => $from->amount_re - $amount
                ]);

                $this->income_model->Update_Caisses($to_id, [
                    'amount_re' => $to->amount_re + $amount,
                    'amount' => $to->amount_re + $amount,
                    'montant' => $to->amount_re + $amount
                ]);
                ...
                $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>');
                redirect('admin/transfer/index');
            } else {
                echo json_encode(['status' => 'error', 'message' => 'Montant invalide ou solde insuffisant.']);
                return;
            }
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Caisses invalides.']);
            return;
        }
    }
    ...
    {
        $m = $this->db
            ->select('t.id, t.amount, t.date, fc.name as from_caisse, tc.name as to_caisse')
        ...
        foreach ($m as $key => $value) {
            ...
            $row[] = '<strong>' . $value->from_caisse . '</strong>';
            $row[] = '<strong>' . $value->to_caisse . '</strong>';
            $row[] = number_format($value->amount, 0, ',', ' ') . ' ' . $currency_symbol;
            $row[] = date('d/m/Y H:i', strtotime($value->date));

            // Tu peux ajouter un bouton d'action ici si besoin :
            $actions = ''; // exemple futur : <a href="#">Voir</a>
            $row[] = $actions;

...
class Quoteitem extends Admin_Controller
{
    ...
    /**
     * GET STOCK ENTRY DATA - For regular users
     * IN JSON FORMAT
     *
    ...
    {
        // Récupère les données du modèle (tout le monde voit tout)
    ...
    public function data()
    {
        ...
        // DEBUG
        error_log('Quote data - User: ' . $current_user);
        error_log('Quote data - Is Super Admin: ' . ($is_superadmin ? 'Yes' : 'No'));
        error_log('Quote data - Is Admin: ' . ($is_admin ? 'No' : 'Yes'));
        error_log('Quote data - Is Admin User: ' . ($is_admin_user ? 'Yes' : 'No'));
    ...
    public function form()
    {
        ...
        // ð¹ Récupérer le nom de l'utilisateur connecté
        $current_user = $this->session->userdata('admin')['username'];
    ...
    public function add()
    {
        ...
        try {
            ...
            // ð¹ Calcul total avec la nouvelle gestion des taxes
            ...
            // ð¹ Calcul du total TTC avec la taxe appropriée
            $total_ttc = $totals['total_after_discount'] + $tax_amount;

            // ð¹ Enregistrement du devis
            $quote_data = [

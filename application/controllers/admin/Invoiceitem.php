...
class Invoiceitem extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        // Récupérer les métriques du tableau de bord (par défaut année courante)
        $dashboard = $this->invoice_model->get_dashboard_metrics_filtered();
    ...
    /**
     * AJAX: retourne les métriques du dashboard selon filtres
     */
    public function get_dashboard_metrics_ajax() {
        ...
        $start = $this->input->post('start') ?: $this->input->get('start');
        $end = $this->input->post('end') ?: $this->input->get('end');
        $status = $this->input->post('status') ?: $this->input->get('status');
        $customer_id = $this->input->post('customer_id') ?: $this->input->get('customer_id');
        ...
        $metrics = $this->invoice_model->get_dashboard_metrics_filtered($entreprise_id, $start, $end, $status, $customer_id);
    ...
    public function printBalance()
    {
        ...
        $data = [
            ...
            'payments' => $this->invoice_model->getPayments($id),
            ...
            'title' => 'Solde facture ' . ($invoice['invoice_number'] ?? ''),
        ];

        $page = $this->load->view('admin/invoice/print_balance', $data, true);
        $payload = ['status' => '1', 'error' => '', 'page' => $page];
 ...
 * =======================================================*/
    ...
    /**
     * Récupère les paramètres de relance configurés dans les settings.
     */
    private function getReminderSettings()
    {
        ...
        return [
            'enabled' => !empty($row['reminder_enabled']) ? 1 : 0,
            'before_days' => isset($row['reminder_before_days']) ? (int) $row['reminder_before_days'] : 0,
            'on_due_date' => !empty($row['reminder_on_due_date']) ? 1 : 0,
            'after_days_1' => isset($row['reminder_after_days_1']) ? (int) $row['reminder_after_days_1'] : 0,
            'after_days_2' => isset($row['reminder_after_days_2']) ? (int) $row['reminder_after_days_2'] : 0,
            'after_days_3' => isset($row['reminder_after_days_3']) ? (int) $row['reminder_after_days_3'] : 0,
            'sender_email' => trim((string) ($row['reminder_sender_email'] ?? $row['email'] ?? '')),
            'sender_name' => trim((string) ($row['reminder_sender_name'] ?? $row['name'] ?? '')),
        ];
    }
    ...
    public function process_reminders()
    {
        ...
        if (empty($settings['enabled'])) {
            echo "Relances désactivées dans les paramètres.";
            return;
        }
        ...
        foreach ($invoices as $invoice) {
            ...
            // Niveaux de relance
            $reminder_sent = false;
            $before_days = (int) ($settings['before_days'] ?? 0);
            $on_due_date = (int) ($settings['on_due_date'] ?? 0);
            $after_days_1 = (int) ($settings['after_days_1'] ?? 0);
            $after_days_2 = (int) ($settings['after_days_2'] ?? 0);
            $after_days_3 = (int) ($settings['after_days_3'] ?? 0);

            if ($before_days > 0 && $diff === $before_days) {
                $reminder_sent = $this->send_reminder($invoice, 'J-' . $before_days);
            } elseif ($on_due_date && $diff === 0) {
                $reminder_sent = $this->send_reminder($invoice, 'J');
            } elseif ($after_days_1 > 0 && $diff === -(int) $after_days_1) {
                $reminder_sent = $this->send_reminder($invoice, 'J+' . $after_days_1);
            } elseif ($after_days_2 > 0 && $diff === -(int) $after_days_2) {
                $reminder_sent = $this->send_reminder($invoice, 'J+' . $after_days_2);
            } elseif ($after_days_3 > 0 && $diff === -(int) $after_days_3) {
                $reminder_sent = $this->send_reminder($invoice, 'J+' . $after_days_3);
            }
    ...
    private function send_reminder($invoice, $level = 'J')
    {
        ...
        if (!empty($reminder_settings['sender_email'])) {
            $company_email = $reminder_settings['sender_email'];
        }
        if (!empty($reminder_settings['sender_name'])) {
            $company_name = $reminder_settings['sender_name'];
        }
    ...
    /**
     * Envoi manuel d'une relance depuis une facture.
     */
    public function send_manual_reminder()
    {
        ...
        $response = ['status' => 'fail', 'message' => ''];
        $invoice_id = $this->input->post('invoice_id') ?: $this->input->post('id');
        $level = $this->input->post('level') ?: 'J+7';

        if (empty($invoice_id)) {
            $response['message'] = 'Identifiant de facture manquant.';
            echo json_encode($response);
            return;
        }
        ...
        if ($this->send_reminder($invoice, $level)) {
            $this->log_reminder($invoice['id'], 0);
            $response = ['status' => 'success', 'message' => 'Relance envoyée avec succès.'];
        } else {
            $response['message'] = 'L\'envoi de la relance a échoué.';
        }
    ...
    /**
     * Imprime toutes les factures d’un client en un seul bloc
     * Utilisation : POST /admin/invoiceitem/printAllByClient avec customer_id
     */
    public function printAllByClient() {
        ...
        if (!$customer_id) {
            show_error("Aucun client sélectionné.");
        }

        // Récupérer les factures du client
        $invoices = $this->invoice_model->getInvoicesByCustomer($customer_id);
        if (empty($invoices)) {
            show_error("Ce client n'a aucune facture.");
        }
        ...
        // Récupérer les infos du client (depuis la première facture)
        $customer = [
            'name'  => $invoices[0]['customer_name'] . ' ' . $invoices[0]['customer_last_name'],
            'email' => $invoices[0]['customer_email'],
            'phone' => $invoices[0]['customer_phone'],
            'address'=> $invoices[0]['customer_address']
        ];
        ...
        $data = [
            ...
            'title'      => 'Toutes les factures de ' . $customer['name']
        ];

        // Générer la vue d'impression
        $page = $this->load->view('admin/invoice/print_all', $data, true);
        $payload = ['status' => '1', 'error' => '', 'page' => $page];
        $this->output->set_content_type('application/json')->set_output(json_encode($payload));
    }

 ...
 * — Relances automatiques par eâmail (avant échéance, jour J, après échéance)
 * — Intégration FNE (Facture Normalisée Electronique)
 */
class Invoiceitem_supplier extends Admin_Controller
{
    // Configuration FNE
    private $fne_config = [
    ...
    public function __construct()
    {
        ...
        // Models
        $this->load->model('invoice_supplier_model');
        $this->load->model('clients_model');
        $this->load->model('itemcategory_model');
        $this->load->model('item_model');
        $this->load->model('journal_model');
        $this->load->model('setting_model');
        // Supplier model (item_supplier table)
    ...
    /**
     * Charge la configuration FNE depuis fichier puis table fne_settings.
     */
    private function load_fne_config()
    {
        $this->config->load('fne_config');

        $fileApiKey = $this->config->item('fne_api_key');
        $fileTestUrl = $this->config->item('fne_test_url');
        $fileProdUrl = $this->config->item('fne_production_url');
        $filePos = $this->config->item('fne_point_of_sale');
        $fileEstablishment = $this->config->item('fne_establishment');

        if (!empty($fileApiKey)) {
            $this->fne_config['api_key'] = trim($fileApiKey);
        }
        if (!empty($fileTestUrl)) {
            $this->fne_config['test_url'] = rtrim($fileTestUrl, '/');
        }
        if (!empty($fileProdUrl)) {
            $this->fne_config['prod_url'] = rtrim($fileProdUrl, '/');
        }
        if (!empty($filePos)) {
            $this->fne_config['point_of_sale'] = trim($filePos);
        }
        if (!empty($fileEstablishment)) {
            $this->fne_config['establishment'] = trim($fileEstablishment);
        }
        ...
        if (!empty($settings['api_key'])) {
            $this->fne_config['api_key'] = trim($settings['api_key']);
        }
        if (!empty($settings['test_url'])) {
            $this->fne_config['test_url'] = rtrim($settings['test_url'], '/');
        }
        if (!empty($settings['prod_url'])) {
            $this->fne_config['prod_url'] = rtrim($settings['prod_url'], '/');
        }
        if (!empty($settings['environment'])) {
            $this->fne_config['environment'] = trim($settings['environment']);
        }
        // point_of_sale / establishment restent prioritaires via fichier,
        // afin d'éviter les valeurs legacy invalides en base.
    }
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('sub_menu', 'Invoiceitem_supplier/index');
        ...
        $this->load->view('admin/invoice_supplier/list', $data);
    ...
    {
        echo $this->invoice_supplier_model->getListData();
    }

    /**
     * Endpoint d'upload et d'extraction automatique d'une facture fournisseur (PDF).
     * - Accepte un champ file `invoice_pdf` (PDF)
     * - Tente l'extraction texte via smalot/pdfparser
     * - Si extraction insuffisante, tente OCR via ImageMagick + Tesseract (si présents)
     * - Insère la facture dans `invoices_supplier` et les lignes trouvées dans `invoice_supplier_items`
     */
    public function upload_pdf()
    {
        if (!$this->rbac->hasPrivilege('facture', 'can_add')) {
            echo json_encode(['status' => 'fail', 'message' => 'Accès refusé']);
            return;
        }

        if (empty($_FILES['invoice_pdf']) || $_FILES['invoice_pdf']['error'] != 0) {
            echo json_encode(['status' => 'fail', 'message' => 'Fichier manquant ou invalide']);
            return;
        }
        ...
        if ($ext !== 'pdf') {
            echo json_encode(['status' => 'fail', 'message' => 'Seul le format PDF est accepté']);
            return;
        }

        $upload_dir = FCPATH . 'uploads\\invoices_supplier\\';
        if (!is_dir($upload_dir)) {
            @mkdir($upload_dir, 0755, true);
        }

        $target = $upload_dir . time() . '_' . preg_replace('/[^a-zA-Z0-9_\-\.]/', '_', basename($file['name']));
        if (!move_uploaded_file($file['tmp_name'], $target)) {
            echo json_encode(['status' => 'fail', 'message' => 'Impossible de sauvegarder le fichier']);
            return;
        }
        ...
        $text = '';
        // Première tentative : extraction texte (si library disponible)
        try {
        ...
        // Si texte trop court, tenter OCR (ImageMagick + Tesseract)
        if (strlen(trim($text)) < 20) {
            $outPrefix = $upload_dir . 'ocr_' . time();
            // Générer des images depuis le PDF (ImageMagick `magick` ou `convert` ou `pdftoppm`)
            @exec('magick -density 300 "' . $target . '" "' . $outPrefix . '.png" 2>&1', $magickOut, $magickRet);
            $imgs = glob($outPrefix . '*.png');
            if (empty($imgs)) {
                // Essayer pdftoppm
                @exec('pdftoppm -png "' . $target . '" "' . $outPrefix . '" 2>&1', $magickOut2, $magickRet2);
                $imgs = glob($outPrefix . '-*.png');
                $imgs = array_merge($imgs, glob($outPrefix . '*.png'));
            }

            foreach ($imgs as $img) {
                // Tesseract (langue française)
                @exec('tesseract "' . $img . '" stdout -l fra 2>&1', $tessOut, $tessRet);
                if (is_array($tessOut)) {
                    $text .= "\n" . implode("\n", $tessOut);
                } else {
                    $text .= "\n" . $tessOut;
        ...
        // Normaliser et analyser le texte
        $lines = preg_split('/\\r\\n|\\r|\\n/', $text);
        $fulltext = implode("\n", $lines);

        // Extraire N° facture
        $invoice_number = null;
        if (preg_match('/(N(?:°|º)?\s*[:\-]?\s*|Facture\s*[:\-]?\s*|Invoice\s*[:\-]?\s*)([A-Z0-9\-\/\\_]{3,60})/i', $fulltext, $m)) {
            $invoice_number = trim($m[2]);
        } elseif (preg_match('/(FAC[-\s]?\d{2,4}[-\s]?\d{1,6})/i', $fulltext, $m2)) {
            $invoice_number = trim($m2[1]);
        }

        // Extraire la date (dd/mm/YYYY ou YYYY-mm-dd)
        $invoice_date = null;
        if (preg_match('/(\d{2}[\/\.\-]\d{2}[\/\.\-]\d{4})/', $fulltext, $md)) {
            $d = str_replace(['.', '-'], '/', $md[1]);
            $dt = DateTime::createFromFormat('d/m/Y', $d);
            if ($dt) {
                $invoice_date = $dt->format('Y-m-d');
            }
        } elseif (preg_match('/(\d{4}[\/\.\-]\d{2}[\/\.\-]\d{2})/', $fulltext, $md2)) {
            $dt = DateTime::createFromFormat('Y-m-d', $md2[1]);
            if ($dt) {
                $invoice_date = $dt->format('Y-m-d');
            }
        }

        // Extraire total TTC (essayer plusieurs clés)
        $total_ttc = 0;
        if (preg_match_all('/Total(?:\s*TTC|\s*):?\s*([0-9 \.,]+)/i', $fulltext, $mt)) {
            $last = end($mt[1]);
            $total_ttc = floatval(str_replace([" ", '.', ','], ['', '', '.'], $last));
        } else {
            if (preg_match_all('/([0-9]{1,3}(?:[ \\.,][0-9]{3})*(?:[\\.,][0-9]{2})?)/', $fulltext, $mnums)) {
                $lastnum = end($mnums[1]);
                $total_ttc = floatval(str_replace([" ", '.', ','], ['', '', '.'], $lastnum));
            }
        }

        // Extraire TVA
        $tva_amount = 0;
        if (preg_match('/TVA(?:\s*[:\-]?\s*)([0-9 \.,]+)/i', $fulltext, $mtva)) {
            $tva_amount = floatval(str_replace([" ", '.', ','], ['', '', '.'], $mtva[1]));
        }

        // Nom fournisseur (premières lignes non vides)
        $supplier = null;
        for ($i = 0; $i < 6; $i++) {
            if (isset($lines[$i]) && strlen(trim($lines[$i])) > 3) {
                $supplier = trim($lines[$i]);
                break;
            }
        }

        // Tentative basique d'extraction des lignes (analyse ligne par ligne)
        $items = [];
        $pos = 0;
        foreach ($lines as $line) {
            if (preg_match_all('/([0-9]{1,3}(?:[ \\.,][0-9]{3})*(?:[\\.,][0-9]{2})?)/', $line, $mn)) {
                $nums = $mn[1];
                if (count($nums) >= 1 && strlen(trim(preg_replace('/[0-9 \\.,]/', '', $line))) > 0) {
                    $line_total_raw = end($nums);
                    $line_total = floatval(str_replace([" ", '.', ','], ['', '', '.'], $line_total_raw));
                    if ($line_total > 0 && strlen(trim($line)) > 10) {
                        $pos++;
                        $items[] = [
                            'description' => trim(preg_replace('/[0-9 \\.,]+$/', '', $line)),
        ...
        $response = [
            ...
            'data' => [
                ...
                'items' => $items,
                'file' => str_replace(FCPATH, '', $target)
            ]
        ];
    ...
    /**
     * Enregistrer la facture parsée (après revue côté client)
     */
    public function save_parsed_invoice()
    {
        if (!$this->rbac->hasPrivilege('facture', 'can_add')) {
            echo json_encode(['status' => 'fail', 'message' => 'Accès refusé']);
            return;
        ...
        if (empty($post) || empty($post['invoice_number'])) {
            echo json_encode(['status' => 'fail', 'message' => 'Données manquantes']);
            return;
        }

        $entreprise_id = $this->session->userdata('entreprise_id') ?? $this->session->userdata('tenant_id') ?? 0;
        $created_by = $this->session->userdata('id') ?? null;

        // Attempt to map supplier name to an existing supplier (item_supplier)
        $supplier_name = isset($post['supplier']) ? trim($post['supplier']) : '';
        $customer_id = isset($post['customer_id']) ? $post['customer_id'] : null;

        if (empty($customer_id) && !empty($supplier_name)) {
            // Try several likely columns to find the supplier record
            $candidate_cols = ['name', 'supplier_name', 'company', 'company_name', 'nom', 'fournisseur'];
            foreach ($candidate_cols as $col) {
                if ($this->db->field_exists($col, 'item_supplier')) {
                    $this->db->select('id')->from('item_supplier')->like($col, $supplier_name);
        ...
        $invoice_data = [
            ...
            'invoice_date' => !empty($post['invoice_date']) ? $post['invoice_date'] : null,
            'due_date' => !empty($post['due_date']) ? $post['due_date'] : null,
            'apply_tva' => isset($post['apply_tva']) ? (int)$post['apply_tva'] : (empty($post['tva_amount']) ? 0 : 1),
            'tva_amount' => !empty($post['tva_amount']) ? floatval($post['tva_amount']) : 0,
            'total_ttc' => !empty($post['total_ttc']) ? floatval($post['total_ttc']) : 0,
            'total_ht' => !empty($post['total_ht']) ? floatval($post['total_ht']) : 0,
            'remaining_amount' => !empty($post['remaining_amount']) ? floatval($post['remaining_amount']) : (isset($post['total_ttc']) ? floatval($post['total_ttc']) : 0),
            'amount_paid' => !empty($post['amount_paid']) ? floatval($post['amount_paid']) : 0,
            'notes' => isset($post['notes']) ? $post['notes'] : null,
            'status' => isset($post['status']) ? (int)$post['status'] : 1,
            'created_by' => $created_by,
            'tenant_id' => $this->session->userdata('tenant_id') ?? 0,
            'entreprise_id' => $entreprise_id,
            'user_name' => isset($post['supplier']) ? $post['supplier'] : null,
        ];

        $this->db->insert('invoices_supplier', $invoice_data);
        $invoice_id = $this->db->insert_id();

        // Insert items
        if (!empty($post['items']) && is_array($post['items'])) {
            foreach ($post['items'] as $idx => $it) {
                $this->db->insert('invoice_supplier_items', [
                    'invoice_id' => $invoice_id,
                    'category_id' => isset($it['category_id']) ? $it['category_id'] : null,
                    ...
                    'unit' => isset($it['unit']) ? $it['unit'] : 'Unité',
                    'line_total' => isset($it['line_total']) ? floatval($it['line_total']) : 0,
                    'position' => isset($it['position']) ? (int)$it['position'] : $idx + 1,
                    'client_id' => null,
                    'tenant_id' => $this->session->userdata('tenant_id') ?? 0,
                    'entreprise_id' => $entreprise_id
                ]);
        ...
        echo json_encode(['status' => 'success', 'message' => 'Facture enregistrée', 'invoice_id' => $invoice_id]);
        return;
    }
    ...
    public function certifyFNE()
    {
        ...
        try {
            ...
            if ($fne_response['status'] === 'success') {
                ...
                $balanceSticker = $fne_response['data']['balance_sticker']
                    ?? ($fne_response['data']['balance_funds'] ?? 0);

                $update_data = [
                    'fne_certified' => 1,
                    'fne_reference' => $fne_response['data']['reference'] ?? '',
                    'fne_token' => $fne_response['data']['token'] ?? '',
                    'fne_balance_sticker' => (int)$balanceSticker,
    ...
    <body>
        ...
        if (empty($validation_errors)) {
            echo "<p class='success'>â Les données sont valides !</p>";
        } else {
            echo "<p class='error'>â Erreurs de validation :</p>";
            echo "<ul class='error'>";
        ...
        foreach ($checks as $field => $expected) {
            ...
            if (!isset($fne_data[$field])) {
                $status = "<span class='error'>â Champ manquant</span>";
            } elseif (in_array('required', $expected) && empty($value)) {
                $status = "<span class='error'>â Champ vide</span>";
            } elseif (in_array('boolean', $expected)) {
                $status = is_bool($value) ? "<span class='valid'>â Booléen valide</span>" : "<span class='error'>â Doit être un booléen</span>";
            } elseif (in_array($value, $expected)) {
                $status = "<span class='valid'>â Valeur valide</span>";
            } elseif (!in_array('required', $expected) && !in_array('boolean', $expected)) {
                $status = "<span class='invalid'>â Valeur doit être parmi: " . implode(', ', $expected) . "</span>";
            } else {
                $status = "<span class='valid'>â OK</span>";
            }
            ...
            foreach ($fne_data['items'] as $i => $item) {
                ...
                foreach ($item_checks as $field => $rules) {
                    ...
                    if (!isset($item[$field])) {
                        ...
                        $status = is_array($value) ? "<span class='valid'>â Tableau valide</span>" : "<span class='error'>â Doit être un tableau</span>";
                        if (is_array($value) && empty($value)) {
                            $status = "<span class='error'>â Tableau vide</span>";
                        }
                    } elseif (in_array('number', $rules)) {
                        $status = is_numeric($value) ? "<span class='valid'>â Nombre valide</span>" : "<span class='error'>â Doit être un nombre</span>";
                        if (is_numeric($value) && in_array('positive', $rules) && $value <= 0) {
                            $status = "<span class='error'>â Doit être positif</span>";
                        }
                    }
    ...
    private function callFNEAPI($data)
    {
        ...
        $baseUrl = $this->fne_config['environment'] === 'prod' && !empty($this->fne_config['prod_url'])
            ? $this->fne_config['prod_url']
            : $this->fne_config['test_url'];
        $baseUrl = rtrim($baseUrl, '/');
        $url = (substr($baseUrl, -23) === '/external/invoices/sign')
            ? $baseUrl
            : $baseUrl . '/external/invoices/sign';

        // Log pour débogage
        log_message('debug', 'FNE Request URL: ' . $url);
        ...
        if ($http_code >= 200 && $http_code < 300) {
            ...
            // Retry intelligent quand FNE rejette l'établissement.
            if (
                $http_code === 400 &&
                is_array($result) &&
                isset($result['errors']) &&
                stripos(json_encode($result['errors']), 'Establishment is invalid') !== false
            ) {
                $candidates = [];
                $currentEstablishment = isset($data['establishment']) ? trim((string)$data['establishment']) : '';
                $currentPos = isset($data['pointOfSale']) ? trim((string)$data['pointOfSale']) : '';

                if ($currentPos !== '' && strcasecmp($currentPos, $currentEstablishment) !== 0) {
                    $candidates[] = $currentPos;
                }

                if ($currentEstablishment !== '') {
                    $ascii = @iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $currentEstablishment);
                    if (!empty($ascii) && strcasecmp($ascii, $currentEstablishment) !== 0) {
                        $candidates[] = trim($ascii);
                    }
                }

                foreach (['INTENDANT', 'Siège', 'Siege'] as $fallbackEstablishment) {
                    if (strcasecmp($fallbackEstablishment, $currentEstablishment) !== 0) {
                        $candidates[] = $fallbackEstablishment;
                    }
                }

                foreach (array_unique($candidates) as $candidate) {
                    $retryData = $data;
                    $retryData['establishment'] = $candidate;
                    $retryPayload = json_encode($retryData, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            ...
            if (isset($result['errors'])) {
                if (is_array($result['errors'])) {
                    $flat = [];
                    array_walk_recursive($result['errors'], function ($v) use (&$flat) {
                        $flat[] = $v;
                    });
                    if (!empty($flat)) {
                        $error_message .= " (" . implode(' | ', $flat) . ")";
                    }
    ...
    /** Envoi d'une relance eâmail formatée */
    ...
    public function setPayment()
    {
        ...
        try {
            $this->form_validation->set_rules('invoice_id', 'Facture fournisseur', 'required|numeric');
            ...
            $this->form_validation->set_rules('source_type', 'Type de source', 'required|in_list[caisse,banque]');
            $this->form_validation->set_rules('source_id', 'Source', 'required|numeric');
            ...
            $invoice = $this->invoice_supplier_model->getInvoiceWithItems($invoice_id);
            if (!$invoice) {
            ...
            if (empty($reference)) {
                $reference = 'SUP-PAY-' . date('YmdHis') . '-' . $invoice_id;
            }
            ...
            if (!$this->invoice_supplier_model->addPayment($payment_data)) {
                throw new Exception("Erreur lors de l'enregistrement du paiement fournisseur");
            }

            // --- 2. METTRE À JOUR LA SOURCE (SORTIE D'ARGENT) ---
            if ($source_type == 'caisse') {
                ...
                $new_balance = $old_balance - $amount; // SORTIE
                if ($new_balance < 0) throw new Exception('Solde insuffisant dans la caisse');
                ...
                $operation_caisse_data = [
                    ...
                    'montant'              => $amount,
                    'designation'          => 'Paiement facture fournisseur #' . $invoice['invoice_number'],
                    'caisse_id'            => $source_id,
                ...
                $this->db->insert('mouvements', [
                    ...
                    'montant'               => $amount,
                    'description'           => 'Paiement facture fournisseur #' . $invoice['invoice_number'],
                    'reference'             => $reference,
                ...
                if ($new_balance < 0) throw new Exception('Solde insuffisant sur le compte bancaire');
                ...
                $bank_operation_data = [
                    ...
                    'designation'      => 'Débit',
                    'name'             => 'Paiement facture fournisseur #' . $invoice['invoice_number'],
                    'nom'              => $invoice['customer_name'] ?? 'Fournisseur',
                    'amount'           => $amount,
                    'reference'        => $reference,
                    'payment_mode'     => $method,
                    'note'             => $notes,
            ...
            $entries_payment = [
                [
                    'date'        => $payment_date,
                    'invoice_id'  => $invoice_id,
                    'account'     => ($source_type == 'caisse') ? '511' : '512',
                    'debit'       => 0,
                    'credit'      => $amount,
                    'description' => 'Paiement fournisseur facture #' . $invoice['invoice_number'],
                    'created_at'  => date('Y-m-d H:i:s'),
                ],
                [
                    'date'        => $payment_date,
                    ...
                    'debit'       => $amount,
                    'credit'      => 0,
                    'description' => 'Règlement fournisseur facture #' . $invoice['invoice_number'],
                    'created_at'  => date('Y-m-d H:i:s'),
                ]
            ];
            ...
            if ($invoice['apply_tva'] && $invoice['tva_amount'] > 0) {
                $entries_payment[] = [
                    ...
                    'invoice_id'  => $invoice_id,
                    'account'     => '4456',
                    'debit'       => $invoice['tva_amount'] * ($amount / $invoice['total_ttc']),
                    'credit'      => 0,
                    'description' => 'TVA déductible sur paiement facture #' . $invoice['invoice_number'],
                    'created_at'  => date('Y-m-d H:i:s'),
                ];
            }
            ...
            $status = ($new_remaining <= 0) ? Invoice_supplier_model::STATUS_PAID : Invoice_supplier_model::STATUS_PARTIAL;
            $this->db->where('id', $invoice_id)->update('invoices_supplier', [
                'status' => $status,
                'paid_at' => ($new_remaining <= 0) ? date('Y-m-d H:i:s') : null
            ]);
            ...
            $response['status']  = 'success';
            $response['message'] = 'Paiement fournisseur enregistré avec succès !';
            $response['data'] = [
                'invoice_id' => $invoice_id,
            ...
            $this->db->trans_rollback();
            $response['message'] = 'Erreur: ' . $e->getMessage();
            log_message('error', 'Supplier Payment Add Error: ' . $e->getMessage());
        }

        echo json_encode($response);
    }
    public function setPayment_16()
    {
        if (!$this->rbac->hasPrivilege('facture', 'can_edit')) {
            access_denied();
        }
        ...
        try {
            ...
            $source_id = $this->input->post('source_id');
            if (empty($source_id)) {
                // Si source_id est vide, essayer de récupérer caisse_id ou banque_id
                if ($source_type == 'caisse') {
                    $source_id = $this->input->post('caisse_id');
                } elseif ($source_type == 'banque') {
                    $source_id = $this->input->post('banque_id');
                }
            }
            if (empty($source_id)) {
           ...
           // }
            ...
            $response['status']  = 'success';
            $response['message'] = 'â Paiement enregistré avec succès !';
            $response['data'] = [
                'invoice_id' => $invoice_id,
                'remaining' => $new_remaining,
                'source_type' => $source_type,
            ...
            $this->db->trans_rollback();
            $response['message'] = 'â Erreur: ' . $e->getMessage();
    ...
    public function setPayment_20()
    {
        ...
        try {
            ...
            // ð¹ Enregistrement dans mouvements
            $mouvement_data = [
                'type_mouvement' => 'entree',
                'montant'        => $amount,
    ...
    public function print()
    {
        ...
        $payload = ['status' => '1', 'error' => '', 'page' => $page];
        $json    = json_encode($payload, $flags);
        if ($json === false) {
            $payload = ['status' => '0', 'error' => 'Encodage JSON impossible.', 'page' => ''];
            $json    = json_encode($payload, $flags);
        }
        $this->output->set_content_type('application/json', 'utf-8')->set_output($json);
    }
    ...
    /** Envoi manuel d'une facture par eâmail */
        ...
        try {
            ...
            if ($data['invoice']) {
                $payload = [
                    ...
                    // ð¹ Ajout des infos utilisateur pour Reply-To
    ...
    public function validateInvoice()
    {
        ...
        // â¡ Mettre à jour le statut
        $this->invoice_model->update($invoiceId, ['status' => 2]);

        // â¡ Générer les écritures comptables
 ...
 * =======================================================*/
    ...
    private function prepareFNEData($invoice)
    {
        ...
        // Point de vente et établissement depuis la config FNE
        $pointOfSale = !empty($this->fne_config['point_of_sale']) ? $this->fne_config['point_of_sale'] : 'PDV001';
        $establishment = !empty($this->fne_config['establishment']) ? $this->fne_config['establishment'] : 'ETABLISSEMENT001';
        $clientPhone = !empty($invoice['customer_phone']) ? preg_replace('/\D+/', '', $invoice['customer_phone']) : '';
        if ($clientPhone === '' && !empty($company['phone'])) {
            $clientPhone = preg_replace('/\D+/', '', $company['phone']);
        }
        $clientEmail = !empty($invoice['customer_email']) ? $invoice['customer_email'] : (!empty($company['email']) ? $company['email'] : '');
    ...
    private function getTaxCode($invoice)
    {
        if (!$invoice['apply_tva'] || $invoice['tva_rate'] <= 0) {
            return 'TVAC'; // Exonération 0% valide FNE
        }

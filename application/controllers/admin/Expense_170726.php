...
class Expense extends Admin_Controller
{
    ...
    public function bank()
    {
        ...
        // Récupérer les banques filtrées par entreprise_id
        $data['banks'] = $this->expense_model->get_banks($entreprise_id);
        ...
        if ($this->form_validation->run() == true) {
            // Récupérer l'entreprise_id pour l'insertion
            $userdata = $this->customlib->getUserData();
            $entreprise_id = $userdata['entreprise_id'] ?? 0;
            ...
            // Récupérer l'ID de transaction (vide si nouvelle transaction)
            $transaction_id = $this->input->post('transaction_id');
            $bank_id = $this->input->post('bank_id');

            // Préparer les données avec entreprise_id
            $data = [
                'bank_id' => $bank_id,
                ...
                'entreprise_id' => $entreprise_id // ← AJOUT DE L'ENTREPRISE_ID
            ];
            ...
            if (!empty($transaction_id)) {
                ...
                $old_transaction = $this->expense_model->get_bank_transaction_by_id($transaction_id);

                if (!$old_transaction) {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <i class="fa fa-exclamation-triangle"></i> Transaction non trouvée
                </div>');
                ...
                $this->expense_model->update_bank_transaction($transaction_id, $data);

                // Recalculer les soldes des banques concernées
                if ($old_bank_id != $bank_id) {
                    // Si la banque a changé, recalculer les deux banques
                    $this->expense_model->recalculate_bank_balance($old_bank_id);
                    $this->expense_model->recalculate_bank_balance($bank_id);
                } else {
                    // Même banque, juste recalculer celle-ci
                    $this->expense_model->recalculate_bank_balance($bank_id);
                }
                ...
                if ($this->db->trans_status() === FALSE) {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <i class="fa fa-exclamation-triangle"></i> Erreur lors de la mise à jour de la transaction
                </div>');
                } else {
                    $this->session->set_flashdata('msg', '<div class="alert alert-success alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <i class="fa fa-check-circle"></i> Transaction modifiée avec succès !
                    <br><small>Montant: ' . number_format($data['amount'], 0, ',', ' ') . ' FCFA</small>
                </div>');
                }
                ...
                // Gestion de la caisse (votre code existant)
                ...
                if ($incomeId > 0) {
                    ...
                    if ($incomeAmountAvailable < $amount) {
                        ...
                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                        <i class="fa fa-exclamation-triangle"></i> Le solde de la caisse sélectionnée est insuffisant
                    </div>');
                        redirect('admin/expense/bank');
                    }
                ...
                if ($insert_id) {
                    $success_msg = '<div class="alert alert-success alert-dismissible fade-in">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <i class="fa fa-check-circle"></i> <strong>Transaction bancaire enregistrée avec succès !</strong><br>
                    <small>
                        <i class="fa fa-money"></i> Montant: ' . number_format($amount, 0, ',', ' ') . ' FCFA
                        <span style="margin-left: 15px;"><i class="fa fa-tag"></i> Réf: ' . htmlspecialchars($data['reference'] ?? 'N/A') . '</span>
                        <span style="margin-left: 15px;"><i class="fa fa-building"></i> Banque: ' . htmlspecialchars($this->input->post('name')) . '</span>
                    </small>
                </div>';
                    ...
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <i class="fa fa-exclamation-triangle"></i> Erreur lors de l\'enregistrement de la transaction bancaire
                </div>');
                }
            }
    ...
    public function bank_17()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($transaction_id)) {
                ...
                if (!$old_transaction) {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger">Transaction non trouvée</div>');
                    redirect('admin/expense/bank');
                }
                ...
                if ($this->db->trans_status() === FALSE) {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger">Erreur lors de la mise à jour</div>');
                } else {
                    $this->session->set_flashdata('msg', '<div class="alert alert-success">Transaction modifiée avec succès</div>');
                }
                ...
                $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Transaction ajoutée avec succès</div>');
            }
    ...
    public function get_transaction_for_edit()
    {
        $transaction_id = $this->input->post('transaction_id');
        ...
        $transaction = $this->expense_model->get_bank_transaction_by_id($transaction_id);
        ...
        $transaction->date_formatted = date($this->customlib->getSchoolDateFormat(), strtotime($transaction->date));
...
// ============ AJOUTER CETTE MÉTHODE AU CONTRÔLEUR ============
    private function recalculate_bank_balance($bank_id)
    {
        if (!$bank_id) return 0;

        // 1. Récupérer le solde initial (balance_re)
        $bank = $this->db->get_where('banks', ['id' => $bank_id])->row();
        if (!$bank) return 0;

        $initial_balance = floatval($bank->balance_re ?? 0);

        // 2. Calculer le total des transactions
        $this->db->select("
        SUM(CASE 
...
// ============ AJOUTER CETTE MÉTHODE POUR L'AJAX ============

    public function bank_270128()
    {
        if (!$this->rbac->hasPrivilege('depense', 'can_view')) {
            access_denied();
        }
    ...
    public function edit_bank_transaction($id)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $old_transaction = $this->db->get_where('bank', ['id' => $id])->row();
                $old_bank_id = $old_transaction->bank_id;
                ...
                // CORRECTION : Bien utiliser 'id' et non 'bank_id'
                $this->db->where('id', $id);
                $this->db->update('bank', $update_data);

                // Récupérer la nouvelle banque
    ...
    public function edit_bank_transaction_250126($id)
    {
        if (!$this->rbac->hasPrivilege('depense', 'can_edit')) {
            access_denied();
        }
    ...
    public function get_bank_transactions()
    {
        ...
        // Récupérer le nom de la banque et le solde initial
        $bank = $this->db->get_where('banks', ['id' => $bank_id])->row();
        $bank_name = $bank ? $bank->name : 'Inconnue';
        $initial_balance = $bank ? $bank->balance_re : 0;
        ...
        // Calculer le solde total = solde initial + total des transactions
        $transaction_total = 0;
        foreach ($transactions as $trans) {
            if ($trans->designation == 'Crédit' ||
                $trans->transaction_type == 'Dépôt' ||
                $trans->transaction_type == 'Virement entrant') {
                $transaction_total += floatval($trans->amount);
            } else {
                $transaction_total -= floatval($trans->amount);
            }
        }
        $total_balance = $initial_balance + $transaction_total;
        ...
        echo '<span style="background-color: #28a745; color: white; padding: 5px 10px; border-radius: 5px;">';
        echo 'Solde initial: ' . number_format($initial_balance, 2, ',', ' ') . ' ' . $currency_symbol;
        echo '</span>';
        echo '<span style="background-color: red; color: white; padding: 5px 10px; border-radius: 5px;">';
        echo 'Solde actuel: ' . number_format($total_balance, 2, ',', ' ') . ' ' . $currency_symbol;
        echo '</span>';
        ...
        if (empty($transactions)) {
            ...
            echo '<th width="120">Entrée</th>';
            echo '<th width="120">Sortie</th>';
            ...
            if ($initial_balance > 0) {
                echo '<tr class="transaction-item initial-balance-row" style="background-color: #f8f9fa;">';
                echo '<td>' . date('d/m/Y') . '</td>';
                echo '<td><em>Solde initial</em></td>';
                echo '<td><em>Dépôt initial</em></td>';
                echo '<td><em>Dépôt</em></td>';
                echo '<td><span class="badge badge-success">Crédit</span></td>';
                echo '<td class="text-success font-weight-bold">' . number_format($initial_balance, 2, ',', ' ') . ' ' . $currency_symbol . '</td>';
                echo '<td class="text-muted">-</td>';
                echo '<td><em>INITIAL</em></td>';
                echo '<td></td>';
                echo '</tr>';
            }
            ...
            echo '</select>';
            echo '</td>';
            echo '<td><input type="text" class="filter-input" data-column="5" placeholder="Entrée..." style="width: 100%; padding: 5px; font-size: 12px;"></td>';
            echo '<td><input type="text" class="filter-input" data-column="6" placeholder="Sortie..." style="width: 100%; padding: 5px; font-size: 12px;"></td>';
            echo '<td><input type="text" class="filter-input" data-column="7" placeholder="Filtrer référence..." style="width: 100%; padding: 5px; font-size: 12px;"></td>';
            echo '<td><nobr><button class="btn btn-xs btn-primary" id="resetFiltersBtn" style="display:none;">Réinitialiser</button></nobr></td>';
            echo '</tr>';
            ...
            foreach ($transactions as $transaction) {
                ...
                // Date
                echo '<td class="transaction-date">' . date('d/m/Y', strtotime($transaction->date)) . '</td>';
                // Nom
                echo '<td class="transaction-name">' . htmlspecialchars($transaction->nom) . '</td>';
                // Libellé
                echo '<td class="transaction-description">' . htmlspecialchars($transaction->name) . '</td>';
                // Type
                echo '<td class="transaction-type">' . htmlspecialchars($transaction->transaction_type) . '</td>';
                // Désignation
                echo '<td class="transaction-designation"><span class="badge ' . ($is_credit ? 'badge-success' : 'badge-danger') . '">' . htmlspecialchars($transaction->designation) . '</span></td>';

                // Entrée
                if ($is_credit) {
                    echo '<td class="transaction-amount text-success font-weight-bold">' . number_format($transaction->amount, 2, ',', ' ') . ' ' . $currency_symbol . '</td>';
                } else {
                    echo '<td class="text-muted">-</td>';
                }
                // Sortie
                if (!$is_credit) {
                    echo '<td class="transaction-amount text-danger font-weight-bold">' . number_format($transaction->amount, 2, ',', ' ') . ' ' . $currency_symbol . '</td>';
                } else {
                    echo '<td class="text-muted">-</td>';
                }

                // Référence
                echo '<td class="transaction-reference">' . htmlspecialchars($transaction->reference ?: '-') . '</td>';
                // Actions
                echo '<td class="transaction-actions">';
                if ($this->rbac->hasPrivilege('depense', 'can_edit')) {
                    echo '<button class="btn btn-sm btn-warning edit-transaction" data-id="' . $transaction->id . '" title="Modifier"><i class="fa fa-edit"></i></button> ';
                }
                if ($this->rbac->hasPrivilege('depense', 'can_delete')) {
                    echo '<button class="btn btn-sm btn-danger delete-transaction" data-id="' . $transaction->id . '" title="Supprimer"><i class="fa fa-trash"></i></button>';
                }
                echo '</td>';
                echo '</tr>';
            }
    ...
    public function get_transaction_details180526($transaction_id)
    {
        if (!$transaction_id) {
    ...
    public function get_transaction_details($transaction_id)
    {
        ...
        // Entrée
        $html .= '<tr>';
        $html .= '<td width="40%"><strong>Entrée :</strong></td>';
        $html .= '<td>';
        if ($transaction->designation == 'Crédit') {
            $html .= '<span class="text-success" style="font-size: 16px; font-weight: bold;">' . number_format($transaction->amount, 2, ',', ' ') . ' ' . $currency_symbol . '</span>';
        } else {
...
// Sortie
        $html .= '<tr>';
        $html .= '<td width="40%"><strong>Sortie :</strong></td>';
        $html .= '<td>';
        if ($transaction->designation != 'Crédit') {
            $html .= '<span class="text-danger" style="font-size: 16px; font-weight: bold;">' . number_format($transaction->amount, 2, ',', ' ') . ' ' . $currency_symbol . '</span>';
        } else {
            $html .= '<em class="text-muted">-</em>';
        }
        $html .= '</td>';
    ...
    {
        // Récupérer le solde initial (balance_re)
        $this->db->select('balance_re');
        $this->db->from('banks');
        $this->db->where('id', $bank_id);
        $initial_balance = $this->db->get()->row()->balance_re ?? 0;
    ...
    END) as transaction_total');
        ...
        $transaction_total = $result->transaction_total ?? 0;

        // Le solde total = solde initial + total des transactions
        $new_balance = $initial_balance + $transaction_total;
    ...
    private function update_bank_balance_200126($bank_id)
    {
    ...
    public function save_bank()
    {
        ...
        $bank_id = $this->input->post('bank_id');
        $initial_balance = floatval($this->input->post('initial_balance') ?: 0);

        $data = [
            'name' => $this->input->post('bank_name'),
            'code' => $this->input->post('bank_code'),
            'account_number' => $this->input->post('account_number'),
            'balance' => $initial_balance, // Même que le solde initial au début
            'balance_re' => $initial_balance, // Solde initial sauvegardé
        ];
        ...
        if ($bank_id) {
            // Pour la mise à jour, ne pas modifier balance_re sauf si spécifié
            $existing_bank = $this->db->get_where('banks', ['id' => $bank_id])->row();
            if ($existing_bank) {
                // Conserver l'ancien balance_re lors de la mise à jour
                // (sauf si vous voulez permettre de modifier le solde initial)
                $data['balance_re'] = $existing_bank->balance_re;
                // Recalculer le solde avec les transactions existantes
                $data['balance'] = $this->recalculate_bank_balance($bank_id, $initial_balance);
            }
...
// Ajoutez cette méthode helper
    private function recalculate_bank_balance_270126($bank_id, $initial_balance = null)
    {
        if ($initial_balance === null) {
            ...
            $initial_balance = $bank->balance_re ?? 0;
        }
    ...
    END) as transaction_total');
        ...
        return $initial_balance + $transaction_total;
    }

    public function get_bank_initial_balance()
    {
        $bank_id = $this->input->post('bank_id');
        ...
        if ($bank) {
            echo json_encode([
                'status' => 'success',
                'initial_balance' => $bank->balance_re
            ]);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Banque non trouvée']);
        }
    }

    public function save_bank_200126()
    {
        $this->load->library('upload');
    ...
    public function get_bank_transactions_180526()
    {
        $bank_id = $this->input->post('bank_id');
        ...
        $bank = $this->db->get_where('banks', ['id' => $bank_id])->row();
        $bank_name = $bank ? $bank->name : 'Inconnue';
        $initial_balance = $bank ? $bank->balance_re : 0; // ← AJOUT IMPORTANT
        ...
        // Calculer le solde total = solde initial + total des transactions
        $transaction_total = 0; // ← Variable pour le total des transactions
        foreach ($transactions as $trans) {
            if ($trans->designation == 'Crédit' ||
                $trans->transaction_type == 'Dépôt' ||
                $trans->transaction_type == 'Virement entrant') {
        ...
        $total_balance = $initial_balance + $transaction_total; // ← CORRECTION ICI
        ...
        echo '<span style="background-color: red; color: white; padding: 5px 10px; border-radius: 5px;">';
        echo 'Solde actuel: ' . number_format($total_balance, 2, ',', ' ') . ' ' . $currency_symbol; // ← Changé "Solde" en "Solde actuel"
        echo '</span>';
        echo '<button id="resetFiltersBtn" class="btn btn-sm btn-outline-secondary ml-2" style="display: none;">';
        echo '<i class="fa fa-refresh"></i> Réinitialiser filtres';
        echo '</button>';
        ...
        // Afficher une ligne pour le solde initial si aucune transaction
        if (empty($transactions)) {
            echo '<div class="alert alert-info">';
            echo '<i class="fa fa-info-circle"></i> Aucune transaction trouvée pour cette banque.';
            echo '</div>';

            // Afficher quand même le solde initial dans le tableau
            echo '<div class="table-responsive">';
            echo '<table class="table table-hover table-bordered" id="transactionsTable">';
            echo '<thead class="thead-dark">';
            echo '<tr>';
            ...
            // Ajouter une ligne pour le solde initial en premier
            if ($initial_balance > 0) {
                echo '<tr class="transaction-item initial-balance-row" style="background-color: #f8f9fa;">';
                echo '<td>' . date('d/m/Y') . '</td>';
                echo '<td><em>Solde initial</em></td>';
    ...
    public function get_bank_transactions_200126()
    {
        $bank_id = $this->input->post('bank_id');

...
class Ordersnostock_purchases extends Admin_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('ordersnostock_model');
    ...
    function index()
    {
        ...
        $this->session->set_userdata('sub_menu', 'Quotenostock/index');
        ...
        $this->load->view('admin/quotenostock/list', $data);
    ...
    public function data()
    {
        ...
        $result = $this->ordersnostock_model->getListData();
    ...
    public function form()
    {
        ...
        $this->session->set_userdata('top_menu', 'Inventory');
        $this->session->set_userdata('sub_menu', 'Ordersnostock/index');
        ...
        // Chargement des vues
        $this->load->view('layout/header', $data);
        $this->load->view('admin/ordersnostock/form', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    {   
        ...
        try {
            ...
            $this->form_validation->set_rules('valid_until', 'Date de validité', 'required');
            $this->form_validation->set_rules('item_category[]', 'Catégorie', 'required');
            $this->form_validation->set_rules('item[]', 'Article', 'required');
            ...
            $this->form_validation->set_rules('tva_rate', 'Taux de TVA', 'numeric');
            $this->form_validation->set_rules('tva_amount', 'Montant TVA', 'numeric');
            ...
            $data = [
                ...
                'apply_tva' => $this->input->post('apply_tva', 0),
                'tva_rate' => $this->input->post('tva_rate', 0),
                'tva_amount' => $this->input->post('tva_amount', 0),
            ...
            $insert_id = $this->quotenostock_model->add($data);
    ...
    public function view($id)
    {
        ...
        if (!$this->rbac->hasPrivilege('Quotenostock', 'can_view')) {
            access_denied();
        }

        // Récupération des données du devis
        $data['quote'] = $this->quotenostock_model->getQuoteWithItems($id);
        ...
        // Chargement des vues
        $this->load->view('layout/header', $data);
        $this->load->view('admin/quotenostock/view', $data);
    ...
    {
        // Vérification des permissions
        if (!$this->rbac->hasPrivilege('Quotenostock', 'can_edit')) {
            access_denied();
        }
        ...
        try {
            // Récupération des données du devis
            $data['quote'] = $this->quotenostock_model->getQuoteWithItems($quote_id);
            if (!$data['quote']) {
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            $quote = $this->quotenostock_model->getQuoteWithItems($id);
            if (!$quote) {
            ...
            // Chargement des vues
            $this->load->view('layout/header', $data);
            $this->load->view('admin/quotenostock/edit', $data);
            $this->load->view('layout/footer', $data);
        } catch (Exception $e) {
    ...
    public function update()
    {
        ...
        try {
            ...
            $update_success = $this->quotenostock_model->update($data);
    ...
    private function createDeliveryFromOrder($quote_id, $order_id)
    {
        ...
        $quote = $this->quotenostock_model->getQuoteWithItems($quote_id);
        if (!$quote) {
            return false;
        }
    ...
    public function validate()
    {
        ...
        try {
            ...
            if ($this->quotenostock_model->isQuoteValidated($id)) {
                throw new Exception('Ce devis est déjà validé');
            }
            if ($this->quotenostock_model->isQuoteRejected($id)) {
            ...
            if (!$this->quotenostock_model->validateQuote($id, $data)) {
    ...
    public function reject()
    {
        ...
        try {
            ...
            if (!$this->quotenostock_model->rejectQuote($id, $data)) {
    ...
    public function print()
    {
        ...
        $invoice_page = $this->load->view('admin/quotenostock/print', $data, true);

...
class Deliveryitem_supplier extends Admin_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('delivery_supplier_model');
    ...
    function index() {
        ...
        $this->session->set_userdata('sub_menu', 'Deliveryitem_supplier/index');
        ...
        $this->load->view('admin/itemdelivery_supplier/list', $data);
    ...
    public function data()
    {
        ...
        $result = $this->delivery_supplier_model->getListData();
    ...
    public function form() {
        ...
        $data = [
            ...
            'clients' => $this->itemsupplier_model->get()
        ];

        // Chargement des vues
        $this->load->view('layout/header', $data);
        $this->load->view('admin/itemdelivery_supplier/form', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add()
    {
        ...
        try {
            ...
            $this->form_validation->set_rules('item_category[]', 'Catégorie', 'required'); // Note: item_category au lieu de item_category_id[]
            $this->form_validation->set_rules('item_name[]', 'Article', 'required'); // Note: item_name au lieu de item_id[]
            ...
            $data = [
                ...
                'deadline' => $this->input->post('valid_until') ? date('Y-m-d', strtotime(str_replace('/', '-', $this->input->post('valid_until')))) : null,
                ...
                'tva_rate' => $this->input->post('tva_rate') ?? 0,
                'tva_amount' => $this->input->post('tva_amount') ?? 0, // Adaptation pour le nouveau formulaire
                'total_ht' => $this->input->post('total_ht') ?? 0, // Adaptation pour le nouveau formulaire
                'total_ttc' => $this->input->post('total_ttc') ?? 0, // Adaptation pour le nouveau formulaire
            ...
            $items = $this->input->post('item_name'); // Adaptation: item_name au lieu de item_id
            ...
            // Construction du tableau d'articles avec gestion des catégories et articles
            foreach ($categories as $index => $category_name) {
                if (empty($items[$index]) || empty($quantities[$index]) || empty($prices[$index])) {
                    throw new Exception('Données d\'article manquantes à la ligne ' . ($index + 1));
                }
                ...
                // Gestion de la catégorie et de l'article
                $category_id = $this->getOrCreateCategory($category_name);
                $item_id = $this->getOrCreateItem($items[$index], $category_id, $units[$index] ?? '', $price);
            ...
            // Enregistrement des données via le modèle
            $insert_id = $this->delivery_supplier_model->add($data);
    ...
    public function view($id)
    {
        ...
        $data['delivery'] = $this->delivery_supplier_model->getDeliveryWithItems($id);
        ...
        if (!$data['delivery']) {
            ...
            redirect('admin/itemdelivery_supplier');
        }
        ...
        // Chargement des vues
        $this->load->view('layout/header', $data);
        $this->load->view('admin/itemdelivery_supplier/view', $data);
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            if (!$delivery) {
                ...
                redirect('admin/deliveryitem_supplier');
            }
            ...
            // Chargement des vues
            $this->load->view('layout/header', $data);
            $this->load->view('admin/delivery_supplier/edit', $data);
            $this->load->view('layout/footer', $data);
    ...
    public function update() {
        ...
        try {
            ...
            $update_success = $this->delivery_supplier_model->update($data);
    ...
    private function createInvoiceFromDelivery($delivery_id) {
        ...
        $delivery = $this->delivery_supplier_model->getDeliveryWithItems($delivery_id);
        if (!$delivery) {
            return false;
        }
        ...
        try {
            ...
            foreach ($delivery['items'] as $item) {
                ...
                if (!$this->db->insert('invoice_supplier_items', $invoice_item_data)) {
    ...
    public function completeDelivery() {
        ...
        try {
            ...
            if ($this->delivery_supplier_model->isDeliveryCompleted($id)) {
                throw new Exception('Ce bon de livraison est déjà complété');
            }
            if ($this->delivery_supplier_model->isDeliveryCancelled($id)) {
            ...
            if (!$this->delivery_supplier_model->completeDelivery($id, $data)) {
    ...
    public function cancelDelivery() {
        ...
        try {
            ...
            if (!$this->delivery_supplier_model->cancelDelivery($id, $data)) {
    ...
    public function print()
    {
        ...
        // â Génération du QR Code avec numéro + client
        ...
        $invoice_page = $this->load->view('admin/itemdelivery_supplier/print', $data, true);
    ...
    public function partialDelivery($id) {
        ...
        try {
            ...
            // Chargement des vues
            $this->load->view('layout/header', $data);
            $this->load->view('admin/itemdelivery_supplier/validate', $data);
            $this->load->view('layout/footer', $data);
    ...
    public function setPartialDelivery() {
        ...
        try {
            ...
            // Mise à jour des données
            $update_success = $this->delivery_supplier_model->partialDelivery($data);
    ...
    public function sendEmail()
    {
        ...
        try {
            // Récupération des données du bon de livraison
            $data['delivery'] = $this->delivery_supplier_model->getDeliveryWithItems($delivery_id);
            if (!$data['delivery']) {
